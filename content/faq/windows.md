...
1. 请 **不要** 直接将 Python 编译到你的 .exe 文件中。 在 Windows 上，Python 必须是一个 DLL 以便处理导入本身就是 DLL 的模块。 （这是首先要知道的未写入文档的关键事实。） 正确的做法，应该是链接到 `python*NN*.dll`；它通常安装在 `C:\Windows\System` 中。 *NN* 是 Python 的版本号，例如数字 "33" 代表 Python 3.3。
