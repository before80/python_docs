...
​	整型 ([`int`]({{< ref "/library/functions#int" >}}))
...
​	The [loader]({{< ref "/glossary/idx#term-loader" >}}) object that the import machinery used to load the module.
...
​	It is **strongly** recommended that you use [`module.__spec__.cached`]({{< ref "/library/modules/importlib#importlib.machinery.ModuleSpec.cached" >}}) instead of `module.__cached__`.
...
​	属性赋值和删除会更新实例的字典，但绝不会更新类的字典。 如果类具有 [`__setattr__()`]({{< ref "/reference/datamodel#object.__setattr__" >}}) 或 [`__delattr__()`]({{< ref "/reference/datamodel#object.__delattr__" >}}) 方法，则将调用该方法而不再直接更新实例的字典。

​	如果类实例具有某些特殊名称的方法，就可以伪装为数字、序列或映射。参见 [特殊方法名称]({{< ref "/reference/datamodel#specialnames" >}}) 一节。
...
​	一个用于存储对象的（可写）属性的字典或其他映射对象。 并非所有实例都具有 `__dict__` 属性；请参阅 [__slots__]({{< ref "/reference/datamodel#slots" >}}) 章节了解详情。

### 3.2.12. I/O 对象 (或称文件对象)

[file object]({{< ref "/glossary/idx#term-file-object" >}}) 表示一个打开的文件。有多种快捷方式可用来创建文件对象: [`open()`]({{< ref "/library/functions#open" >}}) 内置函数，以及 [`os.popen()`]({{< ref "/library/allos/os#os.popen" >}}), [`os.fdopen()`]({{< ref "/library/allos/os#os.fdopen" >}}) 和 socket 对象的 [`makefile()`]({{< ref "/library/ipc/socket#socket.socket.makefile" >}}) 方法 (还可能使用某些扩展模块所提供的其他函数或方法)。

`sys.stdin`, `sys.stdout` 和 `sys.stderr` 会初始化为对应于解释器标准输入、输出和错误流的文件对象；它们都会以文本模式打开，因此都遵循 [`io.TextIOBase`]({{< ref "/library/allos/io#io.TextIOBase" >}}) 抽象类所定义的接口。
...
​	代码对象表示 *编译为字节的* 可执行 Python 代码，或称 [bytecode]({{< ref "/glossary/idx#term-bytecode" >}})。代码对象和函数对象的区别在于函数对象包含对函数全局对象 (函数所属的模块) 的显式引用，而代码对象不包含上下文；而且默认参数值会存放于函数对象而不是代码对象内 (因为它们表示在运行时算出的值)。与函数对象不同，代码对象不可变，也不包含对可变对象的引用 (不论是直接还是间接)。
...
| codeobject.**co_argcount**        | 函数的位置 [形参]({{< ref "/glossary/idx#term-parameter" >}}) 的总数（包括仅限位置形参和具有默认值的形参） |
| codeobject.**co_posonlyargcount** | 函数的仅限位置 [形参]({{< ref "/glossary/idx#term-parameter" >}}) 的总数（包括具有默认值的参数） |
| codeobject.**co_kwonlyargcount**  | 函数的仅限关键字 [形参]({{< ref "/glossary/idx#term-parameter" >}}) 的数量（包括具有默认值的参数） |
| codeobject.**co_nlocals**         | 函数使用的 [局部变量]({{< ref "/reference/executionmodel#naming" >}}) 的数量（包括形参） |
| codeobject.**co_varnames**        | 一个 [`tuple`]({{< ref "/library/stdtypes#tuple" >}})，其中包含函数中局部变量的名称（从形参名称开始） |
| codeobject.**co_cellvars**        | 包含被函数内至少一个 [nested scope]({{< ref "/glossary/idx#term-nested-scope" >}}) 所引用的 [局部变量]({{< ref "/reference/executionmodel#naming" >}}) 的名称的 [`tuple`]({{< ref "/library/stdtypes#tuple" >}})。 |
| codeobject.**co_freevars**        | A [`tuple`]({{< ref "/library/stdtypes#tuple" >}}) containing the names of [free (closure) variables]({{< ref "/glossary/idx#term-closure-variable" >}}) that a [nested scope]({{< ref "/glossary/idx#term-nested-scope" >}}) references in an outer scope. See also [`function.__closure__`]({{< ref "/reference/datamodel#function.__closure__" >}}).Note: references to global and builtin names are *not* included. |
| codeobject.**co_code**            | 一个表示函数中的 [bytecode]({{< ref "/glossary/idx#term-bytecode" >}}) 指令序列的字符串 |
| codeobject.**co_consts**          | 一个包含函数中的 [bytecode]({{< ref "/glossary/idx#term-bytecode" >}}) 所使用的字面值的 [`tuple`]({{< ref "/library/stdtypes#tuple" >}}) |
| codeobject.**co_names**           | 一个包含函数中的 [bytecode]({{< ref "/glossary/idx#term-bytecode" >}}) 所使用的名称的 [`tuple`]({{< ref "/library/stdtypes#tuple" >}}) |
| codeobject.**co_filename**        | 被编译代码所在文件的名称                                     |
| codeobject.**co_firstlineno**     | 函数第一行所对应的行号                                       |
| codeobject.**co_lnotab**          | 一个编码了从 [bytecode]({{< ref "/glossary/idx#term-bytecode" >}}) 偏移量到行号的映射的字符串。 要获取更多细节，请查看解释器的源代码。自 3.12 版本弃用: This attribute of code objects is deprecated, and may be removed in Python 3.15. |
...
​	典型的实现会附带适当的参数使用 `super().__new__(cls[, ...])` 通过发起调用超类的 [`__new__()`]({{< ref "/reference/datamodel#object.__new__" >}}) 方法来创建一个新的类实例然后在返回它之前根据需要修改新创建的实例。
...
​	请参看 [格式规格迷你语言]({{< ref "/library/text/string#formatspec" >}}) 了解标准格式化语法的描述。
...
​	当没有合适的方法返回任何 [`NotImplemented`]({{< ref "/library/constants#NotImplemented" >}}) 以外的值时，`==` 和 `!=` 运算符将分别回退至 `is` 和 `is not`。
...
​	对于特定的敏感属性访问，引发一个 [审计事件]({{< ref "/library/python/sys#auditing" >}}) `object.__getattr__`，附带参数 `obj` 和 `name`。
...
​	The following methods only apply when an instance of the class containing the method (a so-called *descriptor* class) appears in an *owner* class (the descriptor must be in either the owner's class dictionary or in the class dictionary for one of its parents). In the examples below, "the attribute" refers to the attribute whose name is the key of the property in the owner class' [`__dict__`]({{< ref "/reference/datamodel#object.__dict__" >}}). The [`object`]({{< ref "/library/functions#object" >}}) class itself does not implement any of these protocols.

##### object.`__get__`(*self*, *instance*, *owner=None*)

​	调用此方法以获取所有者类的属性（类属性访问）或该类的实例的属性（实例属性访问）。 可选的 *owner* 参数是所有者类而 *instance* 是被用来访问属性的实例，如果通过 *owner* 来访问属性则返回 `None`。

​	此方法应当返回计算得到的属性值或是引发 [`AttributeError`]({{< ref "/library/exceptions#AttributeError" >}}) 异常。

[**PEP 252**](https://peps.python.org/pep-0252/) 指明 [`__get__()`]({{< ref "/reference/datamodel#object.__get__" >}}) 为带有一至二个参数的可调用对象。 Python 自身内置的描述器支持此规格定义；但是，某些第三方工具可能要求必须带两个参数。 Python 自身的 [`__getattribute__()`]({{< ref "/reference/datamodel#object.__getattribute__" >}}) 实现总是会传入两个参数，无论它们是否被要求提供。

##### object.`__set__`(*self*, *instance*, *value*)

​	调用此方法以设置 *instance* 指定的所有者类的实例的属性为新值 *value*。

​	请注意，添加 [`__set__()`]({{< ref "/reference/datamodel#object.__set__" >}}) 或 [`__delete__()`]({{< ref "/reference/datamodel#object.__delete__" >}}) 会将描述器变成“数据描述器”。 更多细节请参阅 [调用描述器]({{< ref "/reference/datamodel#descriptor-invocation" >}})。
...
​	属性 `__objclass__` 会被 [`inspect`]({{< ref "/library/python/inspect#module-inspect" >}}) 模块解读为指定此对象定义所在的类（正确设置此属性有助于动态类属性的运行时内省）。 对于可调用对象来说，它可以指明预期或要求提供一个特定类型（或子类）的实例作为第一个位置参数（例如，CPython 会为在 C 中实现的未绑定方法设置此属性）。



#### 3.3.2.3. 调用描述器

​	总的说来，描述器就是具有“绑定行为”的对象属性，其属性访问已被描述器协议中的方法所重载: [`__get__()`]({{< ref "/reference/datamodel#object.__get__" >}}), [`__set__()`]({{< ref "/reference/datamodel#object.__set__" >}}) 和 [`__delete__()`]({{< ref "/reference/datamodel#object.__delete__" >}})。 如果一个对象定义了以上方法中的任意一个，它就被称为描述器。
...
​	对于实例绑定，发起描述器调用的优先级取决于定义了哪些描述器方法。 一个描述器可以定义 [`__get__()`]({{< ref "/reference/datamodel#object.__get__" >}}), [`__set__()`]({{< ref "/reference/datamodel#object.__set__" >}}) 和 [`__delete__()`]({{< ref "/reference/datamodel#object.__delete__" >}}) 的任意组合。 如果它没有定义 `__get__()`，则访问属性将返回描述器对象自身，除非对象的实例字典中有相应的属性值。 如果描述器定义了 `__set__()` 和/或 `__delete__()`，则它是一个数据描述器；如果两者均未定义，则它是一个非数据描述器。 通常，数据描述器会同时定义 `__get__()` 和 `__set__()`，而非数据描述器则只有 `__get__()` 方法。 定义了 `__get__()` 和 `__set__()` (和/或 `__delete__()`) 的数据描述器总是会重载实例字典中的定义。 与之相对地，非数据描述器则可被实例所重载。

​	Python 方法（包括用 [`@staticmethod`]({{< ref "/library/functions#staticmethod" >}}) 和 [`@classmethod`]({{< ref "/library/functions#classmethod" >}}) 装饰的方法）都是作为非数据描述器来实现的。 因而，实例可以重定义和重写方法。 这允许单个实例获得与相同类的其他实例不一样的行为。

[`property()`]({{< ref "/library/functions#property" >}}) 函数是作为数据描述器来实现的。因此实例不能重载特性属性的行为。



#### 3.3.2.4. __slots__

*__slots__* 允许我们显式地声明数据成员（如特征属性）并禁止创建 [`__dict__`]({{< ref "/reference/datamodel#object.__dict__" >}}) 和 *__weakref__* (除非是在 *__slots__* 中显式地声明或是在父类中可用。)

​	相比使用 [`__dict__`]({{< ref "/reference/datamodel#object.__dict__" >}}) 可以显著节省空间。 属性查找速度也可得到显著的提升。

##### object.`__slots__`

​	这个类变量可赋值为字符串、可迭代对象或由实例使用的变量名组成的字符串序列。 *__slots__* 会为已声明的变量保留空间并阻止自动为每个实例创建 [`__dict__`]({{< ref "/reference/datamodel#object.__dict__" >}}) 和 *__weakref__*。

​	使用 *__slots__* 的注意事项:

- 当继承自一个没有 *__slots__* 的类时，实例的 [`__dict__`]({{< ref "/reference/datamodel#object.__dict__" >}}) 和 *__weakref__* 属性将总是可访问的。
- 没有 [`__dict__`]({{< ref "/reference/datamodel#object.__dict__" >}}) 变量，实例就不能给未在 *__slots__* 定义中列出的新变量赋值。 尝试给一个未列出的变量名赋值将引发 [`AttributeError`]({{< ref "/library/exceptions#AttributeError" >}})。 如果需要动态地给新变量赋值，则要将 `'__dict__'` 加入到在 *__slots__* 中声明的字符串序列中。
- 如果未给每个实例设置 *__weakref__* 变量，则定义了 *__slots__* 的类就不支持对其实例的 [`弱引用`]({{< ref "/library/datatypes/weakref#module-weakref" >}})。 如果需要支持弱引用，则要将 `'__weakref__'` 加入到在 *__slots__* 中声明的字符串序列中。
- *__slots__* 是通过为每个变量名创建 [描述器]({{< ref "/reference/datamodel#descriptors" >}}) 在类层级上实现的。 因此，类属性不能被用来为通过 *__slots__* 定义的实例变量设置默认值；否则，类属性将会覆盖描述器赋值。
- The action of a *__slots__* declaration is not limited to the class where it is defined. *__slots__* declared in parents are available in child classes. However, instances of a child subclass will get a [`__dict__`]({{< ref "/reference/datamodel#object.__dict__" >}}) and *__weakref__* unless the subclass also defines *__slots__* (which should only contain names of any *additional* slots).
- 如果一个类定义的位置在某个基类中也有定义，则由基类位置定义的实例变量将不可访问（除非通过直接从基类获取其描述器的方式）。这会使得程序的含义变成未定义。未来可能会添加一个防止此情况的检查。
- 如果为派生自 [`"variable-length" 内置类型`](https://docs.python.org/zh-cn/3.13/c-api/typeobj.html#c.PyTypeObject.tp_itemsize) 如 [`int`]({{< ref "/library/functions#int" >}}), [`bytes`]({{< ref "/library/stdtypes#bytes" >}}) 和 [`tuple`]({{< ref "/library/stdtypes#tuple" >}}) 的类定义了非空的*__slots__* 则将引发 [`TypeError`]({{< ref "/library/exceptions#TypeError" >}})。
- 任何非字符串的 [iterable]({{< ref "/glossary/idx#term-iterable" >}}) 都可以被赋值给 *__slots__*。
- 如果是使用一个 [`字典`]({{< ref "/library/stdtypes#dict" >}}) 来给 *__slots__* 赋值，则该字典的键将被用作槽位名称。 字典的值可被用来为每个属性提供将被 [`inspect.getdoc()`]({{< ref "/library/python/inspect#inspect.getdoc" >}}) 识别并在and displayed in the output of [`help()`]({{< ref "/library/functions#help" >}}) 的输出中显示的文档字符串。
- [`__class__`]({{< ref "/reference/datamodel#object.__class__" >}}) assignment works only if both classes have the same *__slots__*.
- 带有多槽位父类的 [多重继承]({{< ref "/tutorial/classes#tut-multiple" >}}) 也是可用的，但仅允许一个父类具有由槽位创建的属性（其他基类必须具有空的槽位布局） —— 违反此规则将引发 [`TypeError`]({{< ref "/library/exceptions#TypeError" >}})。
- 如果将 [iterator]({{< ref "/glossary/idx#term-iterator" >}}) 用于 *__slots__* 则会为该迭代器的每个值创建一个 [descriptor]({{< ref "/glossary/idx#term-descriptor" >}})。 但是，*__slots__* 属性将为一个空迭代器。



### 3.3.3. 自定义类创建

​	当一个类继承另一个类时，会在这个父类上调用 [`__init_subclass__()`]({{< ref "/reference/datamodel#object.__init_subclass__" >}})。 这样，就使得编写改变子类行为的类成为可能。 这与类装饰器有很密切的关联，但类装饰器只能影响它们所应用的特定类，而 `__init_subclass__` 则只作用于定义了该方法的类在未来的子类。
...
​	当一个类被创建时，`type.__new__()` 会扫描类变量并对其中带有 [`__set_name__()`]({{< ref "/reference/datamodel#object.__set_name__" >}}) 钩子的对象执行回调。
...
​	如果类变量赋值是在类被创建之后进行的，[`__set_name__()`]({{< ref "/reference/datamodel#object.__set_name__" >}}) 将不会被自动调用。 如有必要，可以直接调用 [`__set_name__()`]({{< ref "/reference/datamodel#object.__set_name__" >}}):
...
​	详情参见 [创建类对象]({{< ref "/reference/datamodel#class-object-creation" >}})。
...
​	默认情况下，类是使用 [`type()`]({{< ref "/library/functions#type" >}}) 来构建的。类体会在一个新的命名空间内执行，类名会被局部绑定到 `type(name, bases, namespace)` 的结果。
...
​	如果一个出现在类定义中的基类不是 [`type`]({{< ref "/library/functions#type" >}}) 的实例，则会在该基类中搜索 `__mro_entries__()` 方法。 如果找到了 `__mro_entries__()` 方法，则在创建类时该基类会被替换为调用 `__mro_entries__()` 的结果。 该方法被调用时将附带传给 *bases* 形参的原始基类元组，并且必须返回一个由将被用来替代该基类的类组成的元组。 返回的元组可能为空：在此情况下，原始基类将被忽略。

> 参见
>
> [`types.resolve_bases()`]({{< ref "/library/datatypes/types#types.resolve_bases" >}})
>
> ​	动态地解析不属于 [`type`]({{< ref "/library/functions#type" >}}) 实例的基类。
>
> [`types.get_original_bases()`]({{< ref "/library/datatypes/types#types.get_original_bases" >}})
>
> ​	在类被 [`__mro_entries__()`]({{< ref "/reference/datamodel#object.__mro_entries__" >}}) 修改之前提取其“原始基类”。
>
...
- 如果没有基类且没有显式指定元类，则使用 [`type()`]({{< ref "/library/functions#type" >}})；
- 如果给出一个显式元类而且 *不是* [`type()`]({{< ref "/library/functions#type" >}}) 的实例，则其会被直接用作元类；
- 如果给出一个 [`type()`]({{< ref "/library/functions#type" >}}) 的实例作为显式元类，或是定义了基类，则使用最近派生的元类。
...
​	一旦确定了适当的元类，就开始准备类的命名空间。 如果元类具有 `__prepare__` 属性，它将以 `namespace = metaclass.__prepare__(name, bases, **kwds)` 的形式被调用（其中如果存在任何额外关键字参数，则应来自类定义）。 `__prepare__` 方法应当被实现为 [`类方法`]({{< ref "/library/functions#classmethod" >}})。 `__prepare__` 所返回的命名空间会被传入 `__new__`，但是当最终的类对象被创建时该命名空间会被拷贝到一个新的 `dict` 中。
...
​	类主体会以（类似于） `exec(body, globals(), namespace)` 的形式被执行。普通调用与 [`exec()`]({{< ref "/library/functions#exec" >}}) 的关键区别在于当类定义发生于函数内部时，词法作用域允许类主体（包括任何方法）引用来自当前和外部作用域的名称。
...
​	如果类主体中有任何方法引用了 `__class__` 或 `super`，这个类对象会通过零参数形式的 [`super()`]({{< ref "/library/functions#super" >}}). `__class__` 所引用，这是由编译器所创建的隐式闭包引用。这使用零参数形式的 [`super()`]({{< ref "/library/functions#super" >}}) 能够正确标识正在基于词法作用域来定义的类，而被用于进行当前调用的类或实例则是基于传递给方法的第一个参数来标识的。

**CPython 实现细节：** 在 CPython 3.6 及之后的版本中，`__class__` 单元会作为类命名空间中的 `__classcell__` 条目被传给元类。 如果存在，它必须被向上传播给 `type.__new__` 调用，以便能正确地初始化该类。 如果不这样做，在 Python 3.8 中将引发 [`RuntimeError`]({{< ref "/library/exceptions#RuntimeError" >}})。

​	当使用默认的元类 [`type`]({{< ref "/library/functions#type" >}})，或者任何最终会调用 `type.__new__` 的元类时，以下额外的自定义步骤将在创建类对象之后被发起调用:

1. `type.__new__` 方法会收集类命名空间中所有定义了 [`__set_name__()`]({{< ref "/reference/datamodel#object.__set_name__" >}}) 方法的属性;
2. 这些 `__set_name__` 方法将附带所定义的类和指定的属性所赋的名称进行调用;
3. 在新类基于方法解析顺序所确定的直接父类上调用 [`__init_subclass__()`]({{< ref "/reference/datamodel#object.__init_subclass__" >}}) 钩子。

​	在类对象创建之后，它会被传给包含在类定义中的类装饰器（如果有的话），得到的对象将作为已定义的类绑定到局部命名空间。

​	When a new class is created by `type.__new__`, the object provided as the namespace parameter is copied to a new ordered mapping and the original object is discarded. The new copy is wrapped in a read-only proxy, which becomes the [`__dict__`]({{< ref "/reference/datamodel#type.__dict__" >}}) attribute of the class object.
...
​	以下方法被用来重载 [`isinstance()`]({{< ref "/library/functions#isinstance" >}}) 和 [`issubclass()`]({{< ref "/library/functions#issubclass" >}}) 内置函数的默认行为。

​	特别地，元类 [`abc.ABCMeta`]({{< ref "/library/python/abc#abc.ABCMeta" >}}) 实现了这些方法以便允许将抽象基类（ABC）作为“虚拟基类”添加到任何类或类型（包括内置类型），包括其他 ABC 之中。
...
​	Includes the specification for customizing [`isinstance()`]({{< ref "/library/functions#isinstance" >}}) and [`issubclass()`]({{< ref "/library/functions#issubclass" >}}) behavior through [`__instancecheck__()`]({{< ref "/reference/datamodel#type.__instancecheck__" >}}) and [`__subclasscheck__()`]({{< ref "/reference/datamodel#type.__subclasscheck__" >}}), with motivation for this functionality in the context of adding Abstract Base Classes (see the [`abc`]({{< ref "/library/python/abc#module-abc" >}}) module) to the language.

### 3.3.5. 模拟泛型类型

​	当使用 [类型标注]({{< ref "/glossary/idx#term-annotation" >}}) 时，使用 Python 的方括号标记来 *形参化* 一个 [generic type]({{< ref "/glossary/idx#term-generic-type" >}}) 往往会很有用处。 例如，`list[int]` 这样的标注可以被用来表示一个 [`list`]({{< ref "/library/stdtypes#list" >}}) 中的所有元素均为 [`int`]({{< ref "/library/functions#int" >}}) 类型。
...
​	当在类上定义时，`__class_getitem__()` 会自动成为类方法。 因此，当它被定义时没有必要使用 [`@classmethod`]({{< ref "/library/functions#classmethod" >}}) 来装饰。

#### 3.3.5.1. *__class_getitem__* 的目的

[`__class_getitem__()`]({{< ref "/reference/datamodel#object.__class_getitem__" >}}) 的目的是允许标准库泛型类的运行时形参化以更方便地对这些类应用 [类型提示]({{< ref "/glossary/idx#term-type-hint" >}})。

​	要实现可以在运行时被形参化并可被静态类型检查所理解的自定义泛型类，用户应当从已经实现了 [`__class_getitem__()`]({{< ref "/reference/datamodel#object.__class_getitem__" >}}) 的标准库类继承，或是从 [`typing.Generic`]({{< ref "/library/development/typing#typing.Generic" >}}) 继承，这个类拥有自己的 `__class_getitem__()` 实现。

​	标准库以外的类上的 [`__class_getitem__()`]({{< ref "/reference/datamodel#object.__class_getitem__" >}}) 自定义实现可能无法被第三方类型检查器如 mypy 所理解。 不建议在任何类上出于类型提示以外的目的使用 `__class_getitem__()`。



#### 3.3.5.2. *__class_getitem__* 与 *__getitem__*

​	通常，使用方括号语法 [抽取]({{< ref "/reference/expressions#subscriptions" >}}) 一个对象将会调用在该对象的类上定义的 [`__getitem__()`]({{< ref "/reference/datamodel#object.__getitem__" >}}) 实例方法。 不过，如果被拟抽取的对象本身是一个类，则可能会调用 [`__class_getitem__()`]({{< ref "/reference/datamodel#object.__class_getitem__" >}}) 类方法。 `__class_getitem__()` 如果被正确地定义，则应当返回一个 [GenericAlias]({{< ref "/library/stdtypes#types-genericalias" >}}) 对象。

​	使用 [表达式]({{< ref "/glossary/idx#term-expression" >}}) `obj[x]` 来呈现，Python 解释器会遵循下面这样的过程来确定应当调用 [`__getitem__()`]({{< ref "/reference/datamodel#object.__getitem__" >}}) 还是 [`__class_getitem__()`]({{< ref "/reference/datamodel#object.__class_getitem__" >}}):
...
​	在 Python 中，所有的类自身也是其他类的实例。 一个类所属的类被称为该类的 [metaclass]({{< ref "/glossary/idx#term-metaclass" >}})，并且大多数类都将 [`type`]({{< ref "/library/functions#type" >}}) 类作为它们的元类。 [`type`]({{< ref "/library/functions#type" >}}) 没有定义 [`__getitem__()`]({{< ref "/reference/datamodel#object.__getitem__" >}})，这意味着 `list[int]`, `dict[str, float]` 和 `tuple[str, bytes]` 这样的表达式都将导致 [`__class_getitem__()`]({{< ref "/reference/datamodel#object.__class_getitem__" >}}) 被调用:
...
​	然而，如果一个类属于定义了 [`__getitem__()`]({{< ref "/reference/datamodel#object.__getitem__" >}}) 的自定义元类，则抽取该类可能导致不同的行为。 这方面的一个例子可以在 [`enum`]({{< ref "/library/datatypes/enum#module-enum" >}}) 模块中找到:
...
>
> ​	介绍 [`__class_getitem__()`]({{< ref "/reference/datamodel#object.__class_getitem__" >}})，并指明 [抽取]({{< ref "/reference/expressions#subscriptions" >}}) 在何时会导致 `__class_getitem__()` 而不是 [`__getitem__()`]({{< ref "/reference/datamodel#object.__getitem__" >}}) 被调用
>
...
​	Called when the instance is "called" as a function; if this method is defined, `x(arg1, arg2, ...)` roughly translates to `type(x).__call__(x, arg1, ...)`. The [`object`]({{< ref "/library/functions#object" >}}) class itself does not provide this method.



### 3.3.7. 模拟容器类型

​	The following methods can be defined to implement container objects. None of them are provided by the [`object`]({{< ref "/library/functions#object" >}}) class itself. Containers usually are [sequences]({{< ref "/glossary/idx#term-sequence" >}}) (such as [`lists`]({{< ref "/library/stdtypes#list" >}}) or [`tuples`]({{< ref "/library/stdtypes#tuple" >}})) or [mappings]({{< ref "/glossary/idx#term-mapping" >}}) (like [dictionaries]({{< ref "/glossary/idx#term-dictionary" >}})), but can represent other containers as well. The first set of methods is used either to emulate a sequence or to emulate a mapping; the difference is that for a sequence, the allowable keys should be the integers *k* for which `0 <= k < N` where *N* is the length of the sequence, or [`slice`]({{< ref "/library/functions#slice" >}}) objects, which define a range of items. It is also recommended that mappings provide the methods `keys()`, `values()`, `items()`, `get()`, `clear()`, `setdefault()`, `pop()`, `popitem()`, `copy()`, and `update()` behaving similar to those for Python's standard [`dictionary`]({{< ref "/library/stdtypes#dict" >}}) objects. The [`collections.abc`]({{< ref "/library/datatypes/collections_abc#module-collections.abc" >}}) module provides a [`MutableMapping`]({{< ref "/library/datatypes/collections_abc#collections.abc.MutableMapping" >}}) [abstract base class]({{< ref "/glossary/idx#term-abstract-base-class" >}}) to help create those methods from a base set of [`__getitem__()`]({{< ref "/reference/datamodel#object.__getitem__" >}}), [`__setitem__()`]({{< ref "/reference/datamodel#object.__setitem__" >}}), [`__delitem__()`]({{< ref "/reference/datamodel#object.__delitem__" >}}), and `keys()`. Mutable sequences should provide methods `append()`, `count()`, `index()`, `extend()`, `insert()`, `pop()`, `remove()`, `reverse()` and `sort()`, like Python standard [`list`]({{< ref "/library/stdtypes#list" >}}) objects. Finally, sequence types should implement addition (meaning concatenation) and multiplication (meaning repetition) by defining the methods [`__add__()`]({{< ref "/reference/datamodel#object.__add__" >}}), [`__radd__()`]({{< ref "/reference/datamodel#object.__radd__" >}}), [`__iadd__()`]({{< ref "/reference/datamodel#object.__iadd__" >}}), [`__mul__()`]({{< ref "/reference/datamodel#object.__mul__" >}}), [`__rmul__()`]({{< ref "/reference/datamodel#object.__rmul__" >}}) and [`__imul__()`]({{< ref "/reference/datamodel#object.__imul__" >}}) described below; they should not define other numerical operators. It is recommended that both mappings and sequences implement the [`__contains__()`]({{< ref "/reference/datamodel#object.__contains__" >}}) method to allow efficient use of the `in` operator; for mappings, `in` should search the mapping's keys; for sequences, it should search through the values. It is further recommended that both mappings and sequences implement the [`__iter__()`]({{< ref "/reference/datamodel#object.__iter__" >}}) method to allow efficient iteration through the container; for mappings, `__iter__()` should iterate through the object's keys; for sequences, it should iterate through the values.

#### object.`__len__`(*self*)

​	调用此方法以实现内置函数 [`len()`]({{< ref "/library/functions#len" >}})。 应该返回对象的长度，以一个 `>=` 0 的整数表示。 此外，如果一个对象未定义 [`__bool__()`]({{< ref "/reference/datamodel#object.__bool__" >}}) 方法而其 `__len__()` 方法返回值为零则它在布尔运算中将被视为具有假值。

**CPython 实现细节：** 在 CPython 中，要求长度最大只能为 [`sys.maxsize`]({{< ref "/library/python/sys#sys.maxsize" >}})。 如果长度大于 `sys.maxsize` 则某些特性 (如 [`len()`]({{< ref "/library/functions#len" >}})) 可能会引发 [`OverflowError`]({{< ref "/library/exceptions#OverflowError" >}})。 要防止真值测试引发 `OverflowError`，对象必须定义 [`__bool__()`]({{< ref "/reference/datamodel#object.__bool__" >}}) 方法。

#### object.**__length_hint__**(*self*)

​	调用此方法以实现 [`operator.length_hint()`]({{< ref "/library/functional/operator#operator.length_hint" >}})。 应该返回对象长度的估计值（可能大于或小于实际长度）。 此长度应为一个 `>=` 0 的整数。 返回值也可以为 [`NotImplemented`]({{< ref "/library/constants#NotImplemented" >}}) ，这会被视作与 `__length_hint__` 方法完全不存在时一样处理。 此方法纯粹是为了优化性能，并不要求正确无误。
...
​	调用此方法以实现 `self[key]` 的求值。 对于 [sequence]({{< ref "/glossary/idx#term-sequence" >}}) 类型，接受的键应为整数。 作为可选项，它们也可能支持 [`slice`]({{< ref "/library/functions#slice" >}}) 对象。 对负数索引的支持也是可选项。 如果 *key* 的类型不正确，则可能引发 [`TypeError`]({{< ref "/library/exceptions#TypeError" >}})。 如果 *key* 为序列索引集合范围以外的值（在进行任何负数索引的特殊解读之后），则应当引发 [`IndexError`]({{< ref "/library/exceptions#IndexError" >}})。 对于 [mapping]({{< ref "/glossary/idx#term-mapping" >}}) 类型，如果 *key* 找不到（不在容器中），则应当引发 [`KeyError`]({{< ref "/library/exceptions#KeyError" >}})。

备注


[`for`]({{< ref "/reference/compound_stmts#for" >}}) 循环在有不合法索引时会期待捕获 [`IndexError`]({{< ref "/library/exceptions#IndexError" >}}) 以便正确地检测到序列的结束。

备注


​	当 [抽取]({{< ref "/reference/expressions#subscriptions" >}}) 一个 *class* 时，可能会调用特殊类方法 [`__class_getitem__()`]({{< ref "/reference/datamodel#object.__class_getitem__" >}}) 而不是 `__getitem__()`。 请参阅 [__class_getitem__ 与 __getitem__]({{< ref "/reference/datamodel#classgetitem-versus-getitem" >}}) 了解详情。

#### object.`__setitem__`(*self*, *key*, *value*)

​	调用此方法以实现向 `self[key]` 赋值。注意事项与 [`__getitem__()`]({{< ref "/reference/datamodel#object.__getitem__" >}}) 相同。为对象实现此方法应该仅限于需要映射允许基于键修改值或添加键，或是序列允许元素被替换时。不正确的 *key* 值所引发的异常应与 [`__getitem__()`]({{< ref "/reference/datamodel#object.__getitem__" >}}) 方法的情况相同。

#### object.`__delitem__`(*self*, *key*)

​	调用此方法以实现 `self[key]` 的删除。注意事项与 [`__getitem__()`]({{< ref "/reference/datamodel#object.__getitem__" >}}) 相同。为对象实现此方法应该权限于需要映射允许移除键，或是序列允许移除元素时。不正确的 *key* 值所引发的异常应与 [`__getitem__()`]({{< ref "/reference/datamodel#object.__getitem__" >}}) 方法的情况相同。

#### object.`__missing__`(*self*, *key*)

​	此方法由 [`dict`]({{< ref "/library/stdtypes#dict" >}}).[`__getitem__()`]({{< ref "/reference/datamodel#object.__getitem__" >}}) 在找不到字典中的键时调用以实现 dict 子类的 `self[key]`。

#### object.`__iter__`(*self*)

​	此方法会在需要为一个容器创建 [iterator]({{< ref "/glossary/idx#term-iterator" >}}) 时被调用。 此方法应当返回一个新的迭代器对象，它可以对容器中的所有对象执行迭代。 对于映射，它应当对窗口中的键执行迭代。

#### object.`__reversed__`(*self*)

​	此方法（如果存在）会被 [`reversed()`]({{< ref "/library/functions#reversed" >}}) 内置函数调用以实现逆向迭代。它应当返回一个新的以逆序逐个迭代容器内所有对象的迭代器对象。

​	如果未提供 [`__reversed__()`]({{< ref "/reference/datamodel#object.__reversed__" >}}) 方法，则 [`reversed()`]({{< ref "/library/functions#reversed" >}}) 内置函数将回退到使用序列协议 ([`__len__()`]({{< ref "/reference/datamodel#object.__len__" >}}) 和 [`__getitem__()`]({{< ref "/reference/datamodel#object.__getitem__" >}}))。支持序列协议的对象应当仅在能够提供比 [`reversed()`]({{< ref "/library/functions#reversed" >}}) 所提供的实现更高效的实现时才提供 [`__reversed__()`]({{< ref "/reference/datamodel#object.__reversed__" >}}) 方法。

​	成员检测运算符 ([`in`]({{< ref "/reference/expressions#in" >}}) 和 [`not in`]({{< ref "/reference/expressions#not-in" >}})) 通常以对容器进行逐个迭代的方式来实现。 不过，容器对象可以提供以下特殊方法并采用更有效率的实现，这样也不要求对象必须为可迭代对象。

#### object.`__contains__`(*self*, *item*)

​	调用此方法以实现成员检测运算符。如果 *item* 是 *self* 的成员则应返回真，否则返回假。对于映射类型，此检测应基于映射的键而不是值或者键值对。

​	对于未定义 [`__contains__()`]({{< ref "/reference/datamodel#object.__contains__" >}}) 的对象，成员检测将首先尝试通过 [`__iter__()`]({{< ref "/reference/datamodel#object.__iter__" >}}) 进行迭代，然后再使用 [`__getitem__()`]({{< ref "/reference/datamodel#object.__getitem__" >}}) 的旧式序列迭代协议，参看 [语言参考中的相应部分]({{< ref "/reference/expressions#membership-test-details" >}})。
...
​	调用这些方法来实现双目算术运算 (`+`, `-`, `*`, `@`, `/`, `//`, `%`, [`divmod()`]({{< ref "/library/functions#divmod" >}}), [`pow()`]({{< ref "/library/functions#pow" >}}), `**`, `<<`, `>>`, `&`, `^`, `|`)。 例如，求表达式 `x + y` 的值，其中 *x* 是具有 [`__add__()`]({{< ref "/reference/datamodel#object.__add__" >}}) 方法的类的一个实例，则会调用 `type(x).__add__(x, y)`。 [`__divmod__()`]({{< ref "/reference/datamodel#object.__divmod__" >}}) 方法应该等价于使用 [`__floordiv__()`]({{< ref "/reference/datamodel#object.__floordiv__" >}}) 和 [`__mod__()`]({{< ref "/reference/datamodel#object.__mod__" >}})；它不应该被关联到 [`__truediv__()`]({{< ref "/reference/datamodel#object.__truediv__" >}})。 请注意如果要支持三目版本的内置 [`pow()`]({{< ref "/library/functions#pow" >}}) 函数则 [`__pow__()`]({{< ref "/reference/datamodel#object.__pow__" >}}) 应当被定义为接受可选的第三个参数。

​	如果这些方法中的某一个不支持与所提供参数进行运算，它应该返回 [`NotImplemented`]({{< ref "/library/constants#NotImplemented" >}}) 。
...
​	调用这些方法来实现具有反射（交换）操作数的双目算术运算 ( `+`, `- ``, ``*`, `@`, `/`, `//`, `%`, [`divmod()`]({{< ref "/library/functions#divmod" >}}), [`pow()`]({{< ref "/library/functions#pow" >}}), `**`, `<<`, `>>`, `&`, `^`, `|`)。 这些函数仅会在左操作数不支持相应运算 [[3\]]({{< ref "/reference/datamodel#id21" >}}) 且两个操作数类型不同时被调用。 [[4\]]({{< ref "/reference/datamodel#id22" >}}) 例 如，求表达式 `x - y` 的值，其中 *y* 是具有 [`__rsub__()`]({{< ref "/reference/datamodel#object.__rsub__" >}}) 方法的类的一个实例，则当 `type(x).__sub__(x, y)` 返回 [`NotImplemented`]({{< ref "/library/constants#NotImplemented" >}}) 时将会调用 `type(y).__rsub__(y, x)`。

​	请注意三元版的 [`pow()`]({{< ref "/library/functions#pow" >}}) 并不会尝试调用 [`__rpow__()`]({{< ref "/reference/datamodel#object.__rpow__" >}}) (因为强制转换规则会太过复杂)。
...
​	调用这些方法来实现增强算术赋值 (`+=`, `-=`, `*=`, `@=`, `/=`, `//=`, `%=`, `**=`, `<<=`, `>>=`, `&=`, `^=`, `|=`)。 这些方法应当尝试原地执行操作 (对 *self* 进行修改) 并返回结果 (结果可以为 *self* 但这并非必须)。 如果某个方法未被定义，或者如果该方法返回 [`NotImplemented`]({{< ref "/library/constants#NotImplemented" >}})，则相应的增强赋值将回退到普通方法。 举例来说，如果 *x* 是一个具有 [`__iadd__()`]({{< ref "/reference/datamodel#object.__iadd__" >}}) 方法的类的实例，则 `x += y` 就等价于 `x = x.__iadd__(y)`。 如果 [`__iadd__()`]({{< ref "/reference/datamodel#object.__iadd__" >}}) 不存在，或者如果 `x.__iadd__(y)` 返回 `NotImplemented`，则将使用 `x.__add__(y)` 和 `y.__radd__(x)`，如同对 `x + y` 求值一样。 在某些情况下，增强赋值可能导致未预期的错误 (参见 [为什么 a_tuple[i\] += ['item'] 会引发异常？]({{< ref "/faq/programming#faq-augmented-assignment-tuple-error" >}}))，但此行为实际上是数据模型的一部分。
...
​	调用此方法以实现一元算术运算 (`-`, `+`, [`abs()`]({{< ref "/library/functions#abs" >}}) 和 `~`)。
...
​	调用这些方法以实现内置函数 [`complex()`]({{< ref "/library/functions#complex" >}}), [`int()`]({{< ref "/library/functions#int" >}}) 和 [`float()`]({{< ref "/library/functions#float" >}})。应当返回一个相应类型的值。

#### object.`__index__`(*self*)

​	调用此方法以实现 [`operator.index()`]({{< ref "/library/functional/operator#operator.index" >}}) 以及 Python 需要无损地将数字对象转换为整数对象的场合（例如切片或是内置的 [`bin()`]({{< ref "/library/functions#bin" >}}), [`hex()`]({{< ref "/library/functions#hex" >}}) 和 [`oct()`]({{< ref "/library/functions#oct" >}}) 函数)。 存在此方法表明数字对象属于整数类型。 必须返回一个整数。

​	如果未定义 [`__int__()`]({{< ref "/reference/datamodel#object.__int__" >}}), [`__float__()`]({{< ref "/reference/datamodel#object.__float__" >}}) 和 [`__complex__()`]({{< ref "/reference/datamodel#object.__complex__" >}}) 则相应的内置函数 [`int()`]({{< ref "/library/functions#int" >}}), [`float()`]({{< ref "/library/functions#float" >}}) 和 [`complex()`]({{< ref "/library/functions#complex" >}}) 将回退为 [`__index__()`]({{< ref "/reference/datamodel#object.__index__" >}})。
...
​	调用这些方法以实现内置函数 [`round()`]({{< ref "/library/functions#round" >}}) 以及 [`math`]({{< ref "/library/numeric/math#module-math" >}}) 函数 [`trunc()`]({{< ref "/library/numeric/math#math.trunc" >}}), [`floor()`]({{< ref "/library/numeric/math#math.floor" >}}) 和 [`ceil()`]({{< ref "/library/numeric/math#math.ceil" >}})。 除了将 *ndigits* 传给 `__round__()` 的情况之外这些方法的返回值都应当是原对象截断为 [`Integral`]({{< ref "/library/numeric/numbers#numbers.Integral" >}}) (通常为 [`int`]({{< ref "/library/functions#int" >}}))。

​	如果 [`__int__()`]({{< ref "/reference/datamodel#object.__int__" >}}) 或 [`__index__()`]({{< ref "/reference/datamodel#object.__index__" >}}) 均未被定义则内置函数 [`int()`]({{< ref "/library/functions#int" >}}) 会回退至 [`__trunc__()`]({{< ref "/reference/datamodel#object.__trunc__" >}})。

> 在 3.11 版本发生变更: 将 [`int()`]({{< ref "/library/functions#int" >}}) 委托给 [`__trunc__()`]({{< ref "/reference/datamodel#object.__trunc__" >}}) 的做法已被弃用。



### 3.3.9. with 语句上下文管理器

*上下文管理器* 是一个对象，它定义了在执行 [`with`]({{< ref "/reference/compound_stmts#with" >}}) 语句时要建立的运行时上下文。 上下文管理器处理进入和退出所需运行时上下文以执行代码块。 通常使用 `with` 语句（在 [with 语句]({{< ref "/reference/compound_stmts#with" >}}) 中描述），但是也可以通过直接调用它们的方法来使用。

​	上下文管理器的典型用法包括保存和恢复各种全局状态，锁定和解锁资源，关闭打开的文件等等。

​	For more information on context managers, see [上下文管理器类型]({{< ref "/library/stdtypes#typecontextmanager" >}}). The [`object`]({{< ref "/library/functions#object" >}}) class itself does not provide the context manager methods.

#### object.`__enter__`(*self*)

​	进入与此对象相关的运行时上下文。 [`with`]({{< ref "/reference/compound_stmts#with" >}}) 语句将会绑定这个方法的返回值到 `as` 子句中指定的目标，如果有的话。

#### object.`__exit__`(*self*, *exc_type*, *exc_value*, *traceback*)

​	退出关联到此对象的运行时上下文。 各个参数描述了导致上下文退出的异常。 如果上下文是无异常地退出的，三个参数都将为 [`None`]({{< ref "/library/constants#None" >}})。

​	如果提供了异常，并且希望方法屏蔽此异常（即避免其被传播），则应当返回真值。 否则的话，异常将在退出此方法时按正常流程处理。

​	请注意 [`__exit__()`]({{< ref "/reference/datamodel#object.__exit__" >}}) 方法不应该重新引发被传入的异常，这是调用者的责任。
...
​	举例来说，如果 `MyClass.__match_args__` 为 `("left", "center", "right")` 则意味着 `case MyClass(x, y)` 就等价于 `case MyClass(left=x, center=y)`。 请注意模式中参数的数量必须小于等于 *__match_args__* 中元素的数量；如果前者大于后者，则尝试模式匹配时将引发 [`TypeError`]({{< ref "/library/exceptions#TypeError" >}})。
...
[缓冲区协议](https://docs.python.org/zh-cn/3.13/c-api/buffer.html#bufferobjects) 为 Python 对象提供了一种向低层级内存数组暴露高效访问的方式。 该协议是通过内置类型如 [`bytes`]({{< ref "/library/stdtypes#bytes" >}}) 和 [`memoryview`]({{< ref "/library/stdtypes#memoryview" >}}) 实现的，还可能由第三方库定义额外的缓冲区类型。

​	虽然缓冲区类型通常都是用 C 实现的，但用 Python 来实现该协议也是可能的。

#### object.`__buffer__`(*self*, *flags*)

​	当从 *self* 请求一个缓冲区时将被调用（例如，从 [`memoryview`]({{< ref "/library/stdtypes#memoryview" >}}) 构造器）。 *flags* 参数是代表所请求缓冲区的类别的整数，例如这会影响返回的缓冲区是只读还是可写。 [`inspect.BufferFlags`]({{< ref "/library/python/inspect#inspect.BufferFlags" >}}) 提供了解读旗标的便利方式。 此方法必须返回一个 [`memoryview`]({{< ref "/library/stdtypes#memoryview" >}}) 对象。

#### object.**__release_buffer__**(*self*, *buffer*)

​	当一个缓冲区不再需要时将被调用。 *buffer* 参数是在此之前由 [`__buffer__()`]({{< ref "/reference/datamodel#object.__buffer__" >}}) 返回的 [`memoryview`]({{< ref "/library/stdtypes#memoryview" >}}) 对象。 此方法必须释放任何关联到该缓冲区的资源。 此方法应当返回 `None`。 不需要执行任何清理的缓冲区对象不要求实现此方法。
...
>
> [`collections.abc.Buffer`]({{< ref "/library/datatypes/collections_abc#collections.abc.Buffer" >}})
>
> ​	缓冲区类型的 ABC。
>
...
​	此行为背后的原理在于包括类型对象在内的所有对象都会实现的几个特殊方法如 [`__hash__()`]({{< ref "/reference/datamodel#object.__hash__" >}}) 和 [`__repr__()`]({{< ref "/reference/datamodel#object.__repr__" >}})。 如果这些方法的隐式查找使用了传统的查找过程，则当它们在对类型对象自身发起调用时将会失败:
...
​	除了出于正确性考虑而会绕过任何实例属性，隐式特殊方法查找通常还会绕过 [`__getattribute__()`]({{< ref "/reference/datamodel#object.__getattribute__" >}}) 方法，甚至包括对象的元类:
...
​	以这种方式绕过 [`__getattribute__()`]({{< ref "/reference/datamodel#object.__getattribute__" >}}) 机制为解释器内部的速度优化提供了显著的空间，其代价则是牺牲了一些处理特殊方法时的灵活性（特殊方法 *must* 必须设置在类对象自身上以便始终一致地由解释器发起调用）。
...
[awaitable]({{< ref "/glossary/idx#term-awaitable" >}}) 对象主要实现了 [`__await__()`]({{< ref "/reference/datamodel#object.__await__" >}}) 方法。 从 [`async def`]({{< ref "/reference/compound_stmts#async-def" >}}) 函数返回的 [协程对象]({{< ref "/glossary/idx#term-coroutine" >}}) 即为可等待对象。

备注


​	从带有 [`types.coroutine()`]({{< ref "/library/datatypes/types#types.coroutine" >}}) 装饰器的生成器返回的 [generator iterator]({{< ref "/glossary/idx#term-generator-iterator" >}}) 对象也属于可等待对象，但它们并未实现 [`__await__()`]({{< ref "/reference/datamodel#object.__await__" >}})。

#### object.`__await__`(*self*)

​	Must return an [iterator]({{< ref "/glossary/idx#term-iterator" >}}). Should be used to implement [awaitable]({{< ref "/glossary/idx#term-awaitable" >}}) objects. For instance, [`asyncio.Future`]({{< ref "/library/ipc/asyncio/asyncio-future#asyncio.Future" >}}) implements this method to be compatible with the [`await`]({{< ref "/reference/expressions#await" >}}) expression. The [`object`]({{< ref "/library/functions#object" >}}) class itself is not awaitable and does not provide this method.

备注


​	本语言不会对 `__await__` 所返回的迭代器产生的对象的类型或值施加任何限制，因为这是负责管理 [awaitable]({{< ref "/glossary/idx#term-awaitable" >}}) 对象的异步执行框架的具体实现 (如 [`asyncio`]({{< ref "/library/ipc/asyncio#module-asyncio" >}})) 专属特性。
...
[协程对象]({{< ref "/glossary/idx#term-coroutine" >}}) 属于 [awaitable]({{< ref "/glossary/idx#term-awaitable" >}}) 对象。 协程的执行可以通过调用 [`__await__()`]({{< ref "/reference/datamodel#object.__await__" >}}) 并迭代其结果来控制。 当协程结束执行并返回时，迭代器会引发 [`StopIteration`]({{< ref "/library/exceptions#StopIteration" >}})，而该异常的 [`value`]({{< ref "/library/exceptions#StopIteration.value" >}}) 属性将存放返回值。 如果协程引发了异常，它会被迭代器传播出去。 协程不应当直接引发未被处理的 [`StopIteration`]({{< ref "/library/exceptions#StopIteration" >}}) 异常。

​	协程也具有下面列出的方法，它们类似于生成器的对应方法 (参见 [生成器-迭代器的方法]({{< ref "/reference/expressions#generator-methods" >}}))。 但是，与生成器不同，协程并不直接支持迭代。

*在 3.5.2 版本发生变更:* 等待一个协程超过一次将引发 [`RuntimeError`]({{< ref "/library/exceptions#RuntimeError" >}})。

#### coroutine.**send**(*value*)

​	开始或恢复协程的执行。 如果 *value* 为 `None`，这将等价于前往 [`__await__()`]({{< ref "/reference/datamodel#object.__await__" >}}) 所返回的迭代器的下一项。 如果 *value* 不为 `None`，此方法将委托给导致协挂起的迭代器的 [`send()`]({{< ref "/reference/expressions#generator.send" >}}) 方法。 其结果（返回值, [`StopIteration`]({{< ref "/library/exceptions#StopIteration" >}}) 或是其他异常）将与上述对 `__await__()` 返回值进行迭代的结果相同。

#### coroutine.**throw**(*value*)

#### coroutine.**throw**(*type*[, *value*[, *traceback*]])

​	在协程内引发指定的异常。 此方法将委托给导致该协程挂起的迭代器的 [`throw()`]({{< ref "/reference/expressions#generator.throw" >}}) 方法，如果存在此方法的话。 否则，该异常将在挂起点被引发。 其结果（返回值，[`StopIteration`]({{< ref "/library/exceptions#StopIteration" >}}) 或是其他异常）将与上述对 [`__await__()`]({{< ref "/reference/datamodel#object.__await__" >}}) 返回值进行迭代的结果相同。 如果该异常未在协程内被捕获，则将回传给调用方。

> 在 3.12 版本发生变更: 第二个签名 (type[, value[, traceback]]) 已被弃用并可能在未来的 Python 版本中移除。

#### coroutine.**close**()

​	此方法会使得协程清理自身并退出。 如果协程被挂起，此方法会先委托给导致协程挂起的迭代器的 [`close()`]({{< ref "/reference/expressions#generator.close" >}}) 方法，如果存在该方法。 然后它会在挂起点引发 [`GeneratorExit`]({{< ref "/library/exceptions#GeneratorExit" >}})，使得协程立即清理自身。 最后，协程会被标记为已结束执行，即使它根本未被启动。
...
​	异步迭代器可在 [`async for`]({{< ref "/reference/compound_stmts#async-for" >}}) 语句中使用。

​	The [`object`]({{< ref "/library/functions#object" >}}) class itself does not provide these methods.
...
​	必须返回一个 *可等待对象* 输出迭代器的下一结果值。 当迭代结束时应该引发 [`StopAsyncIteration`]({{< ref "/library/exceptions#StopAsyncIteration" >}}) 错误。
...
> 在 3.7 版本发生变更: 在 Python 3.7 之前，[`__aiter__()`]({{< ref "/reference/datamodel#object.__aiter__" >}}) 可以返回一个 *可等待对象* 并将被解析为 [异步迭代器]({{< ref "/glossary/idx#term-asynchronous-iterator" >}})。

​	从 Python 3.7 开始，[`__aiter__()`]({{< ref "/reference/datamodel#object.__aiter__" >}}) 必须返回一个异步迭代器对象。 返回任何其他对象都将导致 [`TypeError`]({{< ref "/library/exceptions#TypeError" >}}) 错误。
...
​	异步上下文管理器可在 [`async with`]({{< ref "/reference/compound_stmts#async-with" >}}) 语句中使用。

​	The [`object`]({{< ref "/library/functions#object" >}}) class itself does not provide these methods.

#### object.`__aenter__`(*self*)

​	在语义上类似于 [`__enter__()`]({{< ref "/reference/datamodel#object.__enter__" >}})，仅有的区别在于它必须返回一个 *可等待对象*。

#### object.`__aexit__`(*self*, *exc_type*, *exc_value*, *traceback*)

​	在语义上类似于 [`__exit__()`]({{< ref "/reference/datamodel#object.__exit__" >}})，仅有的区别在于它必须返回一个 *可等待对象*。
...
备注
[[1]({{< ref "/reference/datamodel#id1" >}})]

​	在某些情况下 *有可能* 基于可控的条件改变一个对象的类型。 但这通常不是个好主意，因为如果处理不当会导致一些非常怪异的行为。

[[2]({{< ref "/reference/datamodel#id12" >}})]

[`__hash__()`]({{< ref "/reference/datamodel#object.__hash__" >}}), [`__iter__()`]({{< ref "/reference/datamodel#object.__iter__" >}}), [`__reversed__()`]({{< ref "/reference/datamodel#object.__reversed__" >}}), [`__contains__()`]({{< ref "/reference/datamodel#object.__contains__" >}}), [`__class_getitem__()`]({{< ref "/reference/datamodel#object.__class_getitem__" >}}) 和 [`__fspath__()`]({{< ref "/library/allos/os#os.PathLike.__fspath__" >}}) 方法对此有特殊处理。 其他方法仍然会引发 [`TypeError`]({{< ref "/library/exceptions#TypeError" >}})，但可能会依赖 `None` 是不可调用对象的行为来做到这一点。

[[3]({{< ref "/reference/datamodel#id16" >}})]

​	这里的“不支持”是指该类无此方法，或方法返回 [`NotImplemented`]({{< ref "/library/constants#NotImplemented" >}}) 。 如果你想强制回退到右操作数的反射方法，请不要设置方法为 `None` — 那会造成显式地 *阻塞* 此种回退的相反效果。

[[4]({{< ref "/reference/datamodel#id17" >}})]

​	对于相同类型的操作数，如果非返回方法 -- 例如 [`__add__()`]({{< ref "/reference/datamodel#object.__add__" >}}) -- 失败则会认为整个运算都不被支持，这就是反射方法不会被调用的原因。
