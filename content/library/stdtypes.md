...
3. 从 [`float`]({{< ref "/library/functions#float" >}}) 转换为 [`int`]({{< ref "/library/functions#int" >}}) 将会执行截断，丢弃掉小数部分。 请参阅 [`math.floor()`]({{< ref "/library/numeric/math#math.floor" >}}) 和 [`math.ceil()`]({{< ref "/library/numeric/math#math.ceil" >}}) 函数了解替代的转换方式。
...
​	返回 `True`。 存在于兼容 [`float.is_integer()`]({{< ref "/library/stdtypes#float.is_integer" >}}) 的鸭子类型。
...
​	可以使用 [`functools.cmp_to_key()`]({{< ref "/library/functional/functools#functools.cmp_to_key" >}}) 将 2.x 风格的 *cmp* 函数转换为 *key* 函数。
...
​	如果 *encoding* 或 *errors* 均未给出，则 `str(object)` 将返回 [`type(object).__str__(object)`]({{< ref "/reference/datamodel#object.__str__" >}})，这是 *object* 的“非正式”而适合显示的字符串表示形式。 对于字符串对象，这就是该字符串本身。 如果 *object* 没有 [`__str__()`]({{< ref "/reference/datamodel#object.__str__" >}}) 方法，则 [`str()`]({{< ref "/library/stdtypes#str" >}}) 将回退为返回 [`repr(object)`]({{< ref "/library/functions#repr" >}})。
...
​	此处介绍的格式化操作具有多种怪异特性，可能导致许多常见错误（例如无法正确显示元组和字典）。 使用较新的 [格式化字符串字面值]({{< ref "/reference/lexical_analysis#f-strings" >}})，[`str.format()`]({{< ref "/library/stdtypes#str.format" >}}) 接口或 [模板字符串]({{< ref "/library/text/string#template-strings" >}}) 有助于避免这样的错误。 这些替代方案中的每一种都更好地权衡并提供了简单、灵活以及可扩展性优势。
...
​	如果 *format* 要求一个单独参数，则 *values* 可以为一个非元组对象。 [[5\]]({{< ref "/library/stdtypes#id16" >}}) 否则的话，*values* 必须或者是一个包含项数与格式字符串中指定的转换符项数相同的元组，或者是一个单独映射对象（例如字典）。
...
| `'r'`                  | ​	字符串（使用 [`repr()`]({{< ref "/library/functions#repr" >}}) 转换任何 Python 对象）。 | ​	(5)  |
| `'s'`                  | ​	字符串（使用 [`str()`]({{< ref "/library/stdtypes#str" >}}) 转换任何 Python 对象）。 | ​	(5)  |
| `'a'`                  | ​	字符串（使用 [`ascii()`]({{< ref "/library/functions#ascii" >}}) 转换任何 Python 对象）。 | ​	(5)  |
| `'%'`                  | ​	不转换参数，在结果中输出一个 `'%'` 字符。    |                      |
...
## 二进制序列类型 --- [`bytes`]({{< ref "/library/stdtypes#bytes" >}}), [`bytearray`]({{< ref "/library/stdtypes#bytearray" >}}), [`memoryview`]({{< ref "/library/stdtypes#memoryview" >}})

​	操作二进制数据的核心内置类型是 [`bytes`]({{< ref "/library/stdtypes#bytes" >}}) 和 [`bytearray`]({{< ref "/library/stdtypes#bytearray" >}})。 它们由 [`memoryview`]({{< ref "/library/stdtypes#memoryview" >}}) 提供支持，该对象使用 [缓冲区协议](https://docs.python.org/zh-cn/3.13/c-api/buffer.html#bufferobjects) 来访问其他二进制对象所在内存，不需要创建对象的副本。

[`array`]({{< ref "/library/datatypes/array#module-array" >}}) 模块支持高效地存储基本数据类型，例如 32 位整数和 IEEE754 双精度浮点值。
...
​	像字符串字面值一样，bytes 字面值也可以使用 `r` 前缀来禁用转义序列处理。 请参阅 [字符串与字节串字面值]({{< ref "/reference/lexical_analysis#strings" >}}) 了解有关各种 bytes 字面值形式的详情，包括所支持的转义序列。

​	虽然 bytes 字面值和表示法是基于 ASCII 文本的，但 bytes 对象的行为实际上更像是不可变的整数序列，序列中的每个值的大小被限制为 `0 <= x < 256` (如果违反此限制将引发 [`ValueError`]({{< ref "/library/exceptions#ValueError" >}}))。 这种限制是有意设计用以强调以下事实，虽然许多二进制格式都包含基于 ASCII 的元素，可以通过某些面向文本的算法进行有用的操作，但情况对于任意二进制数据来说通常却并非如此（盲目地将文本处理算法应用于不兼容 ASCII 的二进制数据格式往往将导致数据损坏）。
...
​	另请参阅 [bytes]({{< ref "/library/functions#func-bytes" >}}) 内置类型。

​	由于两个十六进制数码精确对应一个字节，因此十六进制数是描述二进制数据的常用格式。 相应地，bytes 类型具有从此种格式读取数据的附加类方法：

## *classmethod* **fromhex**(*string*)

​	此 [`bytes`]({{< ref "/library/stdtypes#bytes" >}}) 类方法返回一个解码给定字符串的 bytes 对象。 字符串必须由表示每个字节的两个十六进制数码构成，其中的 ASCII 空白符会被忽略。
...
> 在 3.7 版本发生变更: [`bytes.fromhex()`]({{< ref "/library/stdtypes#bytes.fromhex" >}}) 现在会忽略所有 ASCII 空白符而不只是空格符。
...
> 在 3.8 版本发生变更: [`bytes.hex()`]({{< ref "/library/stdtypes#bytes.hex" >}}) 现在支持可选的 *sep* 和 *bytes_per_sep* 形参以在十六进制输出的字节之间插入分隔符。
...
[`bytearray`]({{< ref "/library/stdtypes#bytearray" >}}) 对象是 [`bytes`]({{< ref "/library/stdtypes#bytes" >}}) 对象的可变对应物。
...
​	由于 bytearray 对象是可变的，该对象除了 [bytes 和 bytearray 操作]({{< ref "/library/stdtypes#bytes-methods" >}}) 中所描述的 bytes 和 bytearray 共有操作之外，还支持 [可变]({{< ref "/library/stdtypes#typesseq-mutable" >}}) 序列操作。

​	另请参见 [bytearray]({{< ref "/library/functions#func-bytearray" >}}) 内置类型。

​	由于两个十六进制数码精确对应一个字节，因此十六进制数是描述二进制数据的常用格式。 相应地，bytearray 类型具有从此种格式读取数据的附加类方法：

## *classmethod* **fromhex**(*string*)

[`bytearray`]({{< ref "/library/stdtypes#bytearray" >}}) 类方法返回一个解码给定字符串的 bytearray 对象。 字符串必须由表示每个字节的两个十六进制数码构成，其中的 ASCII 空白符会被忽略。
...
> 在 3.7 版本发生变更: [`bytearray.fromhex()`]({{< ref "/library/stdtypes#bytearray.fromhex" >}}) 现在会忽略所有 ASCII 空白符而不只是空格符。
...
> 在 3.8 版本发生变更: 与 [`bytes.hex()`]({{< ref "/library/stdtypes#bytes.hex" >}}) 相似， [`bytearray.hex()`]({{< ref "/library/stdtypes#bytearray.hex" >}}) 现在支持可选的 *sep* 和 *bytes_per_sep* 参数以在十六进制输出的字节之间插入分隔符。
...
​	bytes 和 bytearray 对象都支持 [通用]({{< ref "/library/stdtypes#typesseq-common" >}}) 序列操作。 它们不仅能与相同类型的操作数，也能与任何 [bytes-like object]({{< ref "/glossary/idx#term-bytes-like-object" >}}) 进行互操作。 由于这样的灵活性，它们可以在操作中自由地混合而不会导致错误。 但是，操作结果的返回值类型可能取决于操作数的顺序。
...
​	要搜索的子序列可以是任意 [bytes-like object]({{< ref "/glossary/idx#term-bytes-like-object" >}}) 或是 0 至 255 范围内的整数。
...
*prefix* 可以是任意 [bytes-like object]({{< ref "/glossary/idx#term-bytes-like-object" >}})。
...
*suffix* 可以是任意 [bytes-like object]({{< ref "/glossary/idx#term-bytes-like-object" >}})。
...
​	返回解码为 [`str`]({{< ref "/library/stdtypes#str" >}}) 的字节串。

*encoding* 默认为 `'utf-8'` ；请参阅 [标准编码]({{< ref "/library/binary/codecs#standard-encodings" >}}) 了解其他可能的值。

*errors* 控制如何处理编码错误。 如为 `'strict'` (默认值)，则会引发 [`UnicodeError`]({{< ref "/library/exceptions#UnicodeError" >}})。 其他可能的值有 `'ignore'`, `'replace'` 以及通过 [`codecs.register_error()`]({{< ref "/library/binary/codecs#codecs.register_error" >}}) 注册的任何其他名称。 请参阅 [错误处理方案]({{< ref "/library/binary/codecs#error-handlers" >}}) 了解详情。
...
​	将 *encoding* 参数传给 [`str`]({{< ref "/library/stdtypes#str" >}}) 允许直接解码任何 [bytes-like object]({{< ref "/glossary/idx#term-bytes-like-object" >}})，无须创建临时的 `bytes` 或 `bytearray` 对象。
...
​	要搜索的后缀可以是任意 [bytes-like object]({{< ref "/glossary/idx#term-bytes-like-object" >}})。
...
[`find()`]({{< ref "/library/stdtypes#bytes.find" >}}) 方法应该只在你需要知道 *sub* 所在位置时使用。 要检查 *sub* 是否为子串，请使用 [`in`]({{< ref "/reference/expressions#in" >}}) 操作符:
...
​	类似于 [`find()`]({{< ref "/library/stdtypes#bytes.find" >}})，但在找不到子序列时会引发 [`ValueError`]({{< ref "/library/exceptions#ValueError" >}})。
...
​	返回一个由 *iterable* 中的二进制数据序列拼接而成的 bytes 或 bytearray 对象。 如果 *iterable* 中存在任何非 [字节类对象]({{< ref "/glossary/idx#term-bytes-like-object" >}}) 包括存在 [`str`]({{< ref "/library/stdtypes#str" >}}) 对象值则会引发 [`TypeError`]({{< ref "/library/exceptions#TypeError" >}})。 提供该方法的 bytes 或 bytearray 对象的内容将作为元素之间的分隔。

## *static* bytes.**maketrans**(*from*, *to*)

## *static* bytearray.**maketrans**(*from*, *to*)

​	此静态方法返回一个可用于 [`bytes.translate()`]({{< ref "/library/stdtypes#bytes.translate" >}}) 的转换对照表，它将把 *from* 中的每个字符映射为 *to* 中相同位置上的字符；*from* 与 *to* 必须都是 [字节类对象]({{< ref "/glossary/idx#term-bytes-like-object" >}}) 并且具有相同的长度。
...
​	要搜索的分隔符可以是任意 [bytes-like object]({{< ref "/glossary/idx#term-bytes-like-object" >}})。
...
​	要搜索的子序列及其替换序列可以是任意 [bytes-like object]({{< ref "/glossary/idx#term-bytes-like-object" >}})。
...
​	类似于 [`rfind()`]({{< ref "/library/stdtypes#bytes.rfind" >}})，但在子序列 *sub* 未找到时会引发 [`ValueError`]({{< ref "/library/exceptions#ValueError" >}})。
...
​	要搜索的前缀可以是任意 [bytes-like object]({{< ref "/glossary/idx#term-bytes-like-object" >}})。
...
​	你可以使用 [`bytes.maketrans()`]({{< ref "/library/stdtypes#bytes.maketrans" >}}) 方法来创建转换表。
...
​	返回原对象的副本，在长度为 *width* 的序列内居中，使用指定的 *fillbyte* 填充两边的空位（默认使用 ASCII 空格符）。 对于 [`bytes`]({{< ref "/library/stdtypes#bytes" >}}) 对象，如果 *width* 小于等于 `len(s)` 则返回原序列的副本。
...
​	返回原对象的副本，在长度为 *width* 的序列中靠左对齐。 使用指定的 *fillbyte* 填充空位（默认使用 ASCII 空格符）。 对于 [`bytes`]({{< ref "/library/stdtypes#bytes" >}}) 对象，如果 *width* 小于等于 `len(s)` 则返回原序列的副本。
...
​	要移除的二进制序列可以是任意 [bytes-like object]({{< ref "/glossary/idx#term-bytes-like-object" >}}) 。 要删除单个前缀字符串，而不是全部给定集合中的字符，请参见 [`str.removeprefix()`]({{< ref "/library/stdtypes#str.removeprefix" >}}) 方法。 例如:
...
​	返回原对象的副本，在长度为 *width* 的序列中靠右对齐。 使用指定的 *fillbyte* 填充空位（默认使用 ASCII 空格符）。 对于 [`bytes`]({{< ref "/library/stdtypes#bytes" >}}) 对象，如果 *width* 小于等于 `len(s)` 则返回原序列的副本。
...
​	将二进制序列拆分为相同类型的子序列，使用 *sep* 作为分隔符。 如果给出了 *maxsplit*，则最多进行 *maxsplit* 次拆分，从 *最右边* 开始。 如果 *sep* 未指定或为 `None`，任何只包含 ASCII 空白符的子序列都会被作为分隔符。 除了从右边开始拆分，[`rsplit()`]({{< ref "/library/stdtypes#bytearray.rsplit" >}}) 的其他行为都类似于下文所述的 [`split()`]({{< ref "/library/stdtypes#bytearray.split" >}})。
...
​	要移除的二进制序列可以是任意 [bytes-like object]({{< ref "/glossary/idx#term-bytes-like-object" >}}) 。 要删除单个后缀字符串，而不是全部给定集合中的字符，请参见 [`str.removesuffix()`]({{< ref "/library/stdtypes#str.removesuffix" >}}) 方法。 例如:
...
​	如果给出了 *sep*，则连续的分隔符不会被组合在一起而是会被视为分隔空子序列 (例如 `b'1,,2'.split(b',')` 将将返回 `[b'1', b'', b'2']`)。 *sep* 参数可能是由多个序列组成的单个分隔符。 使用指定的分隔符拆分一个空序列将返回 `[b'']` 或 `[bytearray(b'')]`，具体取决于被拆分对象的类型。 *sep* 参数可以是任何 [bytes-like object]({{< ref "/glossary/idx#term-bytes-like-object" >}})。
...
​	要移除的字节值二进制序列可以是任意 [bytes-like object]({{< ref "/glossary/idx#term-bytes-like-object" >}})。
...
​	如果序列为 ASCII 标题大小写形式并且序列非空则返回 `True` ，否则返回 `False` 。 请参阅 [`bytes.title()`]({{< ref "/library/stdtypes#bytes.title" >}}) 了解有关“标题大小写”的详细定义。
...
​	返回由原二进制序列中各行组成的列表，在 ASCII 行边界符的位置拆分。 此方法使用 [universal newlines]({{< ref "/glossary/idx#term-universal-newlines" >}}) 方式来分行。 结果列表中不包含换行符，除非给出了 *keepends* 且为真值。
...
​	不同于 [`split()`]({{< ref "/library/stdtypes#bytes.split" >}})，当给出了分隔符 *sep* 时，对于空字符串此方法将返回一个空列表，而末尾的换行不会令结果中增加额外的行:
...
​	Unlike [`str.swapcase()`]({{< ref "/library/stdtypes#str.swapcase" >}}), it is always the case that `bin.swapcase().swapcase() == bin` for the binary versions. Case conversions are symmetrical in ASCII, even though that is not generally true for arbitrary Unicode code points.
...
​	返回原序列的副本，在左边填充 `b'0'` 数码使序列长度为 *width*。 正负值前缀 (`b'+'`/ `b'-'`) 的处理方式是在正负符号 *之后* 填充而非在之前。 对于 [`bytes`]({{< ref "/library/stdtypes#bytes" >}}) 对象，如果 *width* 小于等于 `len(seq)` 则返回原序列。
...
​	如果 *format* 要求一个单独参数，则 *values* 可以为一个非元组对象。 [[5\]]({{< ref "/library/stdtypes#id16" >}}) 否则的话，*values* 必须或是是一个包含项数与格式字节串对象中指定的转换符项数相同的元组，或者是一个单独的映射对象（例如元组）。
...
| `'G'`                  | ​	浮点格式。 如果指数小于 -4 或不小于精度则使用大写指数格式，否则使用十进制格式。 | ​	(4)  |
| `'c'`                  | ​	单个字节（接受整数或单个字节对象）。         |                      |
| `'b'`                  | ​	字节串（任何遵循 [缓冲区协议](https://docs.python.org/zh-cn/3.13/c-api/buffer.html#bufferobjects) 或是具有 [`__bytes__()`]({{< ref "/reference/datamodel#object.__bytes__" >}}) 的对象）。 | ​	(5)  |
...
[`memoryview`]({{< ref "/library/stdtypes#memoryview" >}}) 对象允许 Python 代码访问一个对象的内部数据，只要该对象支持 [缓冲区协议](https://docs.python.org/zh-cn/3.13/c-api/buffer.html#bufferobjects) 而无需进行拷贝。

## *class* **memoryview**(*object*)

​	创建一个引用 *object* 的 [`memoryview`]({{< ref "/library/stdtypes#memoryview" >}}) 。 *object* 必须支持缓冲区协议。支持缓冲区协议的内置对象有 [`bytes`]({{< ref "/library/stdtypes#bytes" >}}) 和 [`bytearray`]({{< ref "/library/stdtypes#bytearray" >}}) 。

[`memoryview`]({{< ref "/library/stdtypes#memoryview" >}}) 有 **元素** 的概念， **元素** 指由原始 *object* 处理的原子内存单元。对于许多简单的类型，如 [`bytes`]({{< ref "/library/stdtypes#bytes" >}}) 和 [`bytearray`]({{< ref "/library/stdtypes#bytearray" >}}) ，一个元素是一个字节，但其他类型，如 [`array.array`]({{< ref "/library/datatypes/array#array.array" >}}) 可能有更大的元素。

`len(view)` 等于 [`tolist`]({{< ref "/library/stdtypes#memoryview.tolist" >}}) 的长度，即视图的嵌套列表表示形式。 如果 `view.ndim = 1`，它将等于视图中元素的数量。

> 在 3.12 版本发生变更: 如果 `view.ndim == 0`，现在 `len(view)` 将引发 [`TypeError`]({{< ref "/library/exceptions#TypeError" >}}) 而不是返回 1.

[`itemsize`]({{< ref "/library/stdtypes#memoryview.itemsize" >}}) 属性将给出单个元素的字节数。

[`memoryview`]({{< ref "/library/stdtypes#memoryview" >}}) 支持通过切片和索引访问其元素。 一维切片的结果将是一个子视图:
...
​	如果 [`format`]({{< ref "/library/stdtypes#memoryview.format" >}}) 是一个来自于 [`struct`]({{< ref "/library/binary/struct#module-struct" >}}) 模块的原生格式说明符，则也支持使用整数或由整数构成的元组进行索引，并返回具有正确类型的单个 *元素*。 一维内存视图可以使用一个整数或由一个整数构成的元组进行索引。 多维内存视图可以使用由恰好 *ndim* 个整数构成的元素进行索引，*ndim* 即其维度。 零维内存视图可以使用空元组进行索引。
...
​	格式符为 'B', 'b' 或 'c' 的 [hashable]({{< ref "/glossary/idx#term-hashable" >}}) (只读) 类型的一维内存视图也是可哈希对象。 哈希被定义为 `hash(m) == hash(m.tobytes())`:
...
> 在 3.3 版本发生变更: 一维内存视图现在可以被切片。 格式符为 'B', 'b' 或 'c' 的一维内存视图现在是 [hashable]({{< ref "/glossary/idx#term-hashable" >}})。

> 在 3.4 版本发生变更: 内存视图现在会自动注册为 [`collections.abc.Sequence`]({{< ref "/library/datatypes/collections_abc#collections.abc.Sequence" >}})

> 在 3.5 版本发生变更: 内存视图现在可使用整数元组进行索引。

[`memoryview`]({{< ref "/library/stdtypes#memoryview" >}}) 具有以下一些方法：

## `__eq__`(*exporter*)

​	memoryview 与 [**PEP 3118**](https://peps.python.org/pep-3118/) 中的导出器这两者如果形状相同，并且如果当使用 [`struct`]({{< ref "/library/binary/struct#module-struct" >}}) 语法解读操作数的相应格式代码时所有对应值都相同，则它们就是等价的。

​	对于 [`tolist()`]({{< ref "/library/stdtypes#memoryview.tolist" >}}) 当前所支持的 [`struct`]({{< ref "/library/binary/struct#module-struct" >}}) 格式字符串子集，如果 `v.tolist() == w.tolist()` 则 `v` 和 `w` 相等:
...
​	如果两边的格式字符串都不被 [`struct`]({{< ref "/library/binary/struct#module-struct" >}}) 模块所支持，则两对象比较结果总是不相等（即使格式字符串和缓冲区内容相同）:
...
​	将缓冲区中的数据作为字节串返回。 这相当于在内存视图上调用 [`bytes`]({{< ref "/library/stdtypes#bytes" >}}) 构造器。
...
​	对于非连续数组，结果等于平面化表示的列表，其中所有元素都转换为字节串。 [`tobytes()`]({{< ref "/library/stdtypes#memoryview.tobytes" >}}) 支持所有格式字符串，不符合 [`struct`]({{< ref "/library/binary/struct#module-struct" >}}) 模块语法的那些也包括在内。
...
> 在 3.8 版本发生变更: 与 [`bytes.hex()`]({{< ref "/library/stdtypes#bytes.hex" >}}) 相似， [`memoryview.hex()`]({{< ref "/library/stdtypes#memoryview.hex" >}}) 现在支持可选的 *sep* 和 *bytes_per_sep* 参数以在十六进制输出的字节之间插入分隔符。
...
> 在 3.3 版本发生变更: [`tolist()`]({{< ref "/library/stdtypes#memoryview.tolist" >}}) 现在支持 [`struct`]({{< ref "/library/binary/struct#module-struct" >}}) 模块语法中的所有单字符原生格式以及多维表示形式。
...
​	释放由内存视图对象所公开的底层缓冲区。 许多对象在被视图所获取时都会采取特殊动作（例如，[`bytearray`]({{< ref "/library/stdtypes#bytearray" >}}) 将会暂时禁止调整大小）；因此，调用 release() 可以方便地尽早去除这些限制（并释放任何多余的资源）。

​	After this method has been called, any further operation on the view raises a [`ValueError`]({{< ref "/library/exceptions#ValueError" >}}) (except [`release()`]({{< ref "/library/stdtypes#memoryview.release" >}}) itself which can be called multiple times):
...
​	将内存视图转化为新的格式或形状。 *shape* 默认为 `[byte_length//new_itemsize]`，这意味着结果视图将是一维的。 返回值是一个新的内存视图，但缓冲区本身不会被复制。 支持的转化有 1D -> C-[contiguous]({{< ref "/glossary/idx#term-contiguous" >}}) 和 C-contiguous -> 1D。

​	目标格式被限制为 [`struct`]({{< ref "/library/binary/struct#module-struct" >}}) 语法中的单一元素的原生格式。 这些格式中的一种必须为字节格式 ('B', 'b' 或 'c')。 结果的字节长度必须与原始长度相同。 请注意全部字节长度可能取决于具体操作系统。
...
​	一个字符串，包含视图中每个元素的格式（表示为 [`struct`]({{< ref "/library/binary/struct#module-struct" >}}) 模块样式）。 内存视图可以从具有任意格式字符串的导出器创建，但某些方法 (例如 [`tolist()`]({{< ref "/library/stdtypes#memoryview.tolist" >}})) 仅限于原生的单元素格式。
...
​	一个整数元组，通过 [`ndim`]({{< ref "/library/stdtypes#memoryview.ndim" >}}) 的长度值给出内存所代表的 N 维数组的形状。

> 在 3.3 版本发生变更: 当 ndim = 0 时值为空元组而不再为 `None`。

## **strides**

​	一个整数元组，通过 [`ndim`]({{< ref "/library/stdtypes#memoryview.ndim" >}}) 的长度给出以字节表示的大小，以便访问数组中每个维度上的每个元素。
...
​	一个表明内存是否为 C-[contiguous]({{< ref "/glossary/idx#term-contiguous" >}}) 的布尔值。
...
​	一个表明内存是否为 Fortran [contiguous]({{< ref "/glossary/idx#term-contiguous" >}}) 的布尔值。
...
​	一个表明内存是否为 [contiguous]({{< ref "/glossary/idx#term-contiguous" >}}) 的布尔值。
...
## 集合类型 --- [`set`]({{< ref "/library/stdtypes#set" >}}), [`frozenset`]({{< ref "/library/stdtypes#frozenset" >}})

*set* 对象是由具有唯一性的 [hashable]({{< ref "/glossary/idx#term-hashable" >}}) 对象所组成的无序多项集。 常见的用途包括成员检测、从序列中去除重复项以及数学中的集合类计算，例如交集、并集、差集与对称差集等等。 （关于其他容器对象请参看 [`dict`]({{< ref "/library/stdtypes#dict" >}}), [`list`]({{< ref "/library/stdtypes#list" >}}) 与 [`tuple`]({{< ref "/library/stdtypes#tuple" >}}) 等内置类，以及 [`collections`](https://docs.python.org/zh-cn/3.13/library/collections.html#module-collections) 模块。）

​	与其他多项集一样，集合也支持 `x in set`, `len(set)` 和 `for x in set`。 作为一种无序的多项集，集合并不记录元素位置或插入顺序。 相应地，集合不支持索引、切片或其他序列类的操作。

​	目前有两种内置集合类型，[`set`]({{< ref "/library/stdtypes#set" >}}) 和 [`frozenset`]({{< ref "/library/stdtypes#frozenset" >}})。 [`set`]({{< ref "/library/stdtypes#set" >}}) 类型是可变的 --- 其内容可以使用 `add()` 和 `remove()` 这样的方法来改变。 由于是可变类型，它没有哈希值，且不能被用作字典的键或其他集合的元素。 [`frozenset`]({{< ref "/library/stdtypes#frozenset" >}}) 类型是不可变并且为 [hashable]({{< ref "/glossary/idx#term-hashable" >}}) --- 其内容在被创建后不能再改变；因此它可以被用作字典的键或其他集合的元素。

​	除了可以使用 [`set`]({{< ref "/library/stdtypes#set" >}}) 构造器，非空的 set (不是 frozenset) 还可以通过将以逗号分隔的元素列表包含于花括号之内来创建，例如: `{'jack', 'sjoerd'}`。
...
​	返回一个新的 set 或 frozenset 对象，其元素来自于 *iterable*。 集合的元素必须为 [hashable]({{< ref "/glossary/idx#term-hashable" >}})。 要表示由集合对象构成的集合，所有的内层集合必须为 [`frozenset`]({{< ref "/library/stdtypes#frozenset" >}}) 对象。 如果未指定 *iterable*，则将返回一个新的空集合。
...
[`set`]({{< ref "/library/stdtypes#set" >}}) 和 [`frozenset`]({{< ref "/library/stdtypes#frozenset" >}}) 的实例提供以下操作：
...
​	注意， [`union()`]({{< ref "/library/stdtypes#frozenset.union" >}}) 、 [`intersection()`]({{< ref "/library/stdtypes#frozenset.intersection" >}}) 、 [`difference()`]({{< ref "/library/stdtypes#frozenset.difference" >}}) 、 [`symmetric_difference()`]({{< ref "/library/stdtypes#frozenset.symmetric_difference" >}}) 、 [`issubset()`]({{< ref "/library/stdtypes#frozenset.issubset" >}}) 和 [`issuperset()`]({{< ref "/library/stdtypes#frozenset.issuperset" >}}) 方法的非运算符版本可以接受任何可迭代对象作为一个参数。相比之下，基于运算符的对应方法则要求参数为集合对象。这就避开了像 `set('abc') & 'cbs'` 这样容易出错的结构，而换成了可读性更好的 `set('abc').intersection('cbs')`。

[`set`]({{< ref "/library/stdtypes#set" >}}) 和 [`frozenset`]({{< ref "/library/stdtypes#frozenset" >}}) 均支持集合与集合的比较。 两个集合当且仅当每个集合中的每个元素均包含于另一个集合之内（即各为对方的子集）时则相等。 一个集合当且仅当其为另一个集合的真子集（即为后者的子集但两者不相等）时则小于另一个集合。 一个集合当且仅当其为另一个集合的真超集（即为后者的超集但两者不相等）时则大于另一个集合。

[`set`]({{< ref "/library/stdtypes#set" >}}) 的实例与 [`frozenset`]({{< ref "/library/stdtypes#frozenset" >}}) 的实例之间基于它们的成员进行比较。 例如 `set('abc') == frozenset('abc')` 返回 `True`，`set('abc') in set([frozenset('abc')])` 也一样。

​	子集与相等比较并不能推广为完全排序函数。 例如，任意两个非空且不相交的集合不相等且互不为对方的子集，因此以下 *所有* 比较均返回 `False`: `a<b`, `a==b`, or `a>b`。

​	由于集合仅定义了部分排序（子集关系），因此由集合构成的列表 [`list.sort()`]({{< ref "/library/stdtypes#list.sort" >}}) 方法的输出并无定义。

​	集合的元素，与字典的键类似，必须为 [hashable]({{< ref "/glossary/idx#term-hashable" >}})。

​	混合了 [`set`]({{< ref "/library/stdtypes#set" >}}) 实例与 [`frozenset`]({{< ref "/library/stdtypes#frozenset" >}}) 的二进制位运算将返回与第一个操作数相同的类型。例如: `frozenset('ab') | set('bc')` 将返回 [`frozenset`]({{< ref "/library/stdtypes#frozenset" >}}) 的实例。

​	下表列出了可用于 [`set`]({{< ref "/library/stdtypes#set" >}}) 而不能用于不可变的 [`frozenset`]({{< ref "/library/stdtypes#frozenset" >}}) 实例的操作：
...
​	从集合中移除元素 *elem*。 如果 *elem* 不存在于集合中则会引发 [`KeyError`]({{< ref "/library/exceptions#KeyError" >}})。
...
​	从集合中移除并返回任意一个元素。 如果集合为空则会引发 [`KeyError`]({{< ref "/library/exceptions#KeyError" >}})。

## **clear**()

​	从集合中移除所有元素。

​	请注意，非运算符版本的 [`update()`]({{< ref "/library/stdtypes#frozenset.update" >}}), [`intersection_update()`]({{< ref "/library/stdtypes#frozenset.intersection_update" >}}), [`difference_update()`]({{< ref "/library/stdtypes#frozenset.difference_update" >}}) 和 [`symmetric_difference_update()`]({{< ref "/library/stdtypes#frozenset.symmetric_difference_update" >}}) 方法将接受任意可迭代对象作为参数。

​	请注意，[`__contains__()`]({{< ref "/reference/datamodel#object.__contains__" >}}), [`remove()`]({{< ref "/library/stdtypes#frozenset.remove" >}}) 和 [`discard()`]({{< ref "/library/stdtypes#frozenset.discard" >}}) 方法的 *elem* 参数可以是一个集合。 为支持搜索等价的冻结集合，将根据 *elem* 临时创建一个相应的对象。



## 映射类型 --- [`dict`]({{< ref "/library/stdtypes#dict" >}})

[mapping]({{< ref "/glossary/idx#term-mapping" >}}) 对象会将 [hashable]({{< ref "/glossary/idx#term-hashable" >}}) 值映射到任意对象。 映射属于可变对象。 目前仅有一种标准映射类型 *字典*。 （关于其他容器对象请参看 [`list`]({{< ref "/library/stdtypes#list" >}}), [`set`]({{< ref "/library/stdtypes#set" >}}) 与 [`tuple`]({{< ref "/library/stdtypes#tuple" >}}) 等内置类，以及 [`collections`](https://docs.python.org/zh-cn/3.13/library/collections.html#module-collections) 模块。）

​	字典的键 *几乎* 可以为任何值。 不是 [hashable]({{< ref "/glossary/idx#term-hashable" >}}) 的值，即包含列表、字典或其他可变类型（按值比较而非按对象标识比较）的值不可被用作键。 比较结果相等的值（如 `1`, `1.0` 和 `True` 等）可被互换使用以索引同一个字典条目。
...
​	返回 *d* 中以 *key* 为键的项。 如果映射中不存在 *key* 则会引发 [`KeyError`]({{< ref "/library/exceptions#KeyError" >}})。

​	如果字典的子类定义了方法 `__missing__()` 并且 *key* 不存在，则 `d[key]` 操作将调用该方法并附带键 *key* 作为参数。 `d[key]` 随后将返回或引发 `__missing__(key)` 调用所返回或引发的任何对象或异常。 没有其他操作或方法会发起调用 `__missing__()`。 如果未定义 `__missing__()`，则会引发 [`KeyError`]({{< ref "/library/exceptions#KeyError" >}})。 `__missing__()` 必须是一个方法；它不能是一个实例变量:
...
​	将 `d[key]` 从 *d* 中移除。 如果映射中不存在 *key* 则会引发 [`KeyError`]({{< ref "/library/exceptions#KeyError" >}})。
...
[`fromkeys()`]({{< ref "/library/stdtypes#dict.fromkeys" >}}) 是一个返回新字典的类方法。 *value* 默认为 `None`。 所有值都只引用一个单独的实例，因此让 *value* 成为一个可变对象例如空列表通常是没有意义的。 要获取不同的值，请改用 [字典推导式]({{< ref "/reference/expressions#dict" >}})。

## **get**(*key*, *default=None*)

​	如果 *key* 存在于字典中则返回 *key* 的值，否则返回 *default*。 如果 *default* 未给出则默认为 `None`，因而此方法绝不会引发 [`KeyError`]({{< ref "/library/exceptions#KeyError" >}})。

## **items**()

​	返回由字典项 (`(键, 值)` 对) 组成的一个新视图。 参见 [视图对象文档]({{< ref "/library/stdtypes#dict-views" >}})。

## **keys**()

​	返回由字典键组成的一个新视图。 参见 [视图对象文档]({{< ref "/library/stdtypes#dict-views" >}})。

## **pop**(*key*[, *default*])

​	如果 *key* 存在于字典中则将其移除并返回其值，否则返回 *default*。 如果 *default* 未给出且 *key* 不存在于字典中，则会引发 [`KeyError`]({{< ref "/library/exceptions#KeyError" >}})。

## **popitem**()

​	从字典中移除并返回一个 `(键, 值)` 对。 键值对会按 LIFO 的顺序被返回。

[`popitem()`]({{< ref "/library/stdtypes#dict.popitem" >}}) 适用于对字典进行消耗性的迭代，这在集合算法中经常被使用。 如果字典为空，调用 [`popitem()`]({{< ref "/library/stdtypes#dict.popitem" >}}) 将引发 [`KeyError`]({{< ref "/library/exceptions#KeyError" >}})。

> 在 3.7 版本发生变更: 现在会确保采用 LIFO 顺序。 在之前的版本中，[`popitem()`]({{< ref "/library/stdtypes#dict.popitem" >}}) 会返回一个任意的键/值对。
...
[`update()`]({{< ref "/library/stdtypes#dict.update" >}}) accepts either another object with a `keys()` method (in which case [`__getitem__()`]({{< ref "/reference/datamodel#object.__getitem__" >}}) is called with every key returned from the method) or an iterable of key/value pairs (as tuples or other iterables of length two). If keyword arguments are specified, the dictionary is then updated with those key/value pairs: `d.update(red=1, blue=2)`.

## **values**()

​	返回由字典值组成的一个新视图。 参见 [视图对象文档]({{< ref "/library/stdtypes#dict-views" >}})。
...
​	用 *other* 的键和值更新字典 *d* ，*other* 可以是 [mapping]({{< ref "/glossary/idx#term-mapping" >}}) 或 [iterable]({{< ref "/glossary/idx#term-iterable" >}}) 的键值对。当 *d* 和 *other* 有相同键时， *other* 的值优先。

> Added in version 3.9.
>

​	两个字典的比较当且仅当它们具有相同的 `(键, 值)` 对时才会相等（不考虑顺序）。 排序比较 ('<', '<=', '>=', '>') 会引发 [`TypeError`]({{< ref "/library/exceptions#TypeError" >}})。
...
[`types.MappingProxyType`]({{< ref "/library/datatypes/types#types.MappingProxyType" >}}) 可被用来创建一个 [`dict`]({{< ref "/library/stdtypes#dict" >}}) 的只读视图。



### 字典视图对象

​	由 [`dict.keys()`]({{< ref "/library/stdtypes#dict.keys" >}}), [`dict.values()`]({{< ref "/library/stdtypes#dict.values" >}}) 和 [`dict.items()`]({{< ref "/library/stdtypes#dict.items" >}}) 所返回的对象是 *视图对象*。 该对象提供字典条目的一个动态视图，这意味着当字典改变时，视图也会相应改变。
...
​	键和值是按插入时的顺序进行迭代的。 这样就允许使用 [`zip()`]({{< ref "/library/functions#zip" >}}) 来创建 `(值, 键)` 对: `pairs = zip(d.values(), d.keys())`。 另一个创建相同列表的方式是 `pairs = [(v, k) for (k, v) in d.items()]`.

​	在添加或删除字典中的条目期间对视图进行迭代可能引发 [`RuntimeError`]({{< ref "/library/exceptions#RuntimeError" >}}) 或者无法完全迭代所有条目。
...
​	返回 [`types.MappingProxyType`]({{< ref "/library/datatypes/types#types.MappingProxyType" >}}) 对象，封装了字典视图指向的原始字典。

> Added in version 3.10.
>

​	键视图与集合类似因为其条目是唯一的并且为 [hashable]({{< ref "/glossary/idx#term-hashable" >}})。 条视图也有类似集合的操作因为 (键, 值) 对是唯一的并且键是可哈希的。 如果条目视图中的所有值也都是可哈希的，那么条目视图就可以与其他集合执行互操作。 （值视图不会被认为与集合类似因为条目通常不是唯一的）。 对于与集合类似的视图，可以使用为抽象基类 [`collections.abc.Set`]({{< ref "/library/datatypes/collections_abc#collections.abc.Set" >}}) 定义的所有操作（例如，`==`, `<` 或 `^` 等）。 虽然使用了集合运算符，但与集合类似的视图接受任何可迭代对象作为其操作数，而不像集合那样只接受集合作为输入。
...
​	Python 的 [`with`]({{< ref "/reference/compound_stmts#with" >}}) 语句支持通过上下文管理器所定义的运行时上下文这一概念。 此对象的实现使用了一对专门方法，允许用户自定义类来定义运行时上下文，在语句体被执行前进入该上下文，并在语句执行完毕时退出该上下文：

## contextmanager.`__enter__`()

​	进入运行时上下文并返回此对象或关联到该运行时上下文的其他对象。 此方法的返回值会绑定到使用此上下文管理器的 [`with`]({{< ref "/reference/compound_stmts#with" >}}) 语句的 `as` 子句中的标识符。

​	一个返回其自身的上下文管理器的例子是 [file object]({{< ref "/glossary/idx#term-file-object" >}})。 文件对象会从 __enter__() 返回其自身，以允许 [`open()`]({{< ref "/library/functions#open" >}}) 被用作 [`with`]({{< ref "/reference/compound_stmts#with" >}}) 语句中的上下文表达式。

​	一个返回关联对象的上下文管理器的例子是 [`decimal.localcontext()`]({{< ref "/library/numeric/decimal#decimal.localcontext" >}}) 所返回的对象。 此种管理器会将活动的 decimal 上下文设为原始 decimal 上下文的一个副本并返回该副本。 这允许对 [`with`]({{< ref "/reference/compound_stmts#with" >}}) 语句的语句体中的当前 decimal 上下文进行更改，而不会影响 `with` 语句以外的代码。

## contextmanager.`__exit__`(*exc_type*, *exc_val*, *exc_tb*)

​	退出运行时上下文并返回一个布尔值旗标来表明所发生的任何异常是否应当被屏蔽。 如果在执行 [`with`]({{< ref "/reference/compound_stmts#with" >}}) 语句的语句体期间发生了异常，则参数会包含异常的类型、值以及回溯信息。 在其他情况下三个参数均为 `None`。

​	自此方法返回一个真值将导致 [`with`]({{< ref "/reference/compound_stmts#with" >}}) 语句屏蔽异常并继续执行紧随在 `with` 语句之后的语句。 否则异常将在此方法结束执行后继续传播。 在此方法执行期间发生的异常将会取代 `with` 语句的语句体中发生的任何异常。

​	传入的异常绝对不应当被显式地重新引发 —— 相反地，此方法应当返回一个假值以表明方法已成功完成并且不希望屏蔽被引发的异常。 这允许上下文管理代码方便地检测 [`__exit__()`]({{< ref "/reference/datamodel#object.__exit__" >}}) 方法是否确实已失败。

​	Python 定义了一些上下文管理器来支持简易的线程同步、文件或其他对象的快速关闭，以及更方便地操作活动的十进制算术上下文。 除了实现上下文管理协议以外，不同类型不会被特殊处理。 请参阅 [`contextlib`]({{< ref "/library/python/contextlib#module-contextlib" >}}) 模块查看相关的示例。

​	Python 的 [generator]({{< ref "/glossary/idx#term-generator" >}}) 和 [`contextlib.contextmanager`]({{< ref "/library/python/contextlib#contextlib.contextmanager" >}}) 装饰器提供了实现这些协议的便捷方式。 如果使用 [`contextlib.contextmanager`]({{< ref "/library/python/contextlib#contextlib.contextmanager" >}}) 装饰器来装饰一个生成器函数，它将返回一个实现了必要的 [`__enter__()`]({{< ref "/library/stdtypes#contextmanager.__enter__" >}}) 和 [`__exit__()`]({{< ref "/library/stdtypes#contextmanager.__exit__" >}}) 方法的上下文管理器，而不再是由未经装饰的生成器所产生的迭代器。

​	请注意，Python/C API 中 Python 对象的类型结构中并没有针对这些方法的专门槽位。 想要定义这些方法的扩展类型必须将它们作为普通的 Python 可访问方法来提供。 与设置运行时上下文的开销相比，单个类字典查找的开销可以忽略不计。

## 类型注解的类型 --- [Generic Alias]({{< ref "/library/stdtypes#types-genericalias" >}}) 、 [Union]({{< ref "/library/stdtypes#types-union" >}})

[type annotations]({{< ref "/glossary/idx#term-annotation" >}}) 的内置类型为 [Generic Alias]({{< ref "/library/stdtypes#types-genericalias" >}}) 和 [Union]({{< ref "/library/stdtypes#types-union" >}})。



### GenericAlias 类型

`GenericAlias` 对象通常是通过 [抽取]({{< ref "/reference/expressions#subscriptions" >}}) 一个类来创建的。 它们最常被用于 [容器类]({{< ref "/reference/datamodel#sequence-types" >}})，如 [`list`]({{< ref "/library/stdtypes#list" >}}) 或 [`dict`]({{< ref "/library/stdtypes#dict" >}})。 举例来说，`list[int]` 这个 `GenericAlias` 对象是通过附带 [`int`]({{< ref "/library/functions#int" >}}) 参数抽取 `list` 类来创建的。 `GenericAlias` 对象的主要目的是用于 [类型标注]({{< ref "/glossary/idx#term-annotation" >}})。

​备注
 

​	通常一个类只有在实现了特殊方法 [`__class_getitem__()`]({{< ref "/reference/datamodel#object.__class_getitem__" >}}) 时才支持抽取操作。

`GenericAlias` 对象可作为 [generic type]({{< ref "/glossary/idx#term-generic-type" >}}) 的代理，实现了 *形参化泛型*。

​	对于一个容器类，提供给类的 [抽取]({{< ref "/reference/expressions#subscriptions" >}}) 操作的参数可以指明对象所包含的元素类型。 例如，`set[bytes]` 可在类型标注中用来表示一个 [`set`]({{< ref "/library/stdtypes#set" >}}) 中的所有元素均为 [`bytes`]({{< ref "/library/stdtypes#bytes" >}}) 类型。

​	对于一个定义了 [`__class_getitem__()`]({{< ref "/reference/datamodel#object.__class_getitem__" >}}) 但不属于容器的类，提供给类的抽取操作的参数往往会指明在对象上定义的一个或多个方法的返回值类型。 例如，[`正则表达式`]({{< ref "/library/text/re#module-re" >}}) 可以被用在 [`str`]({{< ref "/library/stdtypes#str" >}}) 数据类型和 [`bytes`]({{< ref "/library/stdtypes#bytes" >}}) 数据类型上:

- 如果 `x = re.search('foo', 'foo')`，则 `x` 将为一个 [re.Match]({{< ref "/library/text/re#match-objects" >}}) 对象而 `x.group(0)` 和 `x[0]` 的返回值将均为 [`str`]({{< ref "/library/stdtypes#str" >}}) 类型。 我们可以在类型标注中使用 `GenericAlias` `re.Match[str]` 来代表这种对象。
- 如果 `y = re.search(b'bar', b'bar')`，(注意 `b` 表示 [`bytes`]({{< ref "/library/stdtypes#bytes" >}}))，则 `y` 也将为一个 `re.Match` 的实例，但 `y.group(0)` 和 `y[0]` 的返回值将均为 [`bytes`]({{< ref "/library/stdtypes#bytes" >}}) 类型。 在类型标注中，我们将使用 `re.Match[bytes]` 来代表这种形式的 [re.Match]({{< ref "/library/text/re#match-objects" >}}) 对象。

`GenericAlias` 对象是 [`types.GenericAlias`]({{< ref "/library/datatypes/types#types.GenericAlias" >}}) 类的实例，该类也可被用来直接创建 `GenericAlias` 对象。

## **T[X, Y, ...]**

​	创建一个代表由类型 *X*, *Y* 来参数化的类型 `T` 的 `GenericAlias`，此类型会更依赖于所使用的 `T`。 例如，一个接受包含 [`float`]({{< ref "/library/functions#float" >}}) 元素的 [`list`]({{< ref "/library/stdtypes#list" >}}) 的函数:
...
​	另一个例子是关于 [mapping]({{< ref "/glossary/idx#term-mapping" >}}) 对象的，用到了 [`dict`]({{< ref "/library/stdtypes#dict" >}})，泛型的两个类型参数分别代表了键类型和值类型。本例中的函数需要一个 `dict`，其键的类型为 [`str`]({{< ref "/library/stdtypes#str" >}})，值的类型为 [`int`]({{< ref "/library/functions#int" >}}):。
...
​	内置函数 [`isinstance()`]({{< ref "/library/functions#isinstance" >}}) 和 [`issubclass()`]({{< ref "/library/functions#issubclass" >}}) 不接受第二个参数为 `GenericAlias` 类型：
...
​	Python 运行时不会强制执行 [类型标注]({{< ref "/glossary/idx#term-annotation" >}})。 这种行为扩展到了泛型及其类型形参。 当由 `GenericAlias` 创建容器对象时，并不会检查容器中为元素指定的类型。 例如，以下代码虽然不被鼓励，但运行时并不会报错:
...
​	在泛型上调用 [`repr()`]({{< ref "/library/functions#repr" >}}) 或 [`str()`]({{< ref "/library/stdtypes#str" >}}) 会显示应用参数之后的类型：
...
​	调用泛型容器的 [`__getitem__()`]({{< ref "/reference/datamodel#object.__getitem__" >}}) 方法将引发异常以防出现 `dict[str][str]` 之类的错误:
...
​	不过，当使用了 [类型变量]({{< ref "/library/development/typing#generics" >}}) 时这种表达式是无效的。 索引必须有与 `GenericAlias` 对象的 [`__args__`]({{< ref "/library/stdtypes#genericalias.__args__" >}}) 中的类型变量条目数量相当的元素。
...
- [`type`]({{< ref "/library/functions#type" >}})
...
- [`collections.abc.Awaitable`]({{< ref "/library/datatypes/collections_abc#collections.abc.Awaitable" >}})
- [`collections.abc.Coroutine`]({{< ref "/library/datatypes/collections_abc#collections.abc.Coroutine" >}})
- [`collections.abc.AsyncIterable`]({{< ref "/library/datatypes/collections_abc#collections.abc.AsyncIterable" >}})
- [`collections.abc.AsyncIterable`]({{< ref "/library/datatypes/collections_abc#collections.abc.AsyncIterable" >}})
- [`collections.abc.AsyncGenerator`]({{< ref "/library/datatypes/collections_abc#collections.abc.AsyncGenerator" >}})
- [`collections.abc.Iterable`]({{< ref "/library/datatypes/collections_abc#collections.abc.Iterable" >}})
- [`collections.abc.Iterator`]({{< ref "/library/datatypes/collections_abc#collections.abc.Iterator" >}})
- [`collections.abc.Generator`]({{< ref "/library/datatypes/collections_abc#collections.abc.Generator" >}})
- [`collections.abc.Reversible`]({{< ref "/library/datatypes/collections_abc#collections.abc.Reversible" >}})
- [`collections.abc.Container`]({{< ref "/library/datatypes/collections_abc#collections.abc.Container" >}})
- [`collections.abc.Collection`]({{< ref "/library/datatypes/collections_abc#collections.abc.Collection" >}})
- [`collections.abc.Callable`]({{< ref "/library/datatypes/collections_abc#collections.abc.Callable" >}})
- [`collections.abc.Set`]({{< ref "/library/datatypes/collections_abc#collections.abc.Set" >}})
- [`collections.abc.MutableSet`]({{< ref "/library/datatypes/collections_abc#collections.abc.MutableSet" >}})
- [`collections.abc.Mapping`]({{< ref "/library/datatypes/collections_abc#collections.abc.Mapping" >}})
- [`collections.abc.MutableMapping`]({{< ref "/library/datatypes/collections_abc#collections.abc.MutableMapping" >}})
- [`collections.abc.Sequence`]({{< ref "/library/datatypes/collections_abc#collections.abc.Sequence" >}})
- [`collections.abc.MutableSequence`]({{< ref "/library/datatypes/collections_abc#collections.abc.MutableSequence" >}})
- [`collections.abc.ByteString`]({{< ref "/library/datatypes/collections_abc#collections.abc.ByteString" >}})
- [`collections.abc.MappingView`]({{< ref "/library/datatypes/collections_abc#collections.abc.MappingView" >}})
- [`collections.abc.KeysView`]({{< ref "/library/datatypes/collections_abc#collections.abc.KeysView" >}})
- [`collections.abc.ItemsView`]({{< ref "/library/datatypes/collections_abc#collections.abc.ItemsView" >}})
- [`collections.abc.ValuesView`]({{< ref "/library/datatypes/collections_abc#collections.abc.ValuesView" >}})
- [`contextlib.AbstractContextManager`]({{< ref "/library/python/contextlib#contextlib.AbstractContextManager" >}})
- [`contextlib.AbstractAsyncContextManager`]({{< ref "/library/python/contextlib#contextlib.AbstractAsyncContextManager" >}})
- [`dataclasses.Field`]({{< ref "/library/python/dataclasses#dataclasses.Field" >}})
- [`functools.cached_property`]({{< ref "/library/functional/functools#functools.cached_property" >}})
- [`functools.partialmethod`]({{< ref "/library/functional/functools#functools.partialmethod" >}})
- [`os.PathLike`]({{< ref "/library/allos/os#os.PathLike" >}})
- [`queue.LifoQueue`]({{< ref "/library/concurrency/queue#queue.LifoQueue" >}})
- [`queue.Queue`]({{< ref "/library/concurrency/queue#queue.Queue" >}})
- [`queue.PriorityQueue`]({{< ref "/library/concurrency/queue#queue.PriorityQueue" >}})
- [`queue.SimpleQueue`]({{< ref "/library/concurrency/queue#queue.SimpleQueue" >}})
- [re.Pattern]({{< ref "/library/text/re#re-objects" >}})
- [re.Match]({{< ref "/library/text/re#match-objects" >}})
- [`shelve.BsdDbShelf`]({{< ref "/library/persistence/shelve#shelve.BsdDbShelf" >}})
- [`shelve.DbfilenameShelf`]({{< ref "/library/persistence/shelve#shelve.DbfilenameShelf" >}})
- [`shelve.Shelf`]({{< ref "/library/persistence/shelve#shelve.Shelf" >}})
- [`types.MappingProxyType`]({{< ref "/library/datatypes/types#types.MappingProxyType" >}})
- [`weakref.WeakKeyDictionary`]({{< ref "/library/datatypes/weakref#weakref.WeakKeyDictionary" >}})
- [`weakref.WeakMethod`]({{< ref "/library/datatypes/weakref#weakref.WeakMethod" >}})
- [`weakref.WeakSet`]({{< ref "/library/datatypes/weakref#weakref.WeakSet" >}})
- [`weakref.WeakValueDictionary`]({{< ref "/library/datatypes/weakref#weakref.WeakValueDictionary" >}})
...
​	该属性是传给泛型类的原始 [`__class_getitem__()`]({{< ref "/reference/datamodel#object.__class_getitem__" >}}) 的泛型所组成的 [`tuple`]({{< ref "/library/stdtypes#tuple" >}}) (长度可能为 1):
...
​	带有参数 [`typing.ParamSpec`]({{< ref "/library/development/typing#typing.ParamSpec" >}}) 的 `GenericAlias` 对象，在类型替换后其 `__parameters__` 可能会不准确，因为 [`typing.ParamSpec`]({{< ref "/library/development/typing#typing.ParamSpec" >}}) 主要用于静态类型检查。

## genericalias.`__unpacked__`

​	一个布尔值，如果别名已使用 `*` 运算符进行解包则为真值 (参见 [`TypeVarTuple`]({{< ref "/library/development/typing#typing.TypeVarTuple" >}}))。
...
​	介绍了对标准库类进行原生形参化的能力，只要它们实现了特殊的类方法 [`__class_getitem__()`]({{< ref "/reference/datamodel#object.__class_getitem__" >}})。

## [泛型（Generic）]({{< ref "/library/development/typing#generics" >}}), [用户自定义泛型]({{< ref "/library/development/typing#user-defined-generics" >}}) 和 [`typing.Generic`]({{< ref "/library/development/typing#typing.Generic" >}})
...
​	联合对象包含了在多个 [类型对象]({{< ref "/library/stdtypes#bltin-type-objects" >}}) 上执行 `|` (按位或) 运算后的值。 这些类型主要用于 [类型标注]({{< ref "/glossary/idx#term-annotation" >}})。与 [`typing.Union`]({{< ref "/library/development/typing#typing.Union" >}}) 相比，联合类型表达式可以实现更简洁的类型提示语法。

## **X | Y | ...**

​	定义包含了 *X*、*Y* 等类型的 union 对象。 `X | Y` 表示 X 或 Y。相当于 `typing.Union[X, Y]` 。比如以下函数的参数应为类型 [`int`]({{< ref "/library/functions#int" >}}) 或 [`float`]({{< ref "/library/functions#float" >}}) ：
...
[`isinstance()`]({{< ref "/library/functions#isinstance" >}}) 和 [`issubclass()`]({{< ref "/library/functions#issubclass" >}}) 也支持 union 对象：
...
​	但是联合对象中的 [参数化泛型]({{< ref "/library/stdtypes#types-genericalias" >}}) 将无法被检测:
...
​	union 对象构成的用户类型可以经由 [`types.UnionType`]({{< ref "/library/datatypes/types#types.UnionType" >}}) 访问，并可用于 [`isinstance()`]({{< ref "/library/functions#isinstance" >}}) 检查。 而不能由类型直接实例化为对象：
...
​	模块唯一的特殊操作是属性访问: `m.name`，这里 *m* 为一个模块而 *name* 访问定义在 *m* 的符号表中的一个名称。 模块属性可以被赋值。 （请注意 [`import`]({{< ref "/reference/simple_stmts#import" >}}) 语句严格来说也是对模块对象的一种操作；`import foo` 不要求存在一个名为 *foo* 的模块对象，而是要求存在一个对于名为 *foo* 的模块的 (永久性) *定义*。）

​	每个模块都有一个特殊属性 [`__dict__`]({{< ref "/reference/datamodel#object.__dict__" >}})。 这是包含模块的符号表的字典。 修改此字典将实际改变模块的符号表，但是无法直接对 [`__dict__`]({{< ref "/reference/datamodel#object.__dict__" >}}) 赋值 (你可以写 `m.__dict__['a'] = 1`，这会将 `m.a` 定义为 `1`，但是你不能写 `m.__dict__ = {}`)。 不建议直接修改 [`__dict__`]({{< ref "/reference/datamodel#object.__dict__" >}})。
...
​	关于这些类型请参阅 [对象、值与类型]({{< ref "/reference/datamodel#objects" >}}) 和 [类定义]({{< ref "/reference/compound_stmts#class" >}})。
...
​	更多信息请参阅 [函数定义]({{< ref "/reference/compound_stmts#function" >}})。



### 方法

​	方法是使用属性表示法来调用的函数。 存在两种形式: [内置方法]({{< ref "/reference/datamodel#builtin-methods" >}}) (如列表的 `append()`) 和 [类实例方法]({{< ref "/reference/datamodel#instance-methods" >}})。 内置方法由支持它们的类型来描述。

​	如果你通过一个实例来访问方法（即定义在类命名空间内的函数），你会得到一个特殊对象: *绑定方法* (或称 [实例方法]({{< ref "/reference/datamodel#instance-methods" >}})) 对象。 当被调用时，它会将 `self` 参数添加到参数列表。 绑定方法具有两个特殊的只读属性: [`m.__self__`]({{< ref "/reference/datamodel#method.__self__" >}}) 操作该方法的对象，而 [`m.__func__`]({{< ref "/reference/datamodel#method.__func__" >}}) 是实现该方法的函数。 调用 `m(arg-1, arg-2, ..., arg-n)` 完全等价于调用 `m.__func__(m.__self__, arg-1, arg-2, ..., arg-n)`。

​	与 [函数对象]({{< ref "/reference/datamodel#user-defined-funcs" >}}) 类似，绑定方法对象也支持获取任意属性。 但是，由于方法属性实际上保存于下层的函数对象中 ([`method.__func__`]({{< ref "/reference/datamodel#method.__func__" >}}))，因此不允许设置绑定方法的方法属性。 尝试设置方法的属性将会导致引发 [`AttributeError`]({{< ref "/library/exceptions#AttributeError" >}})。 想要设置方法属性，你必须在下层的函数对象中显式地设置它。
...
​	请参阅 [实例方法]({{< ref "/reference/datamodel#instance-methods" >}}) 了解更多信息。



### 代码对象

​	代码对象被具体实现用来表示“伪编译”的可执行 Python 代码例如一个函数体。 它们不同于函数对象，因为它们不包含对其全局执行环境的引用。 代码对象由内置的 [`compile()`]({{< ref "/library/functions#compile" >}}) 函数返回，并可通过函数对象的 [`__code__`]({{< ref "/reference/datamodel#function.__code__" >}}) 属性来提取。 另请参阅 [`code`]({{< ref "/library/custominterp/code#module-code" >}}) 模块。

​	访问 [`__code__`]({{< ref "/reference/datamodel#function.__code__" >}}) 会引发一个 [审计事件]({{< ref "/library/python/sys#auditing" >}}) `object.__getattr__`，并附带参数 `obj` 和 `"__code__"`。

​	可以通过将代码对象（而非源码字符串）传给 [`exec()`]({{< ref "/library/functions#exec" >}}) 或 [`eval()`]({{< ref "/library/functions#eval" >}}) 内置函数来执行或求值。

​	更多信息请参阅 [标准类型层级结构]({{< ref "/reference/datamodel#types" >}})。



### 类型对象

​	类型对象表示各种对象类型。 对象的类型可通过内置函数 [`type()`]({{< ref "/library/functions#type" >}}) 来获取。 类型没有特殊的操作。 标准库模块 [`types`]({{< ref "/library/datatypes/types#module-types" >}}) 定义了所有标准内置类型的名称。
...
​	此对象常被用于切片 (参见 [切片]({{< ref "/reference/expressions#slicings" >}}))。 它不支持任何特殊的操作。 省略符对象只有一种值 [`Ellipsis`]({{< ref "/library/constants#Ellipsis" >}}) (这是个内置名称)。 `type(Ellipsis)()` 会生成 [`Ellipsis`]({{< ref "/library/constants#Ellipsis" >}}) 单例。
...
​	此对象会被作为比较和二元运算被应用于它们所不支持的类型时的返回值。 请参阅 [比较运算]({{< ref "/reference/expressions#comparisons" >}}) 了解更多信息。 未实现对象只有一种值 [`NotImplemented`]({{< ref "/library/constants#NotImplemented" >}})。 `type(NotImplemented)()` 会生成这个单例。
...
​	相关信息请参阅 [标准类型层级结构]({{< ref "/reference/datamodel#types" >}})。 其中描述了 [栈帧对象]({{< ref "/reference/datamodel#frame-objects" >}}), [回溯对象]({{< ref "/reference/datamodel#traceback-objects" >}}) 以及切片对象等。



## 特殊属性

​	语言实现为部分对象类型添加了一些特殊的只读属性，它们具有各自的作用。 其中一些并不会被 [`dir()`]({{< ref "/library/functions#dir" >}}) 内置函数所列出。
...
​	类、函数、方法、描述器或生成器实例的 [qualified name]({{< ref "/glossary/idx#term-qualified-name" >}})。
...
​	The [type parameters]({{< ref "/reference/compound_stmts#type-params" >}}) of generic classes, functions, and [type aliases]({{< ref "/library/development/typing#type-aliases" >}}). For classes and functions that are not generic, this will be an empty tuple.
...
​	CPython 对于 [`int`]({{< ref "/library/functions#int" >}}) 和 [`str`]({{< ref "/library/stdtypes#str" >}}) 之间的转换有一个全局限制以缓解拒绝服务攻击。 此限制 *仅会* 作用于十进制或其他以非二的乘方为基数的数字。 十六进制、八进制和二进制转换不受限制。 该限制可以被配置。

[`int`]({{< ref "/library/functions#int" >}}) 类型在 CPython 中是存储为二进制形式的任意长度的数字（通常称为“大数字”）。 不存在可在线性时间内将一个字符串转换为二进制整数或将一个二进制整数转换为字符串的算法，*除非* 基数为 2 的乘方。 对于基数为 10 来说已知最好的算法也有亚二次方复杂度。 转换一个大数值如 `int('1' * 500_000)` 在快速的 CPU 上也会花费一秒以上的时间。
...
​	当一个操作会超出限制时，将引发 [`ValueError`]({{< ref "/library/exceptions#ValueError" >}}):
...
​	默认限制为 4300 位即 [`sys.int_info.default_max_str_digits`]({{< ref "/library/python/sys#sys.int_info" >}}) 的值。 最低限制可被配置为 640 位即 [`sys.int_info.str_digits_check_threshold`]({{< ref "/library/python/sys#sys.int_info" >}})。
...
​	此限制仅会作用于 [`int`]({{< ref "/library/functions#int" >}}) 和 [`str`]({{< ref "/library/stdtypes#str" >}}) 和 [`bytes`]({{< ref "/library/stdtypes#bytes" >}}) 之间存在速度变慢可能的转换:
...
- [`int.from_bytes()`]({{< ref "/library/stdtypes#int.from_bytes" >}}) 和 [`int.to_bytes()`]({{< ref "/library/stdtypes#int.to_bytes" >}})。
- [`hex()`]({{< ref "/library/functions#hex" >}}), [`oct()`]({{< ref "/library/functions#oct" >}}), [`bin()`]({{< ref "/library/functions#bin" >}})。
- [格式规格迷你语言]({{< ref "/library/text/string#formatspec" >}}) 用于十六进制、八进制和二进制数。
- [`str`]({{< ref "/library/stdtypes#str" >}}) 至 [`float`]({{< ref "/library/functions#float" >}})。
- [`str`]({{< ref "/library/stdtypes#str" >}}) 至 [`decimal.Decimal`]({{< ref "/library/numeric/decimal#decimal.Decimal" >}})。

### 配置限制值

​	在 Python 启动之前你可以使用环境变量或解释器命令行旗标来配置限制值:

- [`PYTHONINTMAXSTRDIGITS`]({{< ref "/using/cmdline#envvar-PYTHONINTMAXSTRDIGITS" >}})，例如 `PYTHONINTMAXSTRDIGITS=640 python3` 是将限制设为 640 而 `PYTHONINTMAXSTRDIGITS=0 python3` 是禁用此限制。
- [`-X int_max_str_digits`]({{< ref "/using/cmdline#cmdoption-X" >}})，例如 `python3 -X int_max_str_digits=640`
- [`sys.flags.int_max_str_digits`]({{< ref "/library/python/sys#sys.flags.int_max_str_digits" >}}) 包含 [`PYTHONINTMAXSTRDIGITS`]({{< ref "/using/cmdline#envvar-PYTHONINTMAXSTRDIGITS" >}}) 或 [`-X int_max_str_digits`]({{< ref "/using/cmdline#cmdoption-X" >}}) 的值。 如果环境变量和 `-X` 选项均有设置，则 `-X` 选项优先。 值为 *-1* 表示两者均未设置，因此会在初始化时使用 [`sys.int_info.default_max_str_digits`]({{< ref "/library/python/sys#sys.int_info.default_max_str_digits" >}}) 的值。

​	从代码中，你可以检查当前的限制并使用这些 [`sys`]({{< ref "/library/python/sys#module-sys" >}}) API 来设置新值:

- [`sys.get_int_max_str_digits()`]({{< ref "/library/python/sys#sys.get_int_max_str_digits" >}}) 和 [`sys.set_int_max_str_digits()`]({{< ref "/library/python/sys#sys.set_int_max_str_digits" >}}) 是解释器级限制的读取器和设置器。 子解释器具有它们自己的限制。

​	有关默认值和最小值的信息可在 [`sys.int_info`]({{< ref "/library/python/sys#sys.int_info" >}}) 中找到:

- [`sys.int_info.default_max_str_digits`]({{< ref "/library/python/sys#sys.int_info" >}}) 是已编译的默认限制。
- [`sys.int_info.str_digits_check_threshold`]({{< ref "/library/python/sys#sys.int_info" >}}) 是该限制可接受的最低值（禁用该限制的 0 除外）。
...
​	默认的 [`sys.int_info.default_max_str_digits`]({{< ref "/library/python/sys#sys.int_info.default_max_str_digits" >}}) 被预期对于大多数应用程序来说都是合理的。 如果你的应用程序需要不同的限制值，请使用不预设 Python 版本的代码从你的主入口点进行设置，因为这些 API 是在 3.12 之前的版本所发布的安全补丁中添加的。
...
​备注
[[1]({{< ref "/library/stdtypes#id1" >}})]

​	有关这些特殊方法的额外信息可参看 Python 参考指南 ([基本定制]({{< ref "/reference/datamodel#customization" >}}))。

[[2]({{< ref "/library/stdtypes#id2" >}})]

​	作为结果，列表 `[1, 2]` 与 `[1.0, 2.0]` 是相等的，元组的情况也类似。

[[3]({{< ref "/library/stdtypes#id4" >}})]

​	必须如此，因为解析器无法判断操作数的类型。

[4]([1](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#id6),[2]({{< ref "/library/stdtypes#id7" >}}),[3]({{< ref "/library/stdtypes#id8" >}}),[4]({{< ref "/library/stdtypes#id9" >}}))

​	区分大小写的字符是指所属一般类别属性为 "Lu" (Letter, uppercase), "Ll" (Letter, lowercase) 或 "Lt" (Letter, titlecase) 之一的字符。

[5]([1](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#id10),[2]({{< ref "/library/stdtypes#id11" >}}))
