...
​	引入了几种独特的静态类型的第三方库，例如允许 [静态类型检查器](https://docs.python.org/zh-cn/3.13/glossary.html#term-static-type-checker) 区分简单型和感知型日期时间。
...
​	一个理想化的简单型日期，它假设当今的公历在过去和未来永远有效。 属性: [`year`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.date.year), [`month`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.date.month), and [`day`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.date.day)。
...
​	The most positive [`timedelta`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.timedelta) object, `timedelta(days=999999999, hours=23, minutes=59, seconds=59, microseconds=999999)`.
...
[`date`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.date) 对象也被泛型函数 [`copy.replace()`](https://docs.python.org/zh-cn/3.13/library/copy.html#copy.replace) 所支持。
...
*在 3.9 版本发生变更:* 结果由元组改为 [named tuple](https://docs.python.org/zh-cn/3.13/glossary.html#term-named-tuple)。
...
### [`date`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.date) 用法示例
...
​	另请参阅 [`now()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.now), [`fromtimestamp()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.fromtimestamp)。
...
​	如果 *tz* 不为 `None`，它必须是 [`tzinfo`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo) 子类的一个实例，并且时间戳将被转换到 *tz* 指定的时区。
...
*自 3.12 版本弃用:* 请用带 [`UTC`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.UTC) 的 [`datetime.fromtimestamp()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.fromtimestamp) 代替。
...
​	最早的可表示 [`datetime`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime)，`datetime(MINYEAR, 1, 1, tzinfo=None)`。
...
​	如果 [`tzinfo`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.tzinfo) 为 `None`，则返回 `None`，否则返回 `self.tzinfo.dst(self)`，并且在后者不返回 `None` 或者一个幅度小于一天的 [`timedelta`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.timedelta) 对象时将引发异常。
...
​	在布尔运算时，[`time`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.time) 对象总是被视为真值。
...
[`time`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.time) 对象也被泛型函数 [`copy.replace()`](https://docs.python.org/zh-cn/3.13/library/copy.html#copy.replace) 所支持。
...
- `'hours'`: 以两个数码的 `HH` 格式 包含 [`hour`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.time.hour)。
- `'minutes'`: 以 `HH:MM` 格式包含 [`hour`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.time.hour) 和 [`minute`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.time.minute)。
- `'seconds'`: 以 `HH:MM:SS` 格式包含 [`hour`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.time.hour), [`minute`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.time.minute) 和 [`second`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.time.second)。
- `'milliseconds'`: 包含完整时间，但将秒值的小数部分截断至毫秒。 格式为 `HH:MM:SS.sss`。
- `'microseconds'`: 以 `HH:MM:SS.ffffff` 格式包含完整时间。
...
​	返回一个由显式格式字符串所控制的，代表时间的字符串。 另请参阅 [strftime() 和 strptime() 的行为](https://docs.python.org/zh-cn/3.13/library/datetime.html#strftime-strptime-behavior) 和 [`time.isoformat()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.time.isoformat)。

## time.**__format__**(*format*)

​	与 [`time.strftime()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.time.strftime) 相同。 此方法使得在 [格式化字符串字面值](https://docs.python.org/zh-cn/3.13/reference/lexical_analysis.html#f-strings) 中以及使用 [`str.format()`](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#str.format) 时为 [`time`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.time) 对象指定格式字符串成为可能。 另请参阅 [strftime() 和 strptime() 的行为](https://docs.python.org/zh-cn/3.13/library/datetime.html#strftime-strptime-behavior) 和 [`time.isoformat()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.time.isoformat)。

## time.**utcoffset**()

​	如果 [`tzinfo`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.time.tzinfo) 为 `None`，则返回 `None`，否则返回 `self.tzinfo.utcoffset(None)`，并且在后者不返回 `None` 或一个幅度小于一天的 a [`timedelta`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.timedelta) 对象时将引发异常。

*在 3.7 版本发生变更:* UTC 时差不再限制为一个整数分钟值。

## time.**dst**()

​	如果 [`tzinfo`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.time.tzinfo) 为 `None`，则返回 `None`，否则返回 `self.tzinfo.dst(None)`，并且在后者不返回 `None` 或者一个幅度小于一天的 [`timedelta`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.timedelta) 对象时将引发异常。

*在 3.7 版本发生变更:* DST 差值不再限制为一个整数分钟值。

## time.**tzname**()

​	如果 [`tzinfo`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.time.tzinfo) 为 `None`，则返回 `None`，否则返回 `self.tzinfo.tzname(None)`，如果后者不返回 `None` 或者一个字符串对象则将引发异常。

### 用法示例: [`time`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.time)
...
## [`tzinfo`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo) 对象

## *class* datetime.**tzinfo**

​	这是一个抽象基类，也就是说该类不应被直接实例化。 请定义 [`tzinfo`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo) 的子类来捕获有关特定时区的信息。

[`tzinfo`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo) 的（某个实体子类）的实例可以被传给 [`datetime`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime) 和 [`time`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.time) 对象的构造器。 这些对象会将它们的属性视为对应于本地时间，并且 [`tzinfo`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo) 对象支持展示本地时间与 UTC 的差值、时区名称以及 DST 差值的方法，都是与传给它们的日期或时间对象的相对值。

​	你需要派生一个实体子类，并且（至少）提供你使用 [`datetime`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime) 方法所需要的标准 [`tzinfo`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo) 方法的实现。 `datetime` 模块提供了 [`timezone`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.timezone)，这是 [`tzinfo`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo) 的一个简单实体子类，它能以与 UTC 的固定差值来表示不同的时区，例如 UTC 本身或北美的 EST 和 EDT。

​	对于封存操作的特殊要求：一个 [`tzinfo`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo) 子类必须具有可不带参数调用的 [`__init__()`](https://docs.python.org/zh-cn/3.13/reference/datamodel.html#object.__init__) 方法，否则它虽然可以被封存，但可能无法再次解封。 这是个技术性要求，在未来可能会被取消。

​	一个 [`tzinfo`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo) 的实体子类可能需要实现以下方法。 具体需要实现的方法取决于感知型 `datetime` 对象如何使用它。 如果有疑问，可以简单地全部实现它们。objects. If in doubt, simply implement all of them.

## tzinfo.**utcoffset**(*dt*)

​	将本地时间与 UTC 时差返回为一个 [`timedelta`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.timedelta) 对象，如果本地时区在 UTC 以东则为正值。 如果本地时区在 UTC 以西则为负值。

​	这表示与 UTC 的 *总计* 时差；举例来说，如果一个 [`tzinfo`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo) 对象同时代表时区和 DST 调整，则 [`utcoffset()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo.utcoffset) 应当返回两者的和。 如果 UTC 时差不确定则返回 `None`。 在其他情况下返回值必须为一个 [`timedelta`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.timedelta) 对象，其取值严格限制于 `-timedelta(hours=24)` 和 `timedelta(hours=24)` 之间（差值的幅度必须小于一天）。 大多数 [`utcoffset()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo.utcoffset) 的实现看起来可能像是以下两者之一:
...
​	如果 [`utcoffset()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo.utcoffset) 返回值不为 `None`，则 [`dst()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo.dst) 也不应返回 `None`。

​	默认的 [`utcoffset()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo.utcoffset) 实现会引发 [`NotImplementedError`](https://docs.python.org/zh-cn/3.13/library/exceptions.html#NotImplementedError)。

*在 3.7 版本发生变更:* UTC 时差不再限制为一个整数分钟值。

## tzinfo.**dst**(*dt*)

​	将夏令时（DST）调整返回为一个 [`timedelta`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.timedelta) 对象，如果 DST 信息未知则返回 `None`。

​	如果 DST 未启用则返回 `timedelta(0)`。 如果 DST 已启用，则将差值作为一个 [`timedelta`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.timedelta) 对象返回（请参阅 [`utcoffset()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo.utcoffset) 了解详情）。 请注意 DST 差值如果可用，就会直接被加入 [`utcoffset()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo.utcoffset) 所返回的 UTC 时差，因此无需额外查询 [`dst()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo.dst)，除非你希望单独获取 DST 信息。 例如，[`datetime.timetuple()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.timetuple) 会调用其 [`tzinfo`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.tzinfo) 属性的 [`dst()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo.dst) 方法来确定应该如何设置 [`tm_isdst`](https://docs.python.org/zh-cn/3.13/library/time.html#time.struct_time.tm_isdst) 旗标，而 [`tzinfo.fromutc()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo.fromutc) 会调用 [`dst()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo.dst) 来在跨越时区时处理 DST 的改变。

​	一个可以同时处理标准时和夏令时的 [`tzinfo`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo) 子类的实例 *tz* 必须在此情形中保持一致：

```
tz.utcoffset(dt) - tz.dst(dt)
```

​	必须为具有 `dt.tzinfo == tz` 的每个 [`datetime`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime) *dt* 返回同样的结果。 对于同样的 [`tzinfo`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo) 子类，此表达式会产生特定时区的“标准时差”，它不应依赖于具体日期或时间，而只依赖于地理位置。 [`datetime.astimezone()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.astimezone) 的实现依赖于此方法，但无法检测违反规则的情况；确保符合规则是程序员的责任。 如果一个 [`tzinfo`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo) 子类不能保证这一点，也许可以重写 [`tzinfo.fromutc()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo.fromutc) 的默认实现以便在任何情况下与 [`astimezone()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.astimezone) 正确配合。

​	大多数 [`dst()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo.dst) 的实现可能会如以下两者之一:
...
​	默认的 [`dst()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo.dst) 实现会引发 [`NotImplementedError`](https://docs.python.org/zh-cn/3.13/library/exceptions.html#NotImplementedError)。

*在 3.7 版本发生变更:* DST 差值不再限制为一个整数分钟值。

## tzinfo.**tzname**(*dt*)

​	将对应于 [`datetime`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime) 对象 *dt* 的时区名称作为字符串返回。 `datetime` 模块未定义任何有关字符串名称的内容，也不要求它具有任何特定含义。 例如``"GMT"`, ``"UTC"`, `"-500"`, `"-5:00"`, `"EDT"`, `"US/Eastern"`, `"America/New York"` 都是有效的返回值。 如果字符串名称未知则返回 `None`。 请注意这是一个方法而不是一个固定的字符串，这主要是因为某些 [`tzinfo`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo) 子类可能需要根据所传入的特定 *dt* 值返回不同的名称，特别是在 [`tzinfo`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo) 类要负责处理夏令时的场合中。

​	默认的 [`tzname()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo.tzname) 实现会引发 [`NotImplementedError`](https://docs.python.org/zh-cn/3.13/library/exceptions.html#NotImplementedError)。

​	这些方法会被 [`datetime`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime) 或 [`time`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.time) 对象调用，用来与它们的同名方法相对应。 [`datetime`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime) 对象会将自身作为传入参数，而 [`time`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.time) 对象会将 `None` 作为传入参数。 这样 [`tzinfo`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo) 子类的方法应当准备好接受 *dt* 参数值为 `None` 或是 [`datetime`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime) 类的实例。

​	当传入 `None` 时，应当由类的设计者来决定最佳回应方式。 例如，返回 `None` 适用于希望该类提示时间对象不参与 [`tzinfo`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo) 协议处理。 让 `utcoffset(None)` 返回标准 UTC 时差也许会更有用处，因为并没有其他可用于发现标准时差的约定惯例。

​	当传入一个 [`datetime`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime) 对象来回应 [`datetime`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime) 方法时，`dt.tzinfo` 与 *self* 是同一对象。 [`tzinfo`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo) 方法可以依赖这一点，除非用户代码直接调用了 [`tzinfo`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo) 方法。 此行为的目的是使得 [`tzinfo`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo) 方法将 *dt* 解读为本地时间，而不需要担心其他时区的相关对象。

​	还有一个额外的 [`tzinfo`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo) 方法，某个子类可能会希望重写它：

## tzinfo.**fromutc**(*dt*)

​	此方法会由默认的 [`datetime.astimezone()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.astimezone) 实现来调用。 当被其调用时，`dt.tzinfo` 为 *self*，并且 *dt* 的日期和时间数据会被视为表示 UTC 时间。 [`fromutc()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo.fromutc) 的目标是调整日期和时间数据，返回一个等价的表示 *self* 的本地时间的 datetime。

​	大多数 [`tzinfo`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo) 子类应该能够毫无问题地继承默认的 [`fromutc()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo.fromutc) 实现。 它的健壮性足以处理固定差值的时区以及同时负责标准时和夏令时的时区，对于后者甚至还能处理 DST 转换时间在各个年份有变化的情况。 一个默认 [`fromutc()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo.fromutc) 实现可能无法在所有情况下正确处理的例子是（与 UTC 的）标准时差取决于所经过的特定日期和时间，这种情况可能由于政治原因而出现。 默认的 [`astimezone()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.astimezone) 和 [`fromutc()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo.fromutc) 实现可能无法生成你希望的结果，如果这个结果恰好是跨越了标准时差发生改变的时刻当中的某个小时值的话。

​	忽略针对错误情况的代码，默认 [`fromutc()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo.fromutc) 实现的行为方式如下:
...
​	在以下 [`tzinfo_examples.py`](https://docs.python.org/zh-cn/3.13/_downloads/6dc1f3f4f0e6ca13cb42ddf4d6cbc8af/tzinfo_examples.py) 文件中有一些 [`tzinfo`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo) 类的例子：
...
​	请注意同时负责标准时和夏令时的 [`tzinfo`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo) 子类在每年两次的 DST 转换点上会出现不可避免的微妙问题。具体而言，考虑美国东部时区 (UTC -0500)，它的 EDT 从三月的第二个星期天 1:59 (EST) 之后一分钟开始，并在十一月的第一天星期天 1:59 (EDT) 之后一分钟结束:
...
​	请注意不同的 [`datetime`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime) 实例仅通过 [`fold`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.fold) 属性值来加以区分，它们在比较时会被视为相等。

​	不允许时间显示存在歧义的应用程序需要显式地检查 [`fold`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.fold) 属性的值，或者避免使用混合式的 [`tzinfo`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo) 子类；当使用 [`timezone`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.timezone) 或者任何其他固定差值的 [`tzinfo`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo) 子类（例如仅表示 EST (固定差值 -5 小时)，或仅表示 EDT (固定差值 -4 小时) 的类时是不会有歧义的）。

​	参见

> ## [`zoneinfo`](https://docs.python.org/zh-cn/3.13/library/zoneinfo.html#module-zoneinfo)
>
> `datetime` 模块有一个基本 [`timezone`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.timezone) 类（用来处理任意与 UTC 的固定时差）及其 [`timezone.utc`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.timezone.utc) 属性（UTC `timezone` 实例）。
>
...
## [`timezone`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.timezone) 对象

[`timezone`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.timezone) 类是 [`tzinfo`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.tzinfo) 的子类，它的每个实例都代表一个以与 UTC 的固定时差来定义的时区。

​	此类的对象不可被用于代表某些特殊地点的时区信息，这些地点在一年的不同日期会使用不同的时差，或是在历史上对民用时间进行过调整。

## *class* datetime.**timezone**(*offset*, *name=None*)

*offset* 参数必须指定为一个 [`timedelta`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.timedelta) 对象，表示本地时间与 UTC 的时差。 它必须严格限制于 `-timedelta(hours=24)` 和 `timedelta(hours=24)` 之间，否则会引发 [`ValueError`](https://docs.python.org/zh-cn/3.13/library/exceptions.html#ValueError)。
...
​	返回当 [`timezone`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.timezone) 实例被构造时指定的固定值。

*dt* 参数会被忽略。 返回值是一个 [`timedelta`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.timedelta) 实例，其值等于本地时间与 UTC 之间的时差。
...
​	返回 `dt + offset`。 *dt* 参数必须为一个感知型 [`datetime`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime) 实例，其中 `tzinfo` 值设为 `self`。
...
## [`strftime()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.strftime) 和 [`strptime()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.strptime) 的行为

[`date`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.date), [`datetime`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime) 和 [`time`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.time) 对象都支持 `strftime(format)` 方法，可用来创建由一个显式格式字符串所控制的表示时间的字符串。

​	相反地，[`datetime.strptime()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.strptime) 类会根据表示日期和时间的字符串和相应的格式字符串来创建一个 [`datetime`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime) 对象。

​	下表提供了 [`strftime()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.strftime) 与 [`strptime()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.strptime) 的高层级比较：

|          | `strftime`                                                   | `strptime`                                                   |
| :------- | :----------------------------------------------------------- | :----------------------------------------------------------- |
| 用法     | 根据给定的格式将对象转换为字符串                             | 将字符串解析为给定相应格式的 [`datetime`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime) 对象 |
| 方法类型 | 实例方法                                                     | 类方法                                                       |
| 方法     | [`date`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.date); [`datetime`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime); [`time`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.time) | [`datetime`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime) |
...
### [`strftime()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.strftime) 和 [`strptime()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.strptime) 格式码
...
​	这些代码可能不是在所有平台上都可与 [`strftime()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.strftime) 方法配合使用。 ISO 8601 年份和 ISO 8601 星期指示符并不能与上面的年份和星期序号指示符相互替代。 调用 [`strptime()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.strptime) 时传入不完整或有歧义的 ISO 8601 指示符将引发 [`ValueError`](https://docs.python.org/zh-cn/3.13/library/exceptions.html#ValueError)。
...
​	总体而言，`d.strftime(fmt)` 类似于 [`time`](https://docs.python.org/zh-cn/3.13/library/time.html#module-time) 模块的 `time.strftime(fmt, d.timetuple())` 但是并非所有对象都支持 [`timetuple()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.date.timetuple) 方法。

​	对于 [`datetime.strptime()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.strptime) 类方法，默认值为 `1900-01-01T00:00:00.000`: 任何未在格式字符串中指定的部分都将从默认值中获取。 [[4\]](https://docs.python.org/zh-cn/3.13/library/datetime.html#id8)
...
​	对于 [`time`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.time) 对象，年、月、日的格式代码不应被使用，因为 `time` 对象没有这些值。 如果它们仍被使用，则年份将被替换为 1900 而月和日将被替换为 1。

​	对于 [`date`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.date) 对象，时、分、秒和微秒的格式代码不应被使用，因为 [`date`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.date) 对象没有这些值。 如果它们仍被使用，则都将被替换为 0。

​	出于相同的原因，对于包含当前区域设置字符集所无法表示的 Unicode 码位的格式字符串的处理方式也取决于具体平台。 在某些平台上这样的码位会不加修改地原样输出，而在其他平台上 `strftime` 则可能引发 [`UnicodeError`](https://docs.python.org/zh-cn/3.13/library/exceptions.html#UnicodeError) 或只返回一个空字符串。

​	注释：

1. 因为该格式依赖于当前语言区域，所以在假定输出值时应当仔细考虑。 字段顺序可能会有变化（例如 "month/day/year" 和 "day/month/year"），并且输出还可能包含非 ASCII 字符。

2. [`strptime()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.strptime) 方法能够解析整个 [1, 9999] 范围内的年份，但 < 1000 的年份必须加零填充为 4 位数字宽度。

   *在 3.2 版本发生变更:* 在之前的版本中，[`strftime()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.strftime) 方法只限用于 >= 1900 的年份。

   *在 3.3 版本发生变更:* 在 3.2 版中，[`strftime()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.strftime) 方法只限用于 >= 1000 的年份。

3. 当与 [`strptime()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.strptime) 方法一起使用时，如果使用 `%I` 指示符来解析时，则 `%p` 指示符只会影响输出时字段。

4. 与 [`time`](https://docs.python.org/zh-cn/3.13/library/time.html#module-time) 模块不同的是，`datetime` 模块不支持闰秒。

5. 当与 [`strptime()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.strptime) 方法一起使用时，`%f` 指示符可接受一至六个数码及左边的零填充。 `%f` 是对 C 标准中格式字符集的扩展（但单独在 datetime 对象中实现，因此它总是可用）。
...
6. 对于简单型对象，`%z`, `%:z` 和 `%Z` 格式代码会被替换为空字符串。
   ...
   [`utcoffset()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.utcoffset) 会被转换为 `±HHMM[SS[.ffffff]]` 形式的字符串，其中 `HH` 为给出 UTC 时差的小时部分的 2 位数码字符串，`MM` 为给出 UTC 时差的分钟部分的 2 位数码字符串，`SS` 为给出 UTC 时差的秒部分的 2 位数码字符串，而 `ffffff` 则为给出 UTC 时差的微秒部分的 6 位数码字符串。 当时差为整数秒时 `ffffff` 部分将被省略，而当时差为整数分钟时 `ffffff` 和 `SS` 部分都将被省略。 举例来说，如果 [`utcoffset()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.utcoffset) 返回 `timedelta(hours=-3, minutes=-30)`，则 `%z` 会被替换为字符串 `'-0330'`。

   *在 3.7 版本发生变更:* UTC 时差不再限制为一个整数分钟值。

   *在 3.7 版本发生变更:* 当向 [`strptime()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.strptime) 方法提供 `%z` 指示符时，UTC 差值可以在时、分和秒之间使用冒号作为分隔符。 例如，`'+01:00:00'` 将被解读为一小时的差值。 此外，提供 `'Z'` 就相当于 `'+00:00'`。
   ...
   ​	在 [`strftime()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.strftime) 中，如果 [`tzname()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.tzname) 返回 `None` 则 `%Z` 会被替换为一个空字符串；在其他情况下 `%Z` 会被替换为该返回值，它必须为一个字符串。

   [`strptime()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.strptime) 仅接受特定的 `%Z` 值：
   ...
   *在 3.2 版本发生变更:* 当提供 `%z` 指示符给 [`strptime()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.strptime) 方法时，将产生一个感知型 [`datetime`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime) 对象。 结果的 `tzinfo` 将被设为一个 [`timezone`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.timezone) 实例。

7. 当与 [`strptime()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.strptime) 方法一起使用时，`%U` 和 `%W` 仅用于指定了星期值和日历年份 (`%Y`) 的计算。

8. 类似于 `%U` 和 `%W`，`%V` 仅用于在 [`strptime()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.strptime) 格式字符串中指定了星期值和 ISO 年份 (`%G`) 的计算。 还要注意 `%G` 和 `%Y` 是不可互换的。

9. 当与 [`strptime()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.strptime) 方法一起使用时，前导的零在格式 `%d`, `%m`, `%H`, `%I`, `%M`, `%S`, `%j`, `%U`, `%W` 和 `%V` 中是可选的。 格式 `%y` 则要求有前导的零。
...
10. 当使用 [`strptime()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.strptime) 解析月份和日期时，始终在格式字符串中包括年份。 如果你需要解析的值缺少年份，则添加一个显式的占位用闰年。 否则当你的代码遇到一个闰日时将引发异常因为解析器所使用的默认年份不是一个闰年。 用户会每隔四年碰到这个程序错误...
    ...
    *Deprecated since version 3.13, will be removed in version 3.15:* 使用包含不带年份的月份日期的格式字符串的 [`strptime()`](https://docs.python.org/zh-cn/3.13/library/datetime.html#datetime.datetime.strptime) 调用现在将引发 [`DeprecationWarning`](https://docs.python.org/zh-cn/3.13/library/exceptions.html#DeprecationWarning)。 在 3.15 或更新的版本中我们可能将其修改为引发错误或将默认年从修改为一年闰年。 参见 [gh-70647](https://github.com/python/cpython/issues/70647)。

​	备注

[[1](https://docs.python.org/zh-cn/3.13/library/datetime.html#id1)]

​	就是说如果我们忽略相对论效应的话。

[[2](https://docs.python.org/zh-cn/3.13/library/datetime.html#id2)]
...
[[3](https://docs.python.org/zh-cn/3.13/library/datetime.html#id3)]
...
[[4](https://docs.python.org/zh-cn/3.13/library/datetime.html#id4)]
