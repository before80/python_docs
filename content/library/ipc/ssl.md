...
​	如果连接另一端的对等方没有证书，则返回 `None`。 如果 SSL 握手还未完成，则会引发 [`ValueError`](https://docs.python.org/zh-cn/3.13/library/exceptions.html#ValueError)。
...
| *客户端* / **服务器**                                        | **SSLv2**                                                    | **SSLv3**                                                    | **TLS** [[3\]](https://docs.python.org/zh-cn/3.13/library/ssl.html#id9) | **TLSv1** | **TLSv1.1** | **TLSv1.2** |
| ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ | --------- | ----------- | ----------- |
...
​	对于客户端套接字，上下文的构造会延迟执行；如果下层的套接字尚未连接，上下文的构造将在对套接字调用 `connect()` 之后执行。 对于服务器端套接字，如果套接字没有远端对等方，它会被视为一个监听套接字，并且服务器端 SSL 包装操作会在通过 `accept()` 方法所接受的客户端连接上自动执行。 此方法可能会引发 [`SSLError`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLError)。
...
​	要将 [`SSLSocket`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket) 包装在另一个 [`SSLSocket`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket) 中，请使用 [`SSLContext.wrap_bio()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLContext.wrap_bio)。

*在 3.5 版本发生变更:* 总是允许传送 server_hostname，即使 OpenSSL 没有 SNI。

*在 3.6 版本发生变更:* 增加了 *session* 参数。

*在 3.7 版本发生变更:* 此方法返回 [`SSLContext.sslsocket_class`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLContext.sslsocket_class) 的实例而不是硬编码的 [`SSLSocket`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket)。
...
​	包装 BIO 对象 *incoming* 和 *outgoing* 并返回一个 [`SSLContext.sslobject_class`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLContext.sslobject_class) (默认为 [`SSLObject`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLObject)) 的实例。 SSL 例程将从 BIO 中读取输入数据并将数据写入到 outgoing BIO。

*server_side*, *server_hostname* 和 *session* 形参具有与 [`SSLContext.wrap_socket()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLContext.wrap_socket) 中相同的含义。

*在 3.6 版本发生变更:* 增加了 *session* 参数。

*在 3.7 版本发生变更:* 此方法返回 [`SSLContext.sslobject_class`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLContext.sslobject_class) 的实例而不是硬编码的 [`SSLObject`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLObject)。
...
​	是否要将匹配 [`SSLSocket.do_handshake()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket.do_handshake) 中对等方证书的主机名。 该上下文的 [`verify_mode`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLContext.verify_mode) 必须被设为 [`CERT_OPTIONAL`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.CERT_OPTIONAL) 或 [`CERT_REQUIRED`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.CERT_REQUIRED)，并且你必须将 *server_hostname* 传给 [`wrap_socket()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLContext.wrap_socket) 以便匹配主机名。 启用主机名检查会自动将 [`verify_mode`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLContext.verify_mode) 从 [`CERT_NONE`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.CERT_NONE) 设为 [`CERT_REQUIRED`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.CERT_REQUIRED)。 只要启用了主机名检查就无法将其设回 [`CERT_NONE`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.CERT_NONE)。 [`PROTOCOL_TLS_CLIENT`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.PROTOCOL_TLS_CLIENT) 协议默认启用主机名检查。 对于其他协议，则必须显式地启用主机名检查。
...
​	一个代表所支持的最高 TLS 版本的 [`TLSVersion`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.TLSVersion) 枚举成员。 该值默认为 [`TLSVersion.MAXIMUM_SUPPORTED`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.TLSVersion.MAXIMUM_SUPPORTED)。 这个属性对于 [`PROTOCOL_TLS`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.PROTOCOL_TLS), [`PROTOCOL_TLS_CLIENT`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.PROTOCOL_TLS_CLIENT) 和 [`PROTOCOL_TLS_SERVER`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.PROTOCOL_TLS_SERVER) 以外的其他协议来说都是只读的。
...
​	一个代表此上下文中所启用的 SSL 选项集的整数。 默认值为 [`OP_ALL`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.OP_ALL)，但你也可以通过在选项间进行 OR 运算来指定其他选项例如 [`OP_NO_SSLv2`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.OP_NO_SSLv2)。
...
*自 3.7 版本弃用:* 自 OpenSSL 1.1.0 起，所有 `OP_NO_SSL*` 和 `OP_NO_TLS*` 选项已被弃用，请改用新的 [`SSLContext.minimum_version`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLContext.minimum_version) 和 [`SSLContext.maximum_version`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLContext.maximum_version)。
...
​	是否要尝试验证其他对等方的证书以及如果验证失败应采取何种行为。 该属性值必须为 [`CERT_NONE`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.CERT_NONE), [`CERT_OPTIONAL`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.CERT_OPTIONAL) 或 [`CERT_REQUIRED`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.CERT_REQUIRED) 之一。
...
​	形参 `callback` 是一个签名为 `def callback(hint: str | None) -> tuple[str | None, bytes]` 的可调用对象。`hint` 形参是服务器所发送的可选身份标识。 返回值是一个 (client-identity, psk) 形式的元组。 其中 client-identity 是一个可选的字符串，它可被服务器用来为客户选择相应的 PSK。 该字符串在使用 UTF-8 编码时必须小于等于 `256` 个八位字节。 PSK 是一个代表预共享密钥的 [bytes-like object](https://docs.python.org/zh-cn/3.13/glossary.html#term-bytes-like-object)。 返回一个零长度的 PSK 以拒绝连接。

​	将 `callback` 设为 [`None`](https://docs.python.org/zh-cn/3.13/library/constants.html#None) 将移除任何现有的回调。
...
- `hint` 形参值将始终为 [`None`](https://docs.python.org/zh-cn/3.13/library/constants.html#None)。
...
​	形参 `callback` 是一个签名为 `def callback(identity: str | None) -> bytes` 的可调用对象。 `identity` 形参是客户端所发送的可选身份标识，可用于选择相应的 PSK。 返回值是一个代表预共享密钥的 [bytes-like object](https://docs.python.org/zh-cn/3.13/glossary.html#term-bytes-like-object)。 返回一个零长度的 PSK 以拒绝连接。
...
​	私钥往往与证书存储在相同的文件中；在此情况下，只需要将 `certfile` 形参传给 [`SSLContext.load_cert_chain()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLContext.load_cert_chain)。 如果私钥是与证书一起存储的，则它应当放在证书链的第一个证书之前:
...
[`PROTOCOL_TLS_CLIENT`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.PROTOCOL_TLS_CLIENT) 协议配置用于证书验证和主机名验证的上下文。 [`verify_mode`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLContext.verify_mode) 设为 [`CERT_REQUIRED`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.CERT_REQUIRED) 而 [`check_hostname`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLContext.check_hostname) 设为 `True`。 所有其他协议都会使用不安全的默认值创建 SSL 上下文。
...
​	当有客户端连接时，你将在套接字上调用 `accept()` 以从另一端获取新的套接字，并使用上下文的 [`SSLContext.wrap_socket()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLContext.wrap_socket) 方法来为连接创建一个服务器端 SSL 套接字:
...
​	并返回至监听新的客户端连接（当然，真正的服务器应当会在单独的线程中处理每个客户端连接，或者将套接字设为 [非阻塞模式](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl-nonblocking) 并使用事件循环）。
...
- 如果一个 I/O 操作会阻塞，大多数 [`SSLSocket`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket) 方法都将引发 [`SSLWantWriteError`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLWantWriteError) 或 [`SSLWantReadError`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLWantReadError) 而非 [`BlockingIOError`](https://docs.python.org/zh-cn/3.13/library/exceptions.html#BlockingIOError)。 如果有必要在下层套接字上执行读取操作将引发 [`SSLWantReadError`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLWantReadError)，在下层套接字上执行写入操作则将引发 [`SSLWantWriteError`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLWantWriteError)。 请注意尝试 *写入* 到 SSL 套接字可能需要先从下层套接字 *读取*，而尝试从 SSL 套接字 *读取* 则可能需要先向下层套接字 *写入*。

  *在 3.5 版本发生变更:* 在较早的 Python 版本中，`SSLSocket.send()` 方法会返回零值而非引发 [`SSLWantWriteError`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLWantWriteError) 或 [`SSLWantReadError`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLWantReadError)。

- 调用 [`select()`](https://docs.python.org/zh-cn/3.13/library/select.html#select.select) 将告诉你可以从 OS 层级的套接字读取（或向其写入），但这并不意味着在上面的 SSL 层有足够的数据。 例如，可能只有部分 SSL 帧已经到达。 因此，你必须准备好处理 `SSLSocket.recv()` 和 `SSLSocket.send()` 失败的情况，并在再次调用 [`select()`](https://docs.python.org/zh-cn/3.13/library/select.html#select.select) 之后重新尝试。

- 相反地，由于 SSL 层具有自己的帧机制，一个 SSL 套接字可能仍有可读取的数据而 [`select()`](https://docs.python.org/zh-cn/3.13/library/select.html#select.select) 并不知道这一点。 因此，你应当先调用 `SSLSocket.recv()` 取走所有潜在的可用数据，然后只在必要时对 [`select()`](https://docs.python.org/zh-cn/3.13/library/select.html#select.select) 调用执行阻塞。

  (当然，类似的保留规则在使用其他原语例如 [`poll()`](https://docs.python.org/zh-cn/3.13/library/select.html#select.poll)，或 [`selectors`](https://docs.python.org/zh-cn/3.13/library/selectors.html#module-selectors) 模块中的原语时也适用)

- SSL 握手本身将是非阻塞的: [`SSLSocket.do_handshake()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket.do_handshake) 方法必须不断重试直至其成功返回。 下面是一个使用 [`select()`](https://docs.python.org/zh-cn/3.13/library/select.html#select.select) 来等待套接字就绪的简短例子:
...
[`asyncio`](https://docs.python.org/zh-cn/3.13/library/asyncio.html#module-asyncio) 模块支持 [非阻塞 SSL 套接字](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl-nonblocking) 并提供了更高层级的 API。 它会使用 [`selectors`](https://docs.python.org/zh-cn/3.13/library/selectors.html#module-selectors) 模块来轮询事件并处理 [`SSLWantWriteError`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLWantWriteError), [`SSLWantReadError`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLWantReadError) 和 [`BlockingIOError`](https://docs.python.org/zh-cn/3.13/library/exceptions.html#BlockingIOError) 等异常。 它还会异步地执行 SSL 握手。
...
​	网络 IO API 与 [`socket.socket`](https://docs.python.org/zh-cn/3.13/library/socket.html#socket.socket) 所提供的功能一致，[`SSLSocket`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket) 也是从那里继承而来的。 这允许 SSL 套接字被用作常规套接字的替代，使得向现有应用程序添加 SSL 支持变得非常容易。

​	将 SSL 协议处理与网络 IO 结合使用通常都能运行良好，但在某些情况下则不能。 此情况的一个例子是 async IO 框架，该框架要使用不同的 IO 多路复用模型而非 (基于就绪状态的) "在文件描述器上执行选择/轮询" 模型，该模型是 [`socket.socket`](https://docs.python.org/zh-cn/3.13/library/socket.html#socket.socket) 和内部 OpenSSL 套接字 IO 例程正常运行的假设前提。 这种情况在该模型效率不高的 Windows 平台上最为常见。 为此还提供了一个 [`SSLSocket`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket) 的简化形式，称为 [`SSLObject`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLObject)。

## *class* ssl.**SSLObject**

[`SSLSocket`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket) 的简化形式，表示一个不包含任何网络 IO 方法的 SSL 协议实例。 这个类通常由想要通过内存缓冲区为 SSL 实现异步 IO 的框架作者来使用。

​	这个类在低层级 SSL 对象上实现了一个接口，与 OpenSSL 所实现的类似。 此对象会捕获 SSL 连接的状态但其本身不提供任何网络 IO。 IO 需要通过单独的 "BIO" 对象来执行，该对象是 OpenSSL 的 IO 抽象层。

​	这个类没有公有构造器。 [`SSLObject`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLObject) 实例必须使用 [`wrap_bio()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLContext.wrap_bio) 方法来创建。 此方法将创建 [`SSLObject`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLObject) 实例并将其绑定到一个 BIO 对。 其中 *incoming* BIO 用来将数据从 Python 传递到 SSL 协议实例，而 *outgoing* BIO 用来进行数据反向传递。

​	可以使用以下方法：

- [`context`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket.context)
- [`server_side`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket.server_side)
- [`server_hostname`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket.server_hostname)
- [`session`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket.session)
- [`session_reused`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket.session_reused)
- [`read()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket.read)
- [`write()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket.write)
- [`getpeercert()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket.getpeercert)
- [`get_verified_chain()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket.get_verified_chain)
- [`get_unverified_chain()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket.get_unverified_chain)
- [`selected_alpn_protocol()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket.selected_alpn_protocol)
- [`selected_npn_protocol()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket.selected_npn_protocol)
- [`cipher()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket.cipher)
- [`shared_ciphers()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket.shared_ciphers)
- [`compression()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket.compression)
- [`pending()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket.pending)
- [`do_handshake()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket.do_handshake)
- [`verify_client_post_handshake()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket.verify_client_post_handshake)
- [`unwrap()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket.unwrap)
- [`get_channel_binding()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket.get_channel_binding)
- [`version()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket.version)

​	与 [`SSLSocket`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket) 相比，此对象缺少下列特性:

- 任何形式的网络 IO; `recv()` 和 `send()` 仅对下层的 [`MemoryBIO`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.MemoryBIO) 缓冲区执行读取和写入。
- 不存在 *do_handshake_on_connect* 机制。 你必须总是手动调用 [`do_handshake()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket.do_handshake) 来开始握手操作。
- 不存在对 *suppress_ragged_eofs* 的处理。 所有违反协议的文件结束条件将通过 [`SSLEOFError`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLEOFError) 异常来报告。
- 方法 [`unwrap()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket.unwrap) 的调用不返回任何东西，不会如 SSL 套接字那样返回下层的套接字。
- *server_name_callback* 回调被传给 [`SSLContext.set_servername_callback()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLContext.set_servername_callback) 时将获得一个 [`SSLObject`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLObject) 实例而非 [`SSLSocket`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket) 实例作为其第一个形参。

​	有关 [`SSLObject`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLObject) 用法的一些说明:

- 在 [`SSLObject`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLObject) 上的所有 IO 都是 [非阻塞的](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl-nonblocking)。 这意味着例如 [`read()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket.read) 在其需要比 incoming BIO 可用的更多数据时将会引发 [`SSLWantReadError`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLWantReadError)。

*在 3.7 版本发生变更:* [`SSLObject`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLObject) 的实例必须使用 [`wrap_bio()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLContext.wrap_bio) 来创建。 在较早的版本中，直接创建该实例是可能的。 但这从未被写入文档或是被正式支持。

​	SSLObject 会使用内存缓冲区与外部世界通信。 [`MemoryBIO`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.MemoryBIO) 类提供了可被用于此目的的内存缓冲区。 它包装了一个 OpenSSL 内存 BIO (Basic IO) 对象:
...
​	将一个 EOF 标记写入到内存 BIO。 在此方法被调用以后，再调用 [`write()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.MemoryBIO.write) 将是非法的。 属性 [`eof`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.MemoryBIO.eof) will 在缓冲区当前的所有数据都被读取之后将变为真值。
...
[`session`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket.session) 所使用的会话对象。
...
​	针对 **客户端使用**，如果你对于安全策略没有任何特殊要求，则强烈推荐你使用 [`create_default_context()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.create_default_context) 函数来创建你的 SSL 上下文。 它将加载系统的受信任 CA 证书，启用证书验证和主机名检查，并尝试合理地选择安全的协议和密码设置。
...
​	如果连接需要客户端证书，可使用 [`SSLContext.load_cert_chain()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLContext.load_cert_chain) 来添加。

​	作为对比，如果你通过自行调用 [`SSLContext`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLContext) 构造器来创建 SSL 上下文，它默认将不会启用证书验证和主机名检查。 如果你这样做，请阅读下面的段落以达到良好的安全级别。

### 手动设置

#### 验证证书

​	当直接调用 [`SSLContext`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLContext) 构造器时，默认值为 [`CERT_NONE`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.CERT_NONE)。 由于它不会验证对端的身份，因而是不安全的，特别是在大部分时间里你都希望能确保与你通信的服务器的可靠性的客户端模式下。 为此，当在客户端模式下，强烈建议使用 [`CERT_REQUIRED`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.CERT_REQUIRED)。 但是，仅靠它本身是不够的；你还必须检查服务器证书，它可通过调用 [`SSLSocket.getpeercert()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket.getpeercert) 来获取，确定它与目标服务相匹配。 对于许多协议和应用程序来说，服务可通过主机名来进行标识。 这种通用检查会在 [`SSLContext.check_hostname`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLContext.check_hostname) 已启用时自动执行。

*在 3.7 版本发生变更:* 主机名匹配现在是由 OpenSSL 来执行的。 Python 不会再使用 `match_hostname()`。

​	在服务器模式下，如果你想要使用 SSL 层来验证客户端（而不是使用更高层级的验证机制），你也必须要指定 [`CERT_REQUIRED`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.CERT_REQUIRED) 并以类似方式检查客户端证书。
...
​	上面创建的 SSL 上下文将只允许与服务器进行 TLSv1.3 及更高版本（如果你的系统支持）的连接。 在默认情况下 [`PROTOCOL_TLS_CLIENT`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.PROTOCOL_TLS_CLIENT) 将使用证书验证和主机名检查。 你必须将证书加载到上下文中。
...
​	如果使用此模块作为多进程应用的一部分（例如，使用 [`multiprocessing`](https://docs.python.org/zh-cn/3.13/library/multiprocessing.html#module-multiprocessing) 或 [`concurrent.futures`](https://docs.python.org/zh-cn/3.13/library/concurrent.futures.html#module-concurrent.futures) 模块），请注意 OpenSSL 的内部随机数字生成器并不能正确处理分叉的进程。 应用程序必须修改父进程的 PRNG 状态，如果它们要使用任何包含 [`os.fork()`](https://docs.python.org/zh-cn/3.13/library/os.html#os.fork) 的 SSL 特征的话。 任何对 [`RAND_add()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.RAND_add) 或 [`RAND_bytes()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.RAND_bytes) 的成功调用都可以做到这一点。
...
- TLS 1.3 使用一组不同的加密套件集。 默认情况下所有 AES-GCM 和 ChaCha20 加密套件都会被启用。 [`SSLContext.set_ciphers()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLContext.set_ciphers) 方法还不能启用或禁用任何 TLS 1.3 加密，但 [`SSLContext.get_ciphers()`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLContext.get_ciphers) 会返回它们。
- 会话凭据不再会作为初始握手的组成部分被发送而是以不同的方式来处理。 [`SSLSocket.session`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSocket.session) 和 [`SSLSession`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLSession) 与 TLS 1.3 不兼容。
- 客户端证书在初始握手期间也不会再被验证。 服务器可以在任何时候请求证书。 客户端会在它们从服务器发送或接收应用数据时处理证书请求。
- 早期数据、延迟的 TLS 客户端证书请求、签名算法配置和密钥重生成等 TLS 1.3 特性尚未被支持。

> 参见
>
> - Class [`socket.socket`](https://docs.python.org/zh-cn/3.13/library/socket.html#socket.socket)
>
>   下层 [`socket`](https://docs.python.org/zh-cn/3.13/library/socket.html#module-socket) 类的文档
>
