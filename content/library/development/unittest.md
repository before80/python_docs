...
​	继承 [`unittest.TestCase`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestCase) 就创建了一个测试样例。上述三个独立的测试是三个类的方法，这些方法的命名都以 `test` 开头。 这个命名约定告诉测试运行者类的哪些方法表示测试。
...
​	相似的，我们提供了一个 [`tearDown()`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestCase.tearDown) 方法在测试方法运行后进行清理工作。
...
​	一些用户希望直接使用 [`unittest`](https://docs.python.org/zh-cn/3.13/library/unittest.html#module-unittest) 运行已有的测试代码，而不需要把已有的每个测试函数转化为一个 [`TestCase`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestCase) 的子类。
...
​	这个类实现了 [`TestCase`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestCase) 的部分接口，允许测试运行方驱动测试，但不提供可被测试代码用来检查和报告错误的方法。 这个类被用于创建使用传统测试代码的测试用例，允许它被集成到基于 [`unittest`](https://docs.python.org/zh-cn/3.13/library/unittest.html#module-unittest) 的测试框架中。
...
​	返回包含在给定模块中的所有测试用例的测试套件。 此方法会在 *module* 中搜索从派生自 [`TestCase`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestCase) 的类并为该类定义的每个测试方法创建一个类实例。
...
​	一个包含 [`TestCase`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestCase) 实例和保存了格式化回溯信息的字符串的 2 元组的列表。 每个元素代表一个使用 [assert* 方法](https://docs.python.org/zh-cn/3.13/library/unittest.html#assert-methods) 显式地发出失败信号的测试。
...
​	一个包含 [`TestCase`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestCase) 实例和保存了格式化回溯信息的 2 元组的列表。 每个元组代表测试用例的一个已预期的失败或错误。

## **unexpectedSuccesses**

​	一个包含被标记为已预期失败，但却测试成功的 [`TestCase`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestCase) 实例的列表。
...
​	当测试的执行应当被 [`stop()`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestResult.stop) 停止时则设为 `True`。
...
*在 3.4 版本发生变更:* 如果有任何来自测试的 [`unexpectedSuccesses`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestResult.unexpectedSuccesses) 被 [`expectedFailure()`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.expectedFailure) 装饰器所标记则返回 `False`。

## **stop**()

​	此方法可被调用以提示正在运行的测试集要将 [`shouldStop`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestResult.shouldStop) 属性设为 `True` 来表示其应当被中止。 `TestRunner` 对象应当认同此旗标并返回而不再运行任何额外的测试。

​	例如，该特性会被 [`TextTestRunner`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TextTestRunner) 类用来在当用户从键盘发出一个中断信号时停止测试框架。 提供了 `TestRunner` 实现的交互式工具也可通过类似方式来使用该特性。

[`TestResult`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestResult) 类的下列方法被用于维护内部数据结构，并可在子类中被扩展以支持额外的报告需求。 这特别适用于构建支持在运行测试时提供交互式报告的工具。
...
​	当测试用例 *test* 引发了非预期的异常时将被调用。 *err* 是一个元组，其形式与 [`sys.exc_info()`](https://docs.python.org/zh-cn/3.13/library/sys.html#sys.exc_info) 的返回值相同: `(type, value, traceback)`。

​	默认实现会将一个元组 `(test, formatted_err)` 添加到实例的 [`errors`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestResult.errors) 属性，其中 *formatted_err* 是派生自 *err* 的已格式化回溯信息。

## **addFailure**(*test*, *err*)

​	当测试用例 *test* 发出了失败信号时将被调用。 *err* 是一个元组，其形式与 [`sys.exc_info()`](https://docs.python.org/zh-cn/3.13/library/sys.html#sys.exc_info) 的返回值相同: `(type, value, traceback)`。

​	默认实现会将一个元组 `(test, formatted_err)` 添加到实例的 [`failures`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestResult.failures) 属性，其中 *formatted_err* 是派生自 *err* 的已格式化回溯信息。
...
​	默认实现会将一个元组 `(test, reason)` 添加到实例的 [`skipped`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestResult.skipped) 属性。

## **addExpectedFailure**(*test*, *err*)

​	当测试用例 *test* 失败或发生错误，但是使用了 [`expectedFailure()`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.expectedFailure) 装饰器来标记时将被调用。

​	默认实现会将一个元组 `(test, formatted_err)` 添加到实例的 [`expectedFailures`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestResult.expectedFailures) 属性，其中 *formatted_err* 是派生自 *err* 的已格式化回溯信息。

## **addUnexpectedSuccess**(*test*)

​	当测试用例 *test* 使用了was marked with the [`expectedFailure()`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.expectedFailure) 装饰器来标记，但是却执行成功时将被调用。

​	默认实现会将该测试添加到实例的 [`unexpectedSuccesses`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestResult.unexpectedSuccesses) 属性。

## **addSubTest**(*test*, *subtest*, *outcome*)

​	当一个子测试结束时将被调用。 *test* 是对应于该测试方法的测试用例。 *subtest* 是一个描述该子测试的 [`TestCase`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestCase) 实例。

​	如果 *outcome* 为 [`None`](https://docs.python.org/zh-cn/3.13/library/constants.html#None)，则该子测试执行成功。 否则，它将失败并引发一个异常，*outcome* 是一个元组，其形式与 [`sys.exc_info()`](https://docs.python.org/zh-cn/3.13/library/sys.html#sys.exc_info) 的返回值相同: `(type, value, traceback)`。
...
​	用于分享的 [`TestLoader`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestLoader) 类实例。 如果不需要自制 [`TestLoader`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestLoader)，则可以使用该实例而不必重复创建新的实例。
...
​	一个将结果输出到流的基本测试运行器。 如果 *stream* 为默认的 `None`，则会使用 [`sys.stderr`](https://docs.python.org/zh-cn/3.13/library/sys.html#sys.stderr) 作为输出流。 这个类具有一些配置形参，但实际上都非常简单。 运行测试套件的图形化应用程序应当提供替代实现。 这样的实现应当在添加新特性到 unittest 时接受 `**kwargs` 作为修改构造运行器的接口。

​	在默认情况下该运行器将显示 [`DeprecationWarning`](https://docs.python.org/zh-cn/3.13/library/exceptions.html#DeprecationWarning), [`PendingDeprecationWarning`](https://docs.python.org/zh-cn/3.13/library/exceptions.html#PendingDeprecationWarning), [`ResourceWarning`](https://docs.python.org/zh-cn/3.13/library/exceptions.html#ResourceWarning) 和 [`ImportWarning`](https://docs.python.org/zh-cn/3.13/library/exceptions.html#ImportWarning) 即使它们 [默认会被忽略](https://docs.python.org/zh-cn/3.13/library/warnings.html#warning-ignored)。 此行为可使用 Python 的 `-Wd` 或 `-Wa` 选项 并将 *warnings* 保持为 `None` 来覆盖 (参见 [警告控制](https://docs.python.org/zh-cn/3.13/using/cmdline.html#using-on-warnings))。

*在 3.2 版本发生变更:* 增加了 *warnings* 形参。

*在 3.2 版本发生变更:* 默认流会在实例化而不是在导入时被设为 [`sys.stderr`](https://docs.python.org/zh-cn/3.13/library/sys.html#sys.stderr)。
...
​	此方法将返回由 [`run()`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TextTestRunner.run) 使用的 `TestResult` 实例。 它不应当被直接调用，但可在子类中被重载以提供自定义的 `TestResult`。

`_makeResult()` 会实例化传给 `TextTestRunner` 构造器的 `resultclass` 参数所指定的类或可迭代对象。 如果没有提供 `resultclass` 则默认为 [`TextTestResult`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TextTestResult)。 结果类会使用以下参数来实例化:
...
​	此方法是 `TextTestRunner` 的主要公共接口。 此方法接受一个 [`TestSuite`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestSuite) 或 [`TestCase`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestCase) 实例。 通过调用 [`_makeResult()`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TextTestRunner._makeResult) 创建 [`TestResult`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestResult) 来运行测试并将结果打印到标准输出。
...
*argv* 参数可以是传给程序的选项列表，其中第一个元素是程序名。 如未指定或为 `None`，则会使用 [`sys.argv`](https://docs.python.org/zh-cn/3.13/library/sys.html#sys.argv) 的值。

*testRunner* 参数可以是一个测试运行器类或是其已创建的实例。 在默认情况下 `main` 会调用 [`sys.exit()`](https://docs.python.org/zh-cn/3.13/library/sys.html#sys.exit) 并附带一个退出码来指明测试运行是成功 (0) 还是失败 (1)。 退出码为 5 表示没有运行或跳过任何测试。

*testLoader* 参数必须是一个 [`TestLoader`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestLoader) 实例，其默认值为 [`defaultTestLoader`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.defaultTestLoader)。
...
*failfast*, *catchbreak* 和 *buffer* 形参的效果与同名的 [command-line options](https://docs.python.org/zh-cn/3.13/library/unittest.html#command-line-options) 一致。

*warnings* 参数指定在运行测试时所应使用的 [警告过滤器](https://docs.python.org/zh-cn/3.13/library/warnings.html#warning-filter)。 如果未指定，则默认的 `None` 会在将 `-W` 选项传给 **python** 命令时被保留 (参见 [警告控制](https://docs.python.org/zh-cn/3.13/using/cmdline.html#using-on-warnings))，而在其他情况下将被设为 `'default'`。

​	调用 `main` 将返回一个带有 `result` 属性的对象，该属性包含 [`unittest.TestResult`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestResult) 形式的测试运行结果。
...
​	如果一个测试模块定义了 `load_tests` 则它将被 [`TestLoader.loadTestsFromModule()`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestLoader.loadTestsFromModule) 调用并传入下列参数:
...
​	它应当返回一个 [`TestSuite`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestSuite)。

*loader* 是执行载入操作的 [`TestLoader`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestLoader) 实例。 *standard_tests* 是默认要从该模块载入的测试。 测试模块通常只需从标准测试集中添加或移除测试。 第三个参数是在作为测试发现的一部分载入包时使用的。

​	一个从指定 [`TestCase`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestCase) 类集合中载入测试的 `load_tests` 函数看起来可能是这样的:
...
​	如果发现操作是在一个包含包的目录中开始的，不论是通过命令行还是通过调用 [`TestLoader.discover()`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestLoader.discover)，则将在包 `__init__.py` 中检查 `load_tests`。 如果不存在此函数，则发现将在包内部执行递归，就像它是另一个目录一样。 在其他情况下，包中测试的发现操作将留给 `load_tests` 执行，它将附带下列参数被调用:

```
load_tests(loader, standard_tests, pattern)
```

​	这应当返回代表包中所有测试的 [`TestSuite`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestSuite)。 (`standard_tests` 将只包含从 `__init__.py` 获取的测试。)
...
​	类与模块设定是在 [`TestSuite`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestSuite) 中实现的。 当测试套件遇到来自新类的测试时则来自之前的类（如果存在）的 `tearDownClass()` 会被调用，然后再调用来自新类的 `setUpClass()`。
...
​	如果在共享的设定函数中引发了任何异常则测试将被报告错误。 因为没有对应的测试实例，所以会创建一个 `_ErrorHolder` 对象（它具有与 [`TestCase`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestCase) 相同的接口）来代表该错误。 如果你只是使用标准 unittest 测试运行器那么这个细节并不重要，但是如果你是一个框架开发者那么这可能会有关系。
...
​	如果你希望在基类上的 `setUpClass` 和 `tearDownClass` 被调用则你必须自己去调用它们。 在 [`TestCase`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestCase) 中的实现是空的。

​	如果在 `setUpClass` 中引发了异常则类中的测试将不会被运行并且 `tearDownClass` 也不会被运行。 跳过的类中的 `setUpClass` 或 `tearDownClass` 将不会被运行。 如果引发的异常是 [`SkipTest`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.SkipTest) 异常则类将被报告为已跳过而非发生错误。
...
​	如果在 `setUpModule` 中引发了异常则模块中的任何测试都将不会被运行并且 `tearDownModule` 也不会被运行。 如果引发的异常是 [`SkipTest`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.SkipTest) 异常则模块将被报告为已跳过而非发生错误。

​	要添加即使在发生异常时也必须运行的清理代码，请使用 `addModuleCleanup`:

## unittest.**addModuleCleanup**(*function*, */*, **args*, ***kwargs*)

​	在 `tearDownModule()` 之后添加一个要调用的函数来清理测试类运行期间所使用的资源。 函数将按它们被添加的相反顺序被调用 (LIFO)。 它们在调用时将附带它们被添加时传给 [`addModuleCleanup()`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.addModuleCleanup) 的任何参数和关键字参数。
...
​	它将负责调用由It is responsible for calling all the cleanup functions added by [`addModuleCleanup()`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.addModuleCleanup) 添加的所有清理函数。 如果你需要在 `tearDownModule()` *之前* 调用清理函数则可以自行调用 [`doModuleCleanups()`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.doModuleCleanups)。
...
​	处理 control-C 信号的处理器会尝试与安装了自定义 [`signal.SIGINT`](https://docs.python.org/zh-cn/3.13/library/signal.html#signal.SIGINT) 处理器的测试代码保持兼容。 如果是 `unittest` 处理器而 *不是* 已安装的 [`signal.SIGINT`](https://docs.python.org/zh-cn/3.13/library/signal.html#signal.SIGINT) 处理器被调用，即它被系统在测试的下层替换并委托处理，则它会调用默认的处理器。 这通常会是替换了已安装处理器并委托处理的代码所预期的行为。 对于需要禁用 `unittest` control-C 处理的单个测试则可以使用 [`removeHandler()`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.removeHandler) 装饰器。

​	还有一些工具函数让框架开发者可以在测试框架内部启用 control-C 处理功能。

## unittest.**installHandler**()

​	安装 control-C 处理器。 当接收到 [`signal.SIGINT`](https://docs.python.org/zh-cn/3.13/library/signal.html#signal.SIGINT) 时（通常是响应用户按下 control-C）所有已注册的结果都会执行 [`stop()`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestResult.stop) 调用。

## unittest.**registerResult**(*result*)

​	注册一个 [`TestResult`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestResult) 对象用于 control-C 的处理。 注册一个结果将保存指向它的弱引用，因此这并不能防止结果被作为垃圾回收。

​	如果 control-C 未被启用则注册 [`TestResult`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestResult) 对象将没有任何附带影响，因此不论是否启用了该项处理测试框架都可以无条件地注册他们独立创建的所有结果。

## unittest.**removeResult**(*result*)

​	移除一个已注册的结果。 一旦结果被移除则 [`stop()`](https://docs.python.org/zh-cn/3.13/library/unittest.html#unittest.TestResult.stop) 将不再会作为针对 control-C 的响应在结果对象上被调用。
