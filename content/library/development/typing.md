...
[`type`](https://docs.python.org/zh-cn/3.13/reference/simple_stmts.html#type) 语句是在 Python 3.12 中新增加的。 为了向下兼容，类型别名也可以通过简单的赋值来创建:
...
​	然而，我们可以在 "派生的" `NewType` 的基础上创建一个 [`NewType`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.NewType)。
...
​	下标语法总是要刚好使用两个值：参数列表和返回类型。 参数列表必须是一个由类型组成的列表、[`ParamSpec`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.ParamSpec)、[`Concatenate`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.Concatenate) 或省略号。 返回类型必须是单一类型。
...
[`list`](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#list) 只接受一个类型参数，因此类型检查器将在上述代码中对 `y` 赋值时报告错误。同样，[`Mapping`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.Mapping) 只接受两个类型参数：第一个给出键的类型，第二个则给出值的类型。
...
*在 3.12 版本发生变更:* 3.12 版本新增了对泛型和类型别名的语法支持。在之前的版本中，泛型类必须显式继承自 [`Generic`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.Generic)，或者在其基类之一中包含有类型变量。
...
​	也就是说，可对 [`Any`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.Any) 类型的值执行任何操作或方法调用，并赋值给任意变量：
...
​	类型变量元组可以通过在 [类型形参列表](https://docs.python.org/zh-cn/3.13/reference/compound_stmts.html#type-params) 中使用名称前的单个星号 (`*`) 来声明:
...
1. 类型检查器不能对 `inner` 函数进行类型检查，因为 `*args` 和 `**kwargs` 的类型必须是 [`Any`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.Any)。
...
​	该协议可以与 [`isinstance()`](https://docs.python.org/zh-cn/3.13/library/functions.html#isinstance) 和 [`issubclass()`](https://docs.python.org/zh-cn/3.13/library/functions.html#issubclass) 一起使用。应用于非协议的类时，会触发 [`TypeError`](https://docs.python.org/zh-cn/3.13/library/exceptions.html#TypeError)。该指令支持简易结构检查，与 [`collections.abc`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#module-collections.abc) 的 [`Iterable`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.Iterable) 非常类似，只擅长做一件事。 例如：
...
`runtime_checkable()` 将只检查所需方法或属性是否存在，而不检查它们的类型签名或类型。 例如，[`ssl.SSLObject`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLObject) 是一个类，因此它通过了针对 [Callable](https://docs.python.org/zh-cn/3.13/library/typing.html#annotating-callables) 的 [`issubclass()`](https://docs.python.org/zh-cn/3.13/library/functions.html#issubclass) 检查。 但是，`ssl.SSLObject.__init__` 方法的存在只是引发 [`TypeError`](https://docs.python.org/zh-cn/3.13/library/exceptions.html#TypeError) 并附带更具信息量的消息，因此它无法调用 (实例化) [`ssl.SSLObject`](https://docs.python.org/zh-cn/3.13/library/ssl.html#ssl.SSLObject)。
...
*在 3.12 版本发生变更:* 一旦类被创建则运行时可检查协议的成员就会被视为在运行时“已冻结”。 在运行时可检查协议上打上猴子补丁属性仍然有效，但不会影响将对象与协议进行比较的 [`isinstance()`](https://docs.python.org/zh-cn/3.13/library/functions.html#isinstance) 检查。 请参阅 ["Python 3.12 有什么新变化](https://docs.python.org/zh-cn/3.13/whatsnew/3.12.html#whatsnew-typing-py312) 了解更多细节。

## *class* typing.**TypedDict**(*dict*)

​	把类型提示添加至字典的特殊构造器。在运行时，它是纯 [`dict`](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#dict)。
...
​	另一种创建 `TypedDict` 的方法是使用函数调用语法。第二个参数必须是一个 [`dict`](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#dict) 字面值：

```
Point2D = TypedDict('Point2D', {'x': int, 'y': int, 'label': str})
```

​	这种函数式语法允许定义因为是关键字或包含连字符而不是有效 [标识符](https://docs.python.org/zh-cn/3.13/reference/lexical_analysis.html#identifiers) 的键，例如：
...
​	默认情况下，所有的键都必须出现在一个 `TypedDict` 中。 可以使用 [`NotRequired`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.NotRequired) 将单独的键标记为非必要的:
...
​	一个 `total=False` `TypedDict` 中单独的键可以使用 [`Required`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.Required) 标记为必要的:
...
`TypedDict` 不能从非 `TypedDict` 类继承，除了 [`Generic`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.Generic)。 例如:
...
​	要创建与 Python 3.11 或更低版本兼容的泛型 `TypedDict`，请显式地从 [`Generic`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.Generic) 继承：
...
`TypedDict` 可以通过注解字典（参见 [注解最佳实践](https://docs.python.org/zh-cn/3.13/howto/annotations.html#annotations-howto) 了解更多关于注解的最佳实践）、 [`__total__`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.TypedDict.__total__) 、 [`__required_keys__`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.TypedDict.__required_keys__) 和 [`__optional_keys__`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.TypedDict.__optional_keys__) 进行内省。
...
​	该属性 *只* 反映传给当前 `TypedDict` 类的 `total` 参数的值，而不反映这个类在语义上是否完整。 例如，一个 `__total__` 被设为 `True` 的 `TypedDict` 可能有用 [`NotRequired`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.NotRequired) 标记的键，或者它可能继承自另一个设置了 `total=False` 的 `TypedDict`。 因此，使用 [`__required_keys__`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.TypedDict.__required_keys__) 和 [`__optional_keys__`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.TypedDict.__optional_keys__) 进行内省通常会更好。
...
`Point2D.__required_keys__` 和 `Point2D.__optional_keys__` 返回分别包含必要的和非必要的键的 [`frozenset`](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#frozenset) 对象。

​	标记为 [`Required`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.Required) 的键总是会出现在 `__required_keys__` 中而标记为 [`NotRequired`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.NotRequired) 的键总是会出现在 `__optional_keys__` 中。
...
​	对 [`ReadOnly`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.ReadOnly) 的支持反映在下列属性中：
...
*在 3.13 版本发生变更:* 添加了对 [`ReadOnly`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.ReadOnly) 限定符的支持。
...
​	下列协议由 typing 模块提供并已全被装饰为 [`可在运行时检查的`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.runtime_checkable)。
...
​	泛型 `IO[AnyStr]` 及其子类 `TextIO(IO[str])`、`BinaryIO(IO[bytes])` 表示 I/O 流——例如 [`open()`](https://docs.python.org/zh-cn/3.13/library/functions.html#open) 返回的对象——的类型。
...
​	在这里，标注允许类型检查器推断最后一种情况永远不会执行，因为 `arg` 要么是 [`int`](https://docs.python.org/zh-cn/3.13/library/functions.html#int) 要么是 [`str`](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#str)，而这两种选项都已被之前的情况覆盖了。

​	如果类型检查器发现对 `assert_never()` 的调用是可达的，它将报告一个错误。 举例来说，如果 `arg` 的类型标注改为 `int | str | float`，则类型检查器将报告一个错误指出 `unreachable` 为 [`float`](https://docs.python.org/zh-cn/3.13/library/functions.html#float) 类型。 对于通过类型检查的 `assert_never` 调用，参数传入的推断类型必须为兜底类型 [`Never`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.Never)，而不能为任何其他类型。
...
​	在运行时，此函数会将其参数类型打印到 [`sys.stderr`](https://docs.python.org/zh-cn/3.13/library/sys.html#sys.stderr) 并不加修改地返回该参数 (以允许该调用在表达式中使用):
...
​	将一个对象标记为提供类似 [`dataclass`](https://docs.python.org/zh-cn/3.13/library/dataclasses.html#dataclasses.dataclass) 行为的装饰器。

`dataclass_transform` 可被用于装饰类、元类或本身为装饰器的函数。 使用 `@dataclass_transform()` 将让静态类型检查器知道被装饰的对象会执行以类似 [`@dataclasses.dataclass`](https://docs.python.org/zh-cn/3.13/library/dataclasses.html#dataclasses.dataclass) 的方式来转换类的运行时“魔法”。
...
​	上面定义的 `CustomerModel` 类将被类型检查器视为类似于使用 [`@dataclasses.dataclass`](https://docs.python.org/zh-cn/3.13/library/dataclasses.html#dataclasses.dataclass) 创建的类。 例如，类型检查器将假定这些类具有接受 `id` 和 `name` 的 `__init__` 方法。

​	被装饰的类、元类或函数可以接受以下布尔值参数，类型检查器将假定它们具有与 [`@dataclasses.dataclass`](https://docs.python.org/zh-cn/3.13/library/dataclasses.html#dataclasses.dataclass) 装饰器相同的效果: `init`, `eq`, `order`, `unsafe_hash`, `frozen`, `match_args`, `kw_only` 和 `slots`。 这些参数的值 (`True` 或 `False`) 必须可以被静态地求值。

​	传给 `dataclass_transform` 装饰器的参数可以被用来定制被装饰的类、元类或函数的默认行为:

## 参数:

- **eq_default** ([*bool*](https://docs.python.org/zh-cn/3.13/library/functions.html#bool)) -- 指明如果调用方省略了 `eq` 形参则应将其假定为 `True` 还是 `False`。 默认为 `True`。
- **order_default** ([*bool*](https://docs.python.org/zh-cn/3.13/library/functions.html#bool)) -- 指明如果调用方省略了 `order` 形参则应将其假定为 `True` 还是 `False`。 默认为 `False`。
- **kw_only_default** ([*bool*](https://docs.python.org/zh-cn/3.13/library/functions.html#bool)) -- 指明如果调用方省略了 `kw_only` 形参则应将其假定为 `True` 还是 `False`。 默认为 `False`。
- **frozen_default** ([*bool*](https://docs.python.org/zh-cn/3.13/library/functions.html#bool)) -- 指明如果调用方省略了 `frozen` 形参则应将其假定为 `True` 还是 `False`。 默认为 `False`。 .. versionadded:: 3.12
- **field_specifiers** ([*tuple*](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#tuple)*[*[*Callable*](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.Callable)*[**...**,* *Any**]**,* *...**]*) -- 指定一个受支持的类或描述字段的函数的静态列表，类似于 [`dataclasses.field()`](https://docs.python.org/zh-cn/3.13/library/dataclasses.html#dataclasses.field)。 默认为 `()`。
...
​	以 `@overload` 装饰的定义仅对类型检查器有用，因为它们将被非 `@overload` 装饰的定义覆盖。 与此同时，非 `@overload` 装饰的定义将在运行时使用但应被类型检查器忽略。 在运行时，直接调用以 `@overload` 装饰的函数将引发 [`NotImplementedError`](https://docs.python.org/zh-cn/3.13/library/exceptions.html#NotImplementedError)。
...
*在 3.11 版本发生变更:* 现在可以使用 [`get_overloads()`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.get_overloads) 在运行时内省有重载的函数。

## typing.**get_overloads**(*func*)

​	为 *func* 返回以 [`@overload`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.overload) 装饰的定义的序列。

*func* 是用于实现过载函数的函数对象。 例如，根据文档中为 [`@overload`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.overload) 给出的 `process` 定义，`get_overloads(process)` 将为所定义的三个过载函数返回由三个函数对象组成的序列。 如果在不带过载的函数上调用，`get_overloads()` 将返回一个空序列。
...
​	此作用方式类似于类或函数的 [decorator](https://docs.python.org/zh-cn/3.13/glossary.html#term-decorator)。 对于类，它将递归地应用到该类中定义的所有方法和类（但不包括在其超类或子类中定义的方法）。 类型检查器将忽略带有此装饰器的函数或类的所有标注。

`@no_type_check` 将原地改变被装饰的对象。

## @typing.**no_type_check_decorator**

​	让其他装饰器具有 [`no_type_check()`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.no_type_check) 效果的装饰器。

​	本装饰器用 [`no_type_check()`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.no_type_check) 里的装饰函数打包其他装饰器。
...
- 以字符串字面形式或 [`ForwardRef`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.ForwardRef) 对象编码的前向引用会在 *globalns*, *localns* 和 (如适用) *obj* 的 [类型形参](https://docs.python.org/zh-cn/3.13/reference/compound_stmts.html#type-params) 命名空间中求值。如果没有传入 *globalns* 或 *localns*，则会从 *obj* 中推断出适当的命名空间字典。
- `None` 被替换为 [`types.NoneType`](https://docs.python.org/zh-cn/3.13/library/types.html#types.NoneType)。
- 如果在 *obj* 上应用了 [`@no_type_check`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.no_type_check)，返回一个空字典。
- 如果 *obj* 是一个类 `C`，则函数返回一个合并了 `C` 的基类与 `C` 上的基类的字典。 这是通过遍历 [`C.__mro__`](https://docs.python.org/zh-cn/3.13/reference/datamodel.html#type.__mro__) 并迭代地合并 `__annotations__` 字典来实现的。 在 [method resolution order](https://docs.python.org/zh-cn/3.13/glossary.html#term-method-resolution-order) 中较早出现的类上的标注总是会优先于在方法解析顺序中较晚出现的类的标注。
- 除非 *include_extras* 设置为 `True`，否则函数会递归地将所有出现的 `Annotated[T, ...]` 替换为 `T` (详见 [`Annotated`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.Annotated))。

​	另请参阅 [`inspect.get_annotations()`](https://docs.python.org/zh-cn/3.13/library/inspect.html#inspect.get_annotations)，这是一个以更直接方式返回注解的低级函数。

​	备注

 

​	如果 *obj* 注解中的任何前向引用不可解析或不是有效的 Python 代码，此函数将引发 [`NameError`](https://docs.python.org/zh-cn/3.13/library/exceptions.html#NameError) 等异常。例如导入的 [类型别名](https://docs.python.org/zh-cn/3.13/library/typing.html#type-aliases) 包含正向引用，或名称在 [`if TYPE_CHECKING`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.TYPE_CHECKING) 下导入。

*在 3.9 版本发生变更:* 增加了作为 [**PEP 593**](https://peps.python.org/pep-0593/) 组成部分的 `include_extras` 形参。 请参阅 [`Annotated`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.Annotated) 文档了解详情。
...
​	如果 `X` 是一个内置类型或 [`collections`](https://docs.python.org/zh-cn/3.13/library/collections.html#module-collections) 类在 typing 模块中的别名，它将被正规化为原始的类。 如果 `X` 是 [`ParamSpecArgs`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.ParamSpecArgs) 或 [`ParamSpecKwargs`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.ParamSpecKwargs) 的实例，则返回下层的 [`ParamSpec`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.ParamSpec)。 对于不受支持的对象将返回 `None`。
...
​	如果 `X` 是一个并集或是包含在另一个泛型类型中的 [`Literal`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.Literal)，则 `(Y, Z, ...)` 的顺序可能因类型缓存而与原始参数 `[Y, Z, ...]` 存在差异。 对于不受支持的对象将返回 `()`。
...
​	检查一个类型是否为 [`Protocol`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.Protocol)。
...
​	检查一个类型是否为 [`TypedDict`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.TypedDict)。
...
[`dict`](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#dict) 的已弃用的别名。

​	请注意，注释参数时，最好使用抽象的多项集类型，如 [`Mapping`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.Mapping)，而不是使用 [`dict`](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#dict) 或 `typing.Dict`。

*自 3.9 版本弃用:* [`builtins.dict`](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#dict) 现在支持下标操作 (`[]`)。 参见 [**PEP 585**](https://peps.python.org/pep-0585/) 和 [GenericAlias 类型](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#types-genericalias)。

## *class* typing.**List**(*list, MutableSequence[T]*)

[`list`](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#list) 的已弃用的别名。

​	请注意，注释参数时，最好使用抽象的多项集类型，如 [`Sequence`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.Sequence) 或 [`Iterable`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.Iterable)，而不是使用 [`list`](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#list) 或 `typing.List`。

*自 3.9 版本弃用:* [`builtins.list`](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#list) 现在支持下标操作 (`[]`)。 参见 [**PEP 585**](https://peps.python.org/pep-0585/) 和 [GenericAlias 类型](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#types-genericalias)。

## *class* typing.**Set**(*set, MutableSet[T]*)

[`builtins.set`](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#set) 的已弃用的别名。

​	请注意，注释参数时，最好使用抽象的多项集类型，如 [`collections.abc.Set`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.Set)，而不是使用 [`set`](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#set) 或 [`typing.Set`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.Set)。

*自 3.9 版本弃用:* [`builtins.set`](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#set) 现在支持下标操作 (`[]`)。 参见 [**PEP 585**](https://peps.python.org/pep-0585/) 和 [GenericAlias 类型](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#types-genericalias)。

## *class* typing.**FrozenSet**(*frozenset, AbstractSet[T_co]*)

[`builtins.frozenset`](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#frozenset) 的已弃用的别名。

*自 3.9 版本弃用:* [`builtins.frozenset`](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#frozenset) 现在支持下标操作 (`[]`)。 参见 [**PEP 585**](https://peps.python.org/pep-0585/) 和 [GenericAlias 类型](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#types-genericalias)。

## typing.**Tuple**

[`tuple`](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#tuple) 的已弃用的别名。

[`tuple`](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#tuple) 和 `Tuple` 是类型系统中的特例；更多详细信息请参见 [标注元组](https://docs.python.org/zh-cn/3.13/library/typing.html#annotating-tuples)。

*自 3.9 版本弃用:* [`builtins.tuple`](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#tuple) 现在支持下标操作（`[]`）。参见 [**PEP 585**](https://peps.python.org/pep-0585/) 和 [GenericAlias 类型](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#types-genericalias)。

## *class* typing.**Type**(*Generic[CT_co]*)

[`type`](https://docs.python.org/zh-cn/3.13/library/functions.html#type) 的已弃用的别名。
...
[`re.compile()`](https://docs.python.org/zh-cn/3.13/library/re.html#re.compile) 和 [`re.match()`](https://docs.python.org/zh-cn/3.13/library/re.html#re.match) 的返回类型的已弃用的别名。

​	这些类型（与对应的函数）是 [`AnyStr`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.AnyStr) 上的泛型。 `Pattern` 可以被特化为 `Pattern[str]` 或 `Pattern[bytes]`；`Match` 可以被特化为 `Match[str]` 或 `Match[bytes]`。

*自 3.9 版本弃用:* [`re`](https://docs.python.org/zh-cn/3.13/library/re.html#module-re) 模块中的 `Pattern` 与 `Match` 类现已支持 `[]`。详见 [**PEP 585**](https://peps.python.org/pep-0585/) 与 [GenericAlias 类型](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#types-genericalias)。

## *class* typing.**Text**

[`str`](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#str) 的已弃用的别名。
...
#### [`collections.abc`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#module-collections.abc) 中容器 ABC 的别名

## *class* typing.**AbstractSet**(*Collection[T_co]*)

[`collections.abc.Set`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.Set) 的已弃用的别名。

*自 3.9 版本弃用:* [`collections.abc.Set`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.Set) 现在支持下标操作 (`[]`)。 参见 [**PEP 585**](https://peps.python.org/pep-0585/) 和 [GenericAlias 类型](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#types-genericalias)。

## *class* typing.**ByteString**(*Sequence[int]*)

​	该类型代表了 [`bytes`](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#bytes)、[`bytearray`](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#bytearray)、[`memoryview`](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#memoryview) 等字节序列类型。

*Deprecated since version 3.9, will be removed in version 3.14:* 首选 [`collections.abc.Buffer`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.Buffer)，或是 `bytes | bytearray | memoryview` 这样的并集。
...
[`collections.abc.Container`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.Container) 的已弃用的别名。

*自 3.9 版本弃用:* [`collections.abc.Container`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.Container) 现在支持下标操作 (`[]`)。 参见 [**PEP 585**](https://peps.python.org/pep-0585/) 和 [GenericAlias 类型](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#types-genericalias)。

## *class* typing.**ItemsView**(*MappingView, AbstractSet[tuple[KT_co, VT_co]]*)

[`collections.abc.ItemsView`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.ItemsView) 的已弃用的别名。

*自 3.9 版本弃用:* [`collections.abc.ItemsView`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.ItemsView) 现在支持下标操作 (`[]`)。 参见 [**PEP 585**](https://peps.python.org/pep-0585/) 和 [GenericAlias 类型](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#types-genericalias)。

## *class* typing.**KeysView**(*MappingView, AbstractSet[KT_co]*)

[`collections.abc.KeysView`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.KeysView) 的已弃用的别名。

*自 3.9 版本弃用:* [`collections.abc.KeysView`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.KeysView) 现在支持下标操作 (`[]`)。 参见 [**PEP 585**](https://peps.python.org/pep-0585/) 和 [GenericAlias 类型](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#types-genericalias)。

## *class* typing.**Mapping**(*Collection[KT], Generic[KT, VT_co]*)

[`collections.abc.Mapping`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.Mapping) 的已弃用的别名。

*自 3.9 版本弃用:* [`collections.abc.Mapping`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.Mapping) 现在支持下标操作 (`[]`)。 参见 [**PEP 585**](https://peps.python.org/pep-0585/) 和 [GenericAlias 类型](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#types-genericalias)。

## *class* typing.**MappingView**(*Sized*)

[`collections.abc.MappingView`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.MappingView) 的已弃用的别名。

*自 3.9 版本弃用:* [`collections.abc.MappingView`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.MappingView) 现在支持下标操作 (`[]`)。 参见 [**PEP 585**](https://peps.python.org/pep-0585/) 和 [GenericAlias 类型](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#types-genericalias)。

## *class* typing.**MutableMapping**(*Mapping[KT, VT]*)

[`collections.abc.MutableMapping`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.MutableMapping) 的已弃用的别名。

*自 3.9 版本弃用:* [`collections.abc.MutableMapping`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.MutableMapping) 现在支持下标操作 (`[]`)。 参见 [**PEP 585**](https://peps.python.org/pep-0585/) 和 [GenericAlias 类型](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#types-genericalias)。

## *class* typing.**MutableSequence**(*Sequence[T]*)

[`collections.abc.MutableSequence`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.MutableSequence) 的已弃用的别名。

*自 3.9 版本弃用:* [`collections.abc.MutableSequence`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.MutableSequence) 现在支持下标操作 (`[]`)。 参见 [**PEP 585**](https://peps.python.org/pep-0585/) 和 [GenericAlias 类型](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#types-genericalias)。

## *class* typing.**MutableSet**(*AbstractSet[T]*)

[`collections.abc.MutableSet`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.MutableSet) 的已弃用的别名。

*自 3.9 版本弃用:* [`collections.abc.MutableSet`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.MutableSet) 现在支持下标操作 (`[]`)。 参见 [**PEP 585**](https://peps.python.org/pep-0585/) 和 [GenericAlias 类型](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#types-genericalias)。

## *class* typing.**Sequence**(*Reversible[T_co], Collection[T_co]*)

[`collections.abc.Sequence`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.Sequence) 的已弃用的别名。

*自 3.9 版本弃用:* [`collections.abc.Sequence`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.Sequence) 现在支持下标操作 (`[]`)。 参见 [**PEP 585**](https://peps.python.org/pep-0585/) 和 [GenericAlias 类型](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#types-genericalias)。

## *class* typing.**ValuesView**(*MappingView, Collection[_VT_co]*)

[`collections.abc.ValuesView`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.ValuesView) 的已弃用的别名。

*自 3.9 版本弃用:* [`collections.abc.ValuesView`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.ValuesView) 现在支持下标操作 (`[]`)。 参见 [**PEP 585**](https://peps.python.org/pep-0585/) 和 [GenericAlias 类型](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#types-genericalias)。



#### [`collections.abc`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#module-collections.abc) 中异步 ABC 的别名

## *class* typing.**Coroutine**(*Awaitable[ReturnType], Generic[YieldType, SendType, ReturnType]*)

[`collections.abc.Coroutine`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.Coroutine) 的已弃用的别名。
...
[`collections.abc.AsyncGenerator`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.AsyncGenerator) 的已弃用的别名。
...
#### [`collections.abc`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#module-collections.abc) 中其他 ABC 的别名

## *class* typing.**Iterable**(*Generic[T_co]*)

[`collections.abc.Iterable`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.Iterable) 的已弃用的别名

*自 3.9 版本弃用:* [`collections.abc.Iterable`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.Iterable) 现在支持下标操作 (`[]`)。 参见 [**PEP 585**](https://peps.python.org/pep-0585/) 和 [GenericAlias 类型](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#types-genericalias)。

## *class* typing.**Iterator**(*Iterable[T_co]*)

[`collections.abc.Iterator`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.Iterator) 的已弃用的别名。

*自 3.9 版本弃用:* [`collections.abc.Iterator`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.Iterator) 现在支持下标操作 (`[]`)。 参见 [**PEP 585**](https://peps.python.org/pep-0585/) 和 [GenericAlias 类型](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#types-genericalias)。

## typing.**Callable**

[`collections.abc.Callable`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.Callable) 的已弃用的别名。

​	有关如何在类型标注中使用 [`collections.abc.Callable`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.Callable) 和 `typing.Callable` 的详细信息请参阅 [标注可调用对象](https://docs.python.org/zh-cn/3.13/library/typing.html#annotating-callables)。

*自 3.9 版本弃用:* [`collections.abc.Callable`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.Callable) 现在支持下标操作（`[]`）。参见 [**PEP 585**](https://peps.python.org/pep-0585/) 和 [GenericAlias 类型](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#types-genericalias)。

*在 3.10 版本发生变更:* `Callable` 现在支持 [`ParamSpec`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.ParamSpec) 和 [`Concatenate`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.Concatenate)。 详情见 [**PEP 612**](https://peps.python.org/pep-0612/)。

## *class* typing.**Generator**(*Iterator[YieldType], Generic[YieldType, SendType, ReturnType]*)

[`collections.abc.Generator`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.Generator) 的已弃用的别名。

​	有关在注解类型中使用 [`collections.abc.Generator`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.Generator) 和 `typing.Generator` 的详细信息，请参见 [标注生成器和协程](https://docs.python.org/zh-cn/3.13/library/typing.html#annotating-generators-and-coroutines)。

*自 3.9 版本弃用:* [`collections.abc.Generator`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.Generator) 现在支持下标操作 (`[]`)。 参见 [**PEP 585**](https://peps.python.org/pep-0585/) 和 [GenericAlias 类型](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#types-genericalias)。

*在 3.13 版本发生变更:* 添加了发送和返回类型的默认值。

## *class* typing.**Hashable**

[`collections.abc.Hashable`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.Hashable) 的已弃用的别名。

*自 3.12 版本弃用:* 请改为直接使用 [`collections.abc.Hashable`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.Hashable)。

## *class* typing.**Reversible**(*Iterable[T_co]*)

[`collections.abc.Reversible`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.Reversible) 的已弃用的别名。

*自 3.9 版本弃用:* [`collections.abc.Reversible`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.Reversible) 现在支持下标操作 (`[]`)。 参见 [**PEP 585**](https://peps.python.org/pep-0585/) 和 [GenericAlias 类型](https://docs.python.org/zh-cn/3.13/library/stdtypes.html#types-genericalias)。

## *class* typing.**Sized**

[`collections.abc.Sized`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.Sized) 的已弃用的别名。

*自 3.12 版本弃用:* 请改为直接使用 [`collections.abc.Sized`](https://docs.python.org/zh-cn/3.13/library/collections.abc.html#collections.abc.Sized)。



#### [`contextlib`](https://docs.python.org/zh-cn/3.13/library/contextlib.html#module-contextlib) ABC 的别名

## *class* typing.**ContextManager**(*Generic[T_co, ExitT_co]*)

[`contextlib.AbstractContextManager`](https://docs.python.org/zh-cn/3.13/library/contextlib.html#contextlib.AbstractContextManager) 的已弃用的别名。
...
[`contextlib.AbstractAsyncContextManager`](https://docs.python.org/zh-cn/3.13/library/contextlib.html#contextlib.AbstractAsyncContextManager) 的已弃用的别名。
...
| 特性                                                         | 弃用于 | 计划移除                                                     | PEP/问题                                                     |
| :----------------------------------------------------------- | :----- | :----------------------------------------------------------- | :----------------------------------------------------------- |
| 标准容器的 `typing` 版本                                     | 3.9    | 未定（请参阅 [一些已被弃用的别名](https://docs.python.org/zh-cn/3.13/library/typing.html#deprecated-aliases) 了解详情） | [**PEP 585**](https://peps.python.org/pep-0585/)             |
| [`typing.ByteString`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.ByteString) | 3.9    | 3.14                                                         | [gh-91896](https://github.com/python/cpython/issues/91896)   |
| [`typing.Text`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.Text) | 3.11   | 未确定                                                       | [gh-92332](https://github.com/python/cpython/issues/92332)   |
| [`typing.Hashable`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.Hashable) 和 [`typing.Sized`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.Sized) | 3.12   | 未确定                                                       | [gh-94309](https://github.com/python/cpython/issues/94309)   |
| [`typing.TypeAlias`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.TypeAlias) | 3.12   | 未确定                                                       | [**PEP 695**](https://peps.python.org/pep-0695/)             |
| [`@typing.no_type_check_decorator`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.no_type_check_decorator) | 3.13   | 3.15                                                         | [gh-106309](https://github.com/python/cpython/issues/106309) |
| [`typing.AnyStr`](https://docs.python.org/zh-cn/3.13/library/typing.html#typing.AnyStr) | 3.13   | 3.18                                                         | [gh-105578](https://github.com/python/cpython/issues/105578) |
