...
- 对线程的支持。
- 对于 [`ssl`](https://docs.python.org/zh-cn/3.13/library/ssl.html#module-ssl) 和 [`hashlib`](https://docs.python.org/zh-cn/3.13/library/hashlib.html#module-hashlib) 扩展模块 OpenSSL 1.1.1 为最低版本而 OpenSSL 3.0.9 为推荐的最低版本。
- SQLite 3.15.2 用于 [`sqlite3`](https://docs.python.org/zh-cn/3.13/library/sqlite3.html#module-sqlite3) 扩展模块。
- Tcl/Tk 8.5.12 用于 [`tkinter`](https://docs.python.org/zh-cn/3.13/library/tkinter.html#module-tkinter) 模块。
...
*在 3.11 版本发生变更:* C11 编译器，现在需要 IEEE 754 和 NaN 支持。 在 Windows 上，需要 Visual Studio 2017 或更新的版本。 现在需要 Tcl/Tk version 8.5.12 用于 [`tkinter`](https://docs.python.org/zh-cn/3.13/library/tkinter.html#module-tkinter) 模块。
...
​	在 [`sqlite3`](https://docs.python.org/zh-cn/3.13/library/sqlite3.html#module-sqlite3) 模块的 `_sqlite` 扩展模块中是否支持可加载扩展（默认为否）。
...
​	禁用 IPv6 支持（若开启支持则默认启用），见 [`socket`](https://docs.python.org/zh-cn/3.13/library/socket.html#module-socket) 模块。

## **--enable-big-digits**=[15|30]

​	定义 Python [`int`](https://docs.python.org/zh-cn/3.13/library/functions.html#int) 数字的比特大小：15或30比特

​	在默认情况下，数位大小为 30。

​	定义 `PYLONG_BITS_IN_DIGIT` 为 `15` 或 `30`。

​	参见 [`sys.int_info.bits_per_digit`](https://docs.python.org/zh-cn/3.13/library/sys.html#sys.int_info) 。
...
​	Select the default time zone search path for为 [`zoneinfo.TZPATH`](https://docs.python.org/zh-cn/3.13/library/zoneinfo.html#zoneinfo.TZPATH) 选择默认的时区搜索路径。 参见 [`zoneinfo`](https://docs.python.org/zh-cn/3.13/library/zoneinfo.html#module-zoneinfo) 模块的 [编译时配置](https://docs.python.org/zh-cn/3.13/library/zoneinfo.html#zoneinfo-data-compile-time-config)。
...
​	编译 `_decimal` 扩展模块时使用线程本地上下文，而不是协程本地上下文（默认），参见 [`decimal`](https://docs.python.org/zh-cn/3.13/library/decimal.html#module-decimal) 模块。
...
​	覆盖 [`dbm`](https://docs.python.org/zh-cn/3.13/library/dbm.html#module-dbm) 模块的 db 后端检查顺序。
...
​	参阅 [`PYTHONCOERCECLOCALE`](https://docs.python.org/zh-cn/3.13/using/cmdline.html#envvar-PYTHONCOERCECLOCALE) 和 [**PEP 538**](https://peps.python.org/pep-0538/)。
...
[`ensurepip`](https://docs.python.org/zh-cn/3.13/library/ensurepip.html#module-ensurepip) 模块用到的 wheel 包目录（默认为无）。
...
- 增加了 [`-X pystats`](https://docs.python.org/zh-cn/3.13/using/cmdline.html#cmdoption-X) 命令行选项。
- 增加了 `PYTHONSTATS` 环境变量。
- 定义 `Py_STATS` 宏。
- 为 [`sys`](https://docs.python.org/zh-cn/3.13/library/sys.html#module-sys) 模块增加函数：
...
​	启用对不带 [global interpreter lock](https://docs.python.org/zh-cn/3.13/glossary.html#term-global-interpreter-lock) (GIL) 运行 Python 的 **实验性** 支持：自由线程构建版。

​	定义 `Py_GIL_DISABLED` 宏并向 [`sys.abiflags`](https://docs.python.org/zh-cn/3.13/library/sys.html#sys.abiflags) 添加 `"t"`。
...
​	Indicate how to integrate the [JIT compiler](https://docs.python.org/zh-cn/3.13/whatsnew/3.13.html#whatsnew313-jit-compiler).
...
​	将 Python 链接到 `libbz2` 的 C 编译器和链接器旗标，由 [`bz2`](https://docs.python.org/zh-cn/3.13/library/bz2.html#module-bz2) 模块使用，覆盖 `pkg-config`。

## **CURSES_CFLAGS**

## **CURSES_LIBS**

​	针对 `libncurses` 或 `libncursesw` 的 C 编译器和链接器旗标，由 [`curses`](https://docs.python.org/zh-cn/3.13/library/curses.html#module-curses) 模块使用，覆盖 `pkg-config`。
...
​	针对 `libb2` ([BLAKE2](https://docs.python.org/zh-cn/3.13/library/hashlib.html#hashlib-blake2)) 的 C 编译器和链接器旗标，由 [`hashlib`](https://docs.python.org/zh-cn/3.13/library/hashlib.html#module-hashlib) 模块使用，覆盖 `pkg-config`。

## **LIBEDIT_CFLAGS**

## **LIBEDIT_LIBS**

​	针对 `libedit` 的 C 编译器和链接器旗标，由 [`readline`](https://docs.python.org/zh-cn/3.13/library/readline.html#module-readline) 模块使用，覆盖 `pkg-config`。

## **LIBFFI_CFLAGS**

## **LIBFFI_LIBS**

​	针对 `libffi` 的 C 编译器和链接器旗标，由 [`ctypes`](https://docs.python.org/zh-cn/3.13/library/ctypes.html#module-ctypes) 模块使用，覆盖 `pkg-config`。

## **LIBMPDEC_CFLAGS**

## **LIBMPDEC_LIBS**

​	针对 `libmpdec` 的 C 编译器和链接器旗标，由 [`decimal`](https://docs.python.org/zh-cn/3.13/library/decimal.html#module-decimal) 模块使用，覆盖 `pkg-config`。

​	备注

 

​	除非指定了 [`--with-system-libmpdec`](https://docs.python.org/zh-cn/3.13/using/configure.html#cmdoption-with-system-libmpdec) 否则这些环境变量将没有效果。

## **LIBLZMA_CFLAGS**

## **LIBLZMA_LIBS**

​	针对 `liblzma` 的 C 编译器和链接器旗标，由 [`lzma`](https://docs.python.org/zh-cn/3.13/library/lzma.html#module-lzma) 模块使用，覆盖 `pkg-config`。

## **LIBREADLINE_CFLAGS**

## **LIBREADLINE_LIBS**

​	针对 `libreadline` 的 C 编译器和链接器旗标，由 [`readline`](https://docs.python.org/zh-cn/3.13/library/readline.html#module-readline) 模块使用，覆盖 `pkg-config`。

## **LIBSQLITE3_CFLAGS**

## **LIBSQLITE3_LIBS**

​	针对 `libsqlite3` 的 C 编译器和链接器旗标，由 [`sqlite3`](https://docs.python.org/zh-cn/3.13/library/sqlite3.html#module-sqlite3) 模块使用，覆盖 `pkg-config`。

## **LIBUUID_CFLAGS**

## **LIBUUID_LIBS**

​	针对 `libuuid` 的 C 编译器和链接器旗标，由 [`uuid`](https://docs.python.org/zh-cn/3.13/library/uuid.html#module-uuid) 模块使用，覆盖 `pkg-config`。
...
​	针对 `libpanel` 或 `libpanelw` 的 C 编译器和链接器旗标，由 [`curses.panel`](https://docs.python.org/zh-cn/3.13/library/curses.panel.html#module-curses.panel) 模块使用，覆盖 `pkg-config`。
...
​	针对 `libzlib` 的 C 编译器和链接器旗标，由 [`gzip`](https://docs.python.org/zh-cn/3.13/library/gzip.html#module-gzip) 模块使用，覆盖 `pkg-config`。
...
​	该值可在运行时使用 [`sys.prefix`](https://docs.python.org/zh-cn/3.13/library/sys.html#sys.prefix) 获取。

​	作为示例，用户可以使用 `--prefix="$HOME/.local/"` 在其家目录中安装 Python。

## **--exec-prefix**=EPREFIX

​	在 EPREFIX 中安装架构无关的文件，默认为 [`--prefix`](https://docs.python.org/zh-cn/3.13/using/configure.html#cmdoption-prefix)。

​	该值可在运行时使用 [`sys.exec_prefix`](https://docs.python.org/zh-cn/3.13/library/sys.html#sys.exec_prefix) 获取。
...
​	选择 Python 安装时运行的 [`ensurepip`](https://docs.python.org/zh-cn/3.13/library/ensurepip.html#module-ensurepip) 命令。
...
​	用 [`PROFILE_TASK`](https://docs.python.org/zh-cn/3.13/using/configure.html#envvar-PROFILE_TASK) 启用以配置文件主导的优化（PGO）（默认为禁用）。
...
​	禁用快速的 [mimalloc](https://docs.python.org/zh-cn/3.13/c-api/memory.html#mimalloc) 分配器（默认为启用）。

​	参见环境变量 [`PYTHONMALLOC`](https://docs.python.org/zh-cn/3.13/using/cmdline.html#envvar-PYTHONMALLOC) 。

## **--without-pymalloc**

​	禁用特定的 Python 内存分配器 [pymalloc](https://docs.python.org/zh-cn/3.13/c-api/memory.html#pymalloc) （默认为启用）。
...
​	调试版本 Python 是指带有 [`--with-pydebug`](https://docs.python.org/zh-cn/3.13/using/configure.html#cmdoption-with-pydebug) 参数的编译。

​	调试版本的效果：

- 默认显示所有警告：在 [`warnings`](https://docs.python.org/zh-cn/3.13/library/warnings.html#module-warnings) 模块中，默认警告过滤器的列表是空的。
- 在 [`sys.abiflags`](https://docs.python.org/zh-cn/3.13/library/sys.html#sys.abiflags) 中加入 `d` 标记。
- 加入 `sys.gettotalrefcount()` 函数。
- 命令行参数加入 [`-X showrefcount`](https://docs.python.org/zh-cn/3.13/using/cmdline.html#cmdoption-X) 。
- 添加 [`-d`](https://docs.python.org/zh-cn/3.13/using/cmdline.html#cmdoption-d) 命令行选项和 [`PYTHONDEBUG`](https://docs.python.org/zh-cn/3.13/using/cmdline.html#envvar-PYTHONDEBUG) 环境变量用于调试解析器。
- 添加对 `__lltrace__` 变量的支持：如果定义了该变量则会在字节码求值循环中启用低层级追踪。
- 安装 [内存分配调试钩子](https://docs.python.org/zh-cn/3.13/c-api/memory.html#default-memory-allocators) ，以便检测缓冲区溢出和其他内存错误。
- 定义宏 `Py_DEBUG` 和 `Py_REF_DEBUG` 。
- 增加运行时检查：针对由 `#ifdef Py_DEBUG` 和 `#endif` 所包裹的代码。 启用 `assert(...)` 和 `_PyObject_ASSERT(...)` 断言：不设置 `NDEBUG` 宏（另请参阅 [`--with-assertions`](https://docs.python.org/zh-cn/3.13/using/configure.html#cmdoption-with-assertions) 配置选项）。 主要的运行时检查有:
  - 增加了对函数参数的合理性检查。
  - 创建 Unicode 和 int 对象时，内存按某种模式进行了填充，用于检测是否使用了未初始化的对象。
  - 确保有能力清除或替换当前异常的函数在调用时不会引发异常。
  - 检查内存释放器函数是否不改变当前异常。
  - 垃圾收集器（[`gc.collect()`](https://docs.python.org/zh-cn/3.13/library/gc.html#gc.collect) 函数）对对象的一致性进行一些基本检查。
  - 从较宽类型转换到较窄类型时，`Py_SAFE_DOWNCAST()` 宏会检查整数下溢和上溢的情况。

​	参见 [Python 开发模式](https://docs.python.org/zh-cn/3.13/library/devmode.html#devmode) 和配置参数 [`--with-trace-refs`](https://docs.python.org/zh-cn/3.13/using/configure.html#cmdoption-with-trace-refs) 。

*在 3.8 版本发生变更:* 发布构建版和调试构建版现在是 ABI 兼容的：定义了 `Py_DEBUG` 宏不再意味着同时定义了 `Py_TRACE_REFS` 宏（参见 [`--with-trace-refs`](https://docs.python.org/zh-cn/3.13/using/configure.html#cmdoption-with-trace-refs) 选项）。

### 3.3.9. 调试选项

## **--with-pydebug**

[在调试模式下编译 Python](https://docs.python.org/zh-cn/3.13/using/configure.html#debug-build): 定义宏 `Py_DEBUG` (默认为禁用)。
...
- 定义 `Py_TRACE_REFS` 宏。
- Add [`sys.getobjects()`](https://docs.python.org/zh-cn/3.13/library/sys.html#sys.getobjects) function.
- 环境变量加入 [`PYTHONDUMPREFS`](https://docs.python.org/zh-cn/3.13/using/cmdline.html#envvar-PYTHONDUMPREFS) 。

[`PYTHONDUMPREFS`](https://docs.python.org/zh-cn/3.13/using/cmdline.html#envvar-PYTHONDUMPREFS) 环境变量可被用来转储在 Python 退出时仍然存活的对象和引用计数。
...
​	如果设置此参数，则在 [`OPT`](https://docs.python.org/zh-cn/3.13/using/configure.html#envvar-OPT) 编译器变量中不定义 `NDEBUG` 宏。
...
[`LIBMPDEC_CFLAGS`](https://docs.python.org/zh-cn/3.13/using/configure.html#cmdoption-arg-LIBMPDEC_CFLAGS) 和 [`LIBMPDEC_LIBS`](https://docs.python.org/zh-cn/3.13/using/configure.html#cmdoption-arg-LIBMPDEC_LIBS)。

## **--with-readline**=readline|editline

​	为 [`readline`](https://docs.python.org/zh-cn/3.13/library/readline.html#module-readline) 模块指定一个后端库。
...
​	不编译 [`readline`](https://docs.python.org/zh-cn/3.13/library/readline.html#module-readline) 模块（默认会）。
...
​	指定应创建何种通用二进制文件。该选项仅当设置了 [`--enable-universalsdk`](https://docs.python.org/zh-cn/3.13/using/configure.html#cmdoption-enable-universalsdk) 时才有效。
...
​	为 macOS 中的 python 框架指定名称，仅当设置了 [`--enable-framework`](https://docs.python.org/zh-cn/3.13/using/configure.html#cmdoption-enable-framework) 时有效（默认：`Python`）。
...
​	使用 profile-guided optimization (PGO) 构建 Python。 你可以使用 [`--enable-optimizations`](https://docs.python.org/zh-cn/3.13/using/configure.html#cmdoption-enable-optimizations) 配置选项来使其成为 `make` 命令的默认目标（即对应 `make all` 或更简洁的 `make` 命令）。
...
​	在 `make clean` 所做的工作之外，还移除由配置脚本所创建的文件。 再次构建之前将需要运行 `configure`。 [[1\]](https://docs.python.org/zh-cn/3.13/using/configure.html#id5)
...
​	重新生成（几乎）所有的已生成文件。 这包括（但不限于）字节码用例，以及解析器生成器文件。 对于其余的 [已生成文件](https://docs.python.org/zh-cn/3.13/using/configure.html#generated-files) 必须分别运行 `make regen-stdlib-module-names` 和 `autoconf`。
...
​	宏 `PyAPI_FUNC()`, `PyAPI_DATA()` 和 [`PyMODINIT_FUNC`](https://docs.python.org/zh-cn/3.13/c-api/intro.html#c.PyMODINIT_FUNC) 在 `Include/exports.h` 中的定义将因是否定义了 `Py_BUILD_CORE_MODULE` 宏而不同:

- 如果 `Py_BUILD_CORE_MODULE` 定义了，使用 `Py_EXPORTED_SYMBOL` 。
- 否则使用 `Py_IMPORTED_SYMBOL` 。

​	如果宏 `Py_BUILD_CORE_BUILTIN` 被错误地用在作为共享库构建的 C 扩展上，它的 `PyInit_*xxx*()` 函数就不会被导出，导致导入时出现 [`ImportError`](https://docs.python.org/zh-cn/3.13/library/exceptions.html#ImportError) 。
...
[`CPPFLAGS`](https://docs.python.org/zh-cn/3.13/using/configure.html#envvar-CPPFLAGS) 和 [`LDFLAGS`](https://docs.python.org/zh-cn/3.13/using/configure.html#envvar-LDFLAGS) 都需要包含 shell 的值以便能够使用环境变量中指定的目录构建扩展模块。
...
[`CFLAGS_NODIST`](https://docs.python.org/zh-cn/3.13/using/configure.html#envvar-CFLAGS_NODIST) 用于构建解释器和 stdlib C 扩展。 一旦装好 Python 则当某个编译器旗标 *不应* 成为 [`CFLAGS`](https://docs.python.org/zh-cn/3.13/using/configure.html#envvar-CFLAGS) 的一部分时将可使用它 ([gh-65320](https://github.com/python/cpython/issues/65320))。

​	特别地，[`CFLAGS`](https://docs.python.org/zh-cn/3.13/using/configure.html#envvar-CFLAGS) 不应当包含:
...
​	，默认为： `$(CCSHARED)` ，当 [`--enable-shared`](https://docs.python.org/zh-cn/3.13/using/configure.html#cmdoption-enable-shared) 被使用时，则为空字符串
...
​	编译器标志，将标准库的扩展模块作为内置模块来构建，如 [`posix`](https://docs.python.org/zh-cn/3.13/library/posix.html#module-posix) 模块
...
​	变量 [`LDFLAGS`](https://docs.python.org/zh-cn/3.13/using/configure.html#envvar-LDFLAGS) 的值被传递给 `./configure` 脚本。
...
[`LDFLAGS_NODIST`](https://docs.python.org/zh-cn/3.13/using/configure.html#envvar-LDFLAGS_NODIST) 的使用方式与 [`CFLAGS_NODIST`](https://docs.python.org/zh-cn/3.13/using/configure.html#envvar-CFLAGS_NODIST) 相同。 一旦装好 Python 则当某个链接器旗标 *不应* 成为 [`LDFLAGS`](https://docs.python.org/zh-cn/3.13/using/configure.html#envvar-LDFLAGS) 的一部分时将可使用它 ([gh-65320](https://github.com/python/cpython/issues/65320))。

​	特别地，[`LDFLAGS`](https://docs.python.org/zh-cn/3.13/using/configure.html#envvar-LDFLAGS) 不应当包含:

- 编译器旗标 `-L` (用于为库设置搜索路径)。 `-L` 旗标将按从左到右的顺序处理，并且 [`LDFLAGS`](https://docs.python.org/zh-cn/3.13/using/configure.html#envvar-LDFLAGS) 中的任何旗标都将优先于 user- 和 package 层级所提供的 `-L` 旗标。
...
[[1](https://docs.python.org/zh-cn/3.13/using/configure.html#id3)]
